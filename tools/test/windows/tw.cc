...
bool ExportRunfiles(const Path& cwd, const Path& test_srcdir, const std::wstring& env_prefix) {
  Path runfiles_dir;
  ...
  Path java_rf, py_rf;
  if (!GetPathEnv(L"JAVA_RUNFILES", &java_rf) || !GetPathEnv(L"PYTHON_RUNFILES", &py_rf)) {
    return false;
  }
  java_rf.Absolutize(cwd);
  py_rf.Absolutize(cwd);
  if (!UnsetEnv(L"JAVA_RUNFILES") ||
      !UnsetEnv(L"PYTHON_RUNFILES") ||
...
bool ParseArgs(int argc, wchar_t** argv, const Path& exec_root, const Path &srcdir,
               Path* out_executable, std::wstring* out_args, std::wstring* out_runfiles_env_prefix) {
  Path argv0;
  if (!argv0.Set(argv[0])) {
    return false;
  }
  ...
  if (!coverage_dir.empty()) {
    if (argc < 2) {
      LogError(__LINE__, "Usage: $0 <coverage_wrapper> <test_path> [test_args...]");
      return false;
    }
...
int TestWrapperMain(int argc, wchar_t** argv) {
  ...
  if (!AddCurrentDirectoryToPATH() || !GetCwd(&exec_root) ||
      ...
      !ParseArgs(argc, argv, exec_root, srcdir, &executable, &args, &runfiles_env_prefix) ||
      ...
      !ExportShardStatusFile(exec_root) ||
