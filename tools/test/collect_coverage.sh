...
EXEC_OUTPUT_FILE=${EXEC_OUTPUT_FILE:-"$COVERAGE_DIR/_test.exec"}
# make COVERAGE_OUTPUT_FILE and EXEC_OUTPUT_FILE an absolute path
if ! [[ $COVERAGE_OUTPUT_FILE == $ROOT* ]]; then
  COVERAGE_OUTPUT_FILE=$ROOT/$COVERAGE_OUTPUT_FILE
fi

if ! [[ $EXEC_OUTPUT_FILE == $ROOT* ]]; then
  EXEC_OUTPUT_FILE=$ROOT/$EXEC_OUTPUT_FILE
fi
