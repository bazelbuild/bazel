...
class BazelApiTest(test_base.TestBase):
  ...
  def setUp(self):
    ...
    self._bazel_api = BazelApi(lambda *args, **kwargs: self.RunBazel(allow_failure=True,*args,**kwargs))
  ...
  def testTransitiveFragmentsAccuracy(self):
    self.ScratchFile("MODULE.bazel", ["bazel_dep(name = 'rules_cc', version = '0.2.14')"])
    self.ScratchFile('testapp/BUILD', [
