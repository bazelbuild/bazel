...
def _java_runtime_alias(ctx):
    ...
    if java_common.is_java_toolchain_resolution_enabled_do_not_use(ctx = ctx):
        toolchain = ctx.toolchains["@rules_java//java/toolchains/runtime:toolchain_type"]
    else:
...
java_runtime_alias = rule(
    ...
    toolchains = ["@rules_java//java/toolchains/runtime:toolchain_type"],
    attrs = {
...
def _java_toolchain_alias(ctx):
    ...
    if java_common.is_java_toolchain_resolution_enabled_do_not_use(ctx = ctx):
        toolchain = ctx.toolchains["@rules_java//java/toolchains/javac:toolchain_type"]
    else:
...
java_toolchain_alias = rule(
    ...
    toolchains = ["@rules_java//java/toolchains/javac:toolchain_type"],
    attrs = {
