    ...
    if not vc_path or missing_tools:
        repository_ctx.template(
            ...
            paths["@bazel_tools//tools/cpp:CROSSTOOL.tpl"],
            {
                ...
                "%{content}": _get_escaped_windows_msys_crosstool_content(repository_ctx, "msys"),
                "%{msys_x64_mingw_content}": _get_escaped_windows_msys_crosstool_content(repository_ctx, "mingw"),
                "%{opt_content}": "",
                "%{dbg_content}": "",
                "%{link_content}": "",
