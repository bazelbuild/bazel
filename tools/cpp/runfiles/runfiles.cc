...
class DirectoryBased : public RunfilesImpl {
 public:
  DirectoryBased(
      string runfiles_path, const vector<pair<string, string> >&& envvars)
      : RunfilesImpl(std::move(envvars)),
...
Runfiles* RunfilesImpl::Create(const string& argv0,
  ...
  const vector<pair<string, string> > envvars =
    {{"RUNFILES_MANIFEST_FILE", manifest}, {"RUNFILES_DIR", directory},
...
ManifestBased* ManifestBased::Create(
  ...
  return ParseManifest(manifest_path, &runfiles, error)
             ? new ManifestBased(
                 manifest_path, std::move(runfiles), std::move(envvars))
             : nullptr;
}
...
Runfiles* Runfiles::CreateDirectoryBased(const string& directory_path,
  ...
  return new DirectoryBased(
      directory_path,
      {{"RUNFILES_MANIFEST_FILE", ""},
       {"RUNFILES_DIR", directory_path}, {"JAVA_RUNFILES", directory_path}});
}
