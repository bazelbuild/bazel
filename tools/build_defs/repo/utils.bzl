...
def patch(ctx, patches = None, patch_cmds = None, patch_cmds_win = None, patch_tool = None, patch_args = None, auth = None):
    ...
    purl_fragments = getattr(ctx.attr, "purl_fragments", [])
    if purl_fragments and not ctx.path("REPO.bazel").exists:
...
""".format(purl_kwargs = ",\n".join(["        {} = {}".format(k, repr(v)) for k, v in purl_fragments.items()])),
        )
        ctx.file(
            "REPO.bazel",
            """# Generated by Bazel. Add a custom REPO.bazel file to disable generation.
