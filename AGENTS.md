...
*   **Remote Execution:** If you have access to a Remote Build Execution (RBE) cluster, use `--config=remote` to significantly speed up your builds:
    ```bash
...
**Note:** Using a custom `--output_base` (e.g., `/tmp/ob-dev`) is crucial to avoid locking your main workspace's server and ensures your development environment remains isolated.
...
*   **Skyframe:** The incremental evaluation framework. Most core logic is implemented as `SkyFunction`s evaluating `SkyKey`s into `SkyValue`s.
...
For architectural details or introspection capabilities, see: `docs/contribute/codebase.mdx`
