...
final class RegularImmutableMap<K, V> extends ImmutableMap<K, V> {
  ...
  static final ImmutableMap<Object, Object> EMPTY =
      new RegularImmutableMap<>(
          (Entry<Object, Object>[]) ImmutableMap.EMPTY_ENTRY_ARRAY, null, 0, null);
  ...
  private final @Nullable Throwable nonSequenced;

  static <K, V> ImmutableMap<K, V> fromEntries(
      @Nullable Throwable nonSequenced, Entry<K, V>... entries) {
    return fromEntryArray(entries.length, entries, /* throwIfDuplicateKeys= */ true, nonSequenced);
  }
  ...
  static <K, V> ImmutableMap<K, V> fromEntryArray(
      ...
      @Nullable Throwable nonSequenced) {
    ...
    try {
      return fromEntryArrayCheckingBucketOverflow(
          n, entryArray, throwIfDuplicateKeys, nonSequenced);
  ...
  private static <K, V> ImmutableMap<K, V> fromEntryArrayCheckingBucketOverflow(
    ...
    if (duplicates != null) {
      ...
      if (newTableSize != tableSize) {
        return fromEntryArrayCheckingBucketOverflow(
            entries.length, entries, /* throwIfDuplicateKeys= */ true, nonSequenced);
      }
    }
    return new RegularImmutableMap<>(entries, table, mask, nonSequenced);
  }
  ...
  private RegularImmutableMap(
      Entry<K, V>[] entries,
      @CheckForNull @Nullable ImmutableMapEntry<K, V>[] table,
      int mask,
      @Nullable Throwable nonSequenced) {
  ...
  private static final class KeySet<K> extends IndexedImmutableSet<K> {
    ...
    int copyIntoArray(@Nullable Object[] dst, int offset) {
      map.checkSequenced();
      return super.copyIntoArray(dst, offset);
    }
