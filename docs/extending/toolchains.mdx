...
**TIP:** For a toolchain to match a platform, the platform must define *all* of
the toolchain's constraints. But the reverse isn't true: the platform may define
additional constraints not specified by the toolchain. This lets developers
...
@rules_cc//cc/private/toolchain:grep-includes (HOST)
