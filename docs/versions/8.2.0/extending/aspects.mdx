...
[aspects](/versions/8.2.0/rules/lib/globals/bzl#aspect) and provides simple and advanced
examples.
...
![Build graph](/versions/8.2.0/rules/build-graph.png "Build graph")

**Figure 1.** `BUILD` file dependency graph.

Bazel analyzes this dependency graph by calling an implementation function of
the corresponding [rule](/versions/8.2.0/extending/rules) (in this case "java_library") for every
...
![Build Graph with Aspect](/versions/8.2.0/rules/build-graph-aspects.png "Build graph with aspects")
...
the [`aspect`](/versions/8.2.0/rules/lib/globals/bzl#aspect) function.
...
functions. They return [providers](/versions/8.2.0/extending/rules#providers), can generate
[actions](/versions/8.2.0/extending/rules#actions), and take two arguments:

*  `target`: the [target](/versions/8.2.0/rules/lib/builtins/Target) the aspect is being applied to.
*   `ctx`: [`ctx`](/versions/8.2.0/rules/lib/builtins/ctx) object that can be used to access attributes
    and generate outputs and actions.

The implementation function can access the attributes of the target rule via
[`ctx.rule.attr`](/versions/8.2.0/rules/lib/builtins/ctx#rule). It can examine providers that are
...
[`--aspects`](/versions/8.2.0/reference/command-line-reference#flag--aspects)
...
[``--aspects_parameters``](/versions/8.2.0/reference/command-line-reference#flag--aspects_parameters)
...
[`InstrumentedFilesInfo`](/versions/8.2.0/rules/lib/providers/InstrumentedFilesInfo)
(which is taken from the aspect). This means that aspect implementations may
never return [`DefaultInfo`](/versions/8.2.0/rules/lib/providers/DefaultInfo).
...
* [`aspect` API reference](/versions/8.2.0/rules/lib/globals/bzl#aspect)
