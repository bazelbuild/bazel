...
This object holds information about the environment in which the build is running. See the [Rules page](https://bazel.build/versions/8.3.1/extending/rules#configurations) for more on the general concept of configurations.
...
A boolean that tells whether code coverage is enabled for this run. Note that this does not compute whether a specific rule should be instrumented for code coverage data collection. For that, see the [`ctx.coverage_instrumented`](/versions/8.3.1/rules/lib/builtins/ctx#coverage_instrumented) function.
