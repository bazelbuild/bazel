...
Specific instances of this type can be retrieved from the fields of the [apple\_common.platform](/versions/8.3.1/rules/lib/toplevel/apple_common#platform) struct:
...
More commonly, however, the [apple](/versions/8.3.1/rules/lib/fragments/apple) configuration fragment has fields/methods that allow rules to determine the platform for which a target is being built.
