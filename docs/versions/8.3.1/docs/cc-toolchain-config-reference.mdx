...
[`CcToolchainConfigInfo`](/versions/8.3.1/rules/lib/providers/CcToolchainConfigInfo) is a provider that provides the necessary level of
...
[`toolchain_config`](/versions/8.3.1/reference/be/c-cpp#cc_toolchain.toolchain_config) attribute of the
[`cc_toolchain`](/versions/8.3.1/reference/be/c-cpp#cc_toolchain) to your rule.
You can create the `CcToolchainConfigInfo` by calling
[`cc_common.create_cc_toolchain_config_info()`](/versions/8.3.1/rules/lib/toplevel/cc_common#create_cc_toolchain_config_info).
...
1.  User specifies a `cc_toolchain_suite` target in the `BUILD` file and points
    ...
    [`--crosstool_top` option](/versions/8.3.1/docs/user-manual#flag--crosstool_top).
...
<table>
  ...
  <tr>
   ...
   <td>Feature-level. This feature implies the specified feature(s). Enabling a feature also implicitly enables all features implied by it (that is, it functions recursively). Also provides the ability to factor common subsets of functionality out of a set of features, such as the common parts of sanitizers. Implied features cannot be disabled.</td>
  </tr>
  <tr>
   <td><pre>provides = ['feature']</pre></td>
   <td>Feature-level. Indicates that this feature is one of several mutually exclusive alternate features. For example, all of the sanitizers could specify <code>provides = ["sanitizer"]</code>. This improves error handling by listing the alternatives if the user asks for two or more mutually exclusive features at once.</td>
  </tr>
  <tr>
