...
[Optimizing Performance](/versions/8.3.1/rules/performance).

## Recommended flags

The flags are in the
[`bazelrc` configuration syntax](/versions/8.3.1/run/bazelrc#bazelrc-syntax-semantics), so
...
See the [JSON Profile documentation](/versions/8.3.1/rules/performance#performance-profiling) for
