...
*   **Bazel is extensible.** Many [languages](/versions/8.3.1/rules) are
...
1.  **Set up Bazel.** Download and [install Bazel](/versions/8.3.1/install).

2.  **Set up a project [workspace](/versions/8.3.1/concepts/build-ref#workspaces)**, which is a
...
3.  **Write a `BUILD` file**, which tells Bazel what to build and how to
    ...
    [Starlark](/versions/8.3.1/rules/language), a domain-specific language. (See example
...
4. **Run Bazel** from the [command line](/versions/8.3.1/reference/command-line-reference). Bazel
   places your outputs within the workspace.

In addition to building, you can also use Bazel to run
[tests](/versions/8.3.1/reference/test-encyclopedia) and [query](/versions/8.3.1/query/guide) the build
...
2.  **Analyzes** the inputs and their
    [dependencies](/versions/8.3.1/concepts/dependencies), applies the specified build
    rules, and produces an [action](/versions/8.3.1/extending/concepts#evaluation-model)
    graph.
...
[hermetically](/versions/8.3.1/basics/hermeticity) through sandboxing, minimizing skew
and maximizing [reproducibility](/versions/8.3.1/run/build#correct-incremental-rebuilds).
...
[track](/versions/8.3.1/run/build#build-consistency) changes to
...
easily [trace dependencies](/versions/8.3.1/query/guide) in your code.

## Getting started tutorials

To get started with Bazel, see [Getting Started](/versions/8.3.1/start/) or jump
directly to the Bazel tutorials:

*   [Tutorial: Build a C++ Project](/versions/8.3.1/start/cpp)
*   [Tutorial: Build a Java Project](/versions/8.3.1/start/java)
*   [Tutorial: Build an Android Application](/versions/8.3.1/start/android-app)
*   [Tutorial: Build an iOS Application](/versions/8.3.1/start/ios-app)
