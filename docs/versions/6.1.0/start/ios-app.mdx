...
*   Set up the environment by installing Bazel and Xcode, and downloading the
    sample project
*   Set up a Bazel [workspace](/versions/6.1.0/concepts/build-ref#workspace) that contained the source code
...
Follow the [installation instructions](/versions/6.1.0/install/) to install Bazel and
its dependencies.
...
to [external dependencies](/versions/6.1.0/docs/external) required to build the
software.
...
[`git_repository`](/versions/6.1.0/reference/be/workspace#git_repository)
...
[`objc_library`](/versions/6.1.0/reference/be/objective-c#objc_library) rule to tell Bazel
