...
*configuration conditions*, which are labels referencing [`config_setting`](/versions/6.1.0/reference/be/general#config_setting)
...
[`values`](/versions/6.1.0/reference/be/general#config_setting.values) attribute is non-configurable.
...
[configuration transition](/versions/6.1.0/reference/glossary#transition).
...
[`config_setting`](/versions/6.1.0/reference/be/general#config_setting) or
[`constraint_value`](/versions/6.1.0/reference/be/platform#constraint_value).
...
[`config_setting`](/versions/6.1.0/reference/be/general#config_setting)'s
[`values`](/versions/6.1.0/reference/be/general#config_setting.values) attribute:
...
[`--show_progress`](/versions/6.1.0/docs/user-manual#show-progress) isn't allowed because
...
These are triggered with [`config_setting`](/versions/6.1.0/reference/be/general#config_setting)'s
[`flag_values`](/versions/6.1.0/reference/be/general#config_setting.flag_values)
attribute:
...
[`--define`](/versions/6.1.0/reference/command-line-reference#flag--define)
...
[define_values](/versions/6.1.0/reference/be/general#config_setting.define_values) attribute
...
you want to build a target.
   [Platforms](/versions/6.1.0/docs/platforms)
...
[documentation](/versions/6.1.0/concepts/platforms-intro) for details.
...
This can be customized with the [`no_match_error`](/versions/6.1.0/reference/be/functions#select)
attribute:
...
because macros are evaluated in Bazel's [loading phase](/versions/6.1.0/docs/build#loading),
...
Bazel [`query`](/versions/6.1.0/docs/query-how-to) operates over Bazel's
[loading phase](/versions/6.1.0/reference/glossary#loading-phase).
...
[analysis phase](/versions/6.1.0/reference/glossary#analysis-phase)).
So it can't determine which `select()` branches are chosen.

Bazel [`cquery`](/versions/6.1.0/docs/cquery) operates after Bazel's analysis phase, so it has
...
documentation on [rules](/versions/6.1.0/rules/rules) and [macros](/versions/6.1.0/rules/macros)
...
Because [`bind()`](/versions/6.1.0/reference/be/workspace#bind) is a WORKSPACE rule, not a BUILD rule.
...
Instead, you should use [`alias()`](/versions/6.1.0/reference/be/general#alias), with a `select()` in
...
use [cquery](/versions/6.1.0/docs/cquery) and `bazel config` to debug:
...
[cquery docs](/versions/6.1.0/docs/cquery) for guidance on using `somepath` to get the right
one.
