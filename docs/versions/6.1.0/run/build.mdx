...
To run Bazel, go to your base [workspace](/versions/6.1.0/concepts/build-ref#workspace) directory
...
* [`analyze-profile`](/versions/6.1.0/docs/user-manual#analyze-profile): Analyzes build profile data.
* [`aquery`](/versions/6.1.0/docs/user-manual#aquery): Executes a query on the [post-analysis](#analysis) action graph.
* [`build`](#bazel-build): Builds the specified targets.
* [`canonicalize-flags`](/versions/6.1.0/docs/user-manual#canonicalize-flags): Canonicalize Bazel flags.
* [`clean`](/versions/6.1.0/docs/user-manual#clean): Removes output files and optionally stops the server.
* [`cquery`](/versions/6.1.0/docs/cquery): Executes a [post-analysis](#analysis) dependency graph query.
* [`dump`](/versions/6.1.0/docs/user-manual#dump): Dumps the internal state of the Bazel server process.
* [`help`](/versions/6.1.0/docs/user-manual#help): Prints help for commands, or the index.
* [`info`](/versions/6.1.0/docs/user-manual#info): Displays runtime info about the bazel server.
* [`fetch`](#fetching-external-dependencies): Fetches all external dependencies of a target.
* [`mobile-install`](/versions/6.1.0/docs/user-manual#mobile-install): Installs apps on mobile devices.
* [`query`](/versions/6.1.0/docs/query-how-to): Executes a dependency graph query.
* [`run`](/versions/6.1.0/docs/user-manual#running-executables): Runs the specified target.
* [`shutdown`](/versions/6.1.0/docs/user-manual#shutdown): Stops the Bazel server.
* [`test`](/versions/6.1.0/docs/user-manual#running-tests): Builds and runs the specified test targets.
* [`version`](/versions/6.1.0/docs/user-manual#version): Prints version information for Bazel.

### Getting help {:#getting-help}

* `bazel help {{ '<var>' }}command{{ '</var>' }}`: Prints help and options for
  `{{ '<var>' }}command{{ '</var>' }}`.
* `bazel help `[`startup_options`](/versions/6.1.0/docs/user-manual#startup-options): Options for the JVM hosting Bazel.
...
[`--show_result`](/versions/6.1.0/docs/user-manual#show-result) for more details.
...
Whereas [labels](/versions/6.1.0/concepts/labels) are used to specify individual
...
<tr>
  ...
  from <a href="/versions/6.1.0/docs/external">external repositories</a>.</td>
</tr>
<tr>
  <td><code>//:all</code></td>
...
Using the
[`--distdir=/path/to-directory`](/versions/6.1.0/reference/command-line-reference#flag--distdir)
...
[`--distdir`](/versions/6.1.0/reference/command-line-reference#flag--distdir)
...
build target (`//foo:bin`) and one or more of the tools, for example some base
libraries. Such libraries must be built multiple times, for the exec and target
configurations. Bazel takes care of ensuring that all variants are built, and
that the derived files are kept separate to avoid interference; usually such
targets can be built concurrently, since they are independent of each other. If
you see progress messages indicating that a given target is being built multiple
times, this is most likely the explanation.

The exec configuration is derived from the target configuration as follows:

-   The execution platform for the request target becomes the target platform for
...
output files. See [Hermeticity](/versions/6.1.0/concepts/hermeticity) for more details.
...
such as Bazel's [query](/versions/6.1.0/docs/query-how-to) command, which is implemented atop the loading
phase.
