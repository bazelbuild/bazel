...
appropriate
[toolchains](/versions/6.1.0/docs/toolchains)
...
rule to write [configurable attributes](/versions/6.1.0/docs/configurable-attributes).
...
The space of possible choices for platforms is defined by using the
 [`constraint_setting`](/versions/6.1.0/reference/be/platform#constraint_setting) and
 [`constraint_value`](/versions/6.1.0/reference/be/platform#constraint_value) rules within `BUILD` files. `constraint_setting` creates a new dimension, while
...
The [`platform`](/versions/6.1.0/reference/be/platform#platform) rule introduces a new platform with
...
[`target_compatible_with`](/versions/6.1.0/reference/be/common-definitions#common.target_compatible_with)
...
Incompatible tests in a [`test_suite`](/versions/6.1.0/reference/be/general#test_suite) are
similarly skipped if the `test_suite` is specified on the command line with
[`--expand_test_suites`](/versions/6.1.0/reference/command-line-reference#flag--expand_test_suites).
...
[`constraint_value`](/versions/6.1.0/reference/be/platform#constraint_value) that no platform
satisfies.

Use [`select()`](/versions/6.1.0/reference/be/functions#select) in combination with
...
You can use the
[`IncompatiblePlatformProvider`](/versions/6.1.0/rules/lib/IncompatiblePlatformProvider)
