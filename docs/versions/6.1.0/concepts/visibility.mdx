...
[analysis](/versions/6.1.0/reference/glossary#analysis-phase).
...
[Concepts and terminology](/versions/6.1.0/concepts/build-ref).
...
[`visibility`](/versions/6.1.0/reference/be/common-definitions#common.visibility) attribute on
...
*   `"//some_pkg:my_package_group"`: Grants access to all of the packages that
    are part of the given [`package_group`](/versions/6.1.0/reference/be/functions#package_group).

    *   Package groups use a
        [different syntax](/versions/6.1.0/reference/be/functions#package_group.packages) for
...
2. The value of the
[`default_visibility`](/versions/6.1.0/reference/be/functions#package.default_visibility)
argument of the [`package`](/versions/6.1.0/reference/be/functions#package) statement in the
...
[`exports_files`](/versions/6.1.0/reference/be/functions#exports_files). When no `visibility`
...
[`config_setting`](/versions/6.1.0/reference/be/general#config_setting) targets that are
referenced in the keys of a [`select()`](/versions/6.1.0/reference/be/functions#select). There
...
Some rules have [implicit dependencies](/versions/6.1.0/extending/rules#private_attributes_and_implicit_dependencies) —
...
`.bzl` file but not list it in the `srcs` of a [`filegroup`](/versions/6.1.0/reference/be/general#filegroup),
...
[`visibility()`](/versions/6.1.0/rules/lib/globals#visibility) function from within the file.
The argument to `visibility()` is a list of package specifications, just like
the [`packages`](/versions/6.1.0/reference/be/functions#package_group.packages) attribute of
...
[`includes`](/versions/6.1.0/reference/be/functions#package_group.includes) attribute.
