...
An [external repository](/versions/7.2.0/docs/external) is a rule that can be used only
...
[repository_rule](/versions/7.2.0/rules/lib/globals/bzl#repository_rule) function to create a new
...
[`local_repository`](https://bazel.build/versions/7.2.0/reference/be/workspace#local_repository.repo_mapping)
and
[`new_local_repository`](https://bazel.build/versions/7.2.0/reference/be/workspace#new_local_repository.repo_mapping).
...
from the Internet). See [the library](/versions/7.2.0/rules/lib/builtins/repository_ctx) for more
context. Example:
...
repository) depends on it or if it is mentioned on the command line. The
...
* The value of any environment variable passed to `repository_ctx`'s
  ...
  [`repository_rule`](/versions/7.2.0/rules/lib/globals/bzl#repository_rule). The values
  of these environment variables can be hard-wired on the command line with the
  [`--repo_env`](/versions/7.2.0/reference/command-line-reference#flag--repo_env) flag.
...
[`repository_rule`](/versions/7.2.0/rules/lib/globals#repository_rule)
