...
*  [Tutorial: Building a C++ project](/versions/7.2.0/start/cpp)
*  [C++ common use cases](/versions/7.2.0/tutorials/cpp-use-cases)
*  [C/C++ rules](/versions/7.2.0/reference/be/c-cpp)
*  Essential Libraries
...
*  [C++ toolchain configuration](/versions/7.2.0/docs/cc-toolchain-config-reference)
*  [Tutorial: Configuring C++ toolchains](/versions/7.2.0/tutorials/ccp-toolchain-config)
*  [Integrating with C++ rules](/versions/7.2.0/configure/integrate-cpp)
...
*  Each `BUILD` file should contain one [`cc_library`](/versions/7.2.0/reference/be/c-cpp#cc_library)
...
*  For legacy or `third_party` code that requires includes pointing outside the
   ...
   use the [`include_prefix`](/versions/7.2.0/reference/be/c-cpp#cc_library.include_prefix) and
   [`strip_include_prefix`](/versions/7.2.0/reference/be/c-cpp#cc_library.strip_include_prefix)
   arguments on the `cc_library` rule target.

### Toolchain features {:#toolchain-features}

The following optional [features](/versions/7.2.0/docs/cc-toolchain-config-reference#features)
...
[`repo`](/versions/7.2.0/external/overview#repo.bazel),
[`package`](/versions/7.2.0/reference/be/functions#package) or `cc_*` rules:
...
* The `parse_headers` feature makes it so that the C++ compiler is used to parse
  ...
  when using the
  [`--process_headers_in_dependencies`](/versions/7.2.0/reference/command-line-reference#flag--process_headers_in_dependencies)
