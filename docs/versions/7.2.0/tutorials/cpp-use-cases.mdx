...
[Introduction to Bazel: Build a C++ Project](/versions/7.2.0/start/cpp).

For information on cc_library and hdrs header files, see
<a href="/versions/7.2.0/reference/be/c-cpp#cc_library">cc_library</a>.

## Including multiple files in a target {:#multiple-files-target}

You can include multiple files in a single target with
<a href="/versions/7.2.0/reference/be/functions#glob">glob</a>.
For example:
