...
* [`canonicalize-flags`](/versions/9.1.0/docs/user-manual#canonicalize-flags): Canonicalize Bazel flags.
* [`clean`](/versions/9.1.0/docs/user-manual#clean): Removes output files and optionally stops the server.
* [`cquery`](/versions/9.1.0/query/cquery): Executes a [post-analysis](#analysis) dependency graph query.
* [`dump`](/versions/9.1.0/docs/user-manual#dump): Dumps the internal state of the Bazel server process.
* [`help`](/versions/9.1.0/docs/user-manual#help): Prints help for commands, or the index.
* [`info`](/versions/9.1.0/docs/user-manual#info): Displays runtime info about the bazel server.
* [`fetch`](#fetching-external-dependencies): Fetches all external dependencies of a target.
* [`mobile-install`](/versions/9.1.0/docs/user-manual#mobile-install): Installs apps on mobile devices.
* [`query`](/versions/9.1.0/query/guide): Executes a dependency graph query.
* [`run`](/versions/9.1.0/docs/user-manual#running-executables): Runs the specified target.
* [`shutdown`](/versions/9.1.0/docs/user-manual#shutdown): Stops the Bazel server.
...
[`--show_result`](/versions/9.1.0/docs/user-manual#show-result) for more details.
...
Whereas [labels](/versions/9.1.0/concepts/labels) are used to specify individual
...
<tr>
  ...
  from <a href="/versions/9.1.0/docs/external">external repositories</a>.</td>
</tr>
<tr>
  <td><code>//:all</code></td>
...
Using the
[`--distdir=/path/to-directory`](/versions/9.1.0/reference/command-line-reference#flag--distdir)
...
[`--repository_cache`](/versions/9.1.0/reference/command-line-reference#flag--repository_cache)
...
[`--distdir`](/versions/9.1.0/reference/command-line-reference#flag--distdir)
flag. You will need to do this once for every new Bazel binary version, since
the implicit dependencies can be different for every release.
...
output files. See [Hermeticity](/versions/9.1.0/basics/hermeticity) for more details.
...
such as Bazel's [query](/versions/9.1.0/query/guide) command, which is implemented atop the loading
phase.
