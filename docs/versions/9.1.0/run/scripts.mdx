...
invocation [at a time](/versions/UNSAFE_release-9.1.0/run/client-server#clientserver-implementation).
...
By default, Bazel uses a long-running [server process](/versions/UNSAFE_release-9.1.0/run/client-server) as an
...
By default, Bazel reads the [`.bazelrc` file](/versions/UNSAFE_release-9.1.0/run/bazelrc) from the base
...
See the [Performance Profiling](/versions/UNSAFE_release-9.1.0/rules/performance#performance-profiling) section.
