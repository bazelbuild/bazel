...
[strategy flags](/versions/9.1.0/docs/user-manual#strategy-options). Using the `sandboxed`
...
[Persistent workers](/versions/9.1.0/remote/persistent) run in a generic sandbox if you pass
...
sandboxes [persistent workers](/versions/9.1.0/remote/persistent).
...
-   Sandboxing effectively disables any cache the tool may have. You can
    mitigate this by using [persistent workers](/versions/9.1.0/remote/persistent), at
    the cost of weaker sandbox guarantees.

-   [Multiplex workers](/versions/9.1.0/remote/multiplex) require explicit worker support
