...
[Optimizing Performance](/versions/UNSAFE_release-9.1.0/rules/performance).

## Recommended flags

The flags are in the
[`bazelrc` configuration syntax](/versions/UNSAFE_release-9.1.0/run/bazelrc#bazelrc-syntax-semantics), so
...
See the [JSON Profile documentation](/versions/UNSAFE_release-9.1.0/rules/performance#performance-profiling) for
