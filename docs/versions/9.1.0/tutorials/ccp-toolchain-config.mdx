...
1.  If you have not already done so, [download and install Bazel
    7.0.2](https://bazel.build/versions/9.1.0/install) or later.
...
6.  Run the build again. Bazel throws the following error:
    ...
    [`cxx_builtin_include_directories`](/versions/9.1.0/rules/lib/toplevel/cc_common#create_cc_toolchain_config_info)
...
-   You need to specify a matching `platforms` flag in the command line for
    ...
    configuration flags](/versions/9.1.0/concepts/platforms).
-   You have to let the toolchain know where the tools live. In this tutorial
    ...
    [external dependencies](/versions/9.1.0/external/overview). Your tools could come from a
...
configuration](/versions/9.1.0/docs/cc-toolchain-config-reference)
