...
*   `function(...)`: the standard [functions](/versions/UNSAFE_release-9.1.0/query/language#functions)
...
[options](/versions/UNSAFE_release-9.1.0/reference/command-line-reference#build-options)
...
see the [core query ordering contract](/versions/UNSAFE_release-9.1.0/query/language#graph-order).
...
[Skyframe](/versions/UNSAFE_release-9.1.0/reference/skyframe) is the evaluation and
incrementality model of Bazel. On each instance of Bazel server, Skyframe stores the dependency graph
constructed from the previous runs of the [Analysis phase](/versions/UNSAFE_release-9.1.0/run/build#analysis).
...
It is possible for [Aspects](/versions/UNSAFE_release-9.1.0/extending/aspects)
...
[dependency graph](/versions/UNSAFE_release-9.1.0/extending/aspects#aspect_basics).
...
their inputs/outputs), the [JSON profile](/versions/UNSAFE_release-9.1.0/rules/performance#performance-profiling)
