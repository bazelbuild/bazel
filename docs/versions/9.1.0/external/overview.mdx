...
-   **[Module Extensions](extension)**: The
    [`use_repo_rule`](/versions/UNSAFE_release-9.1.0/rules/lib/globals/module#use_repo_rule) and module
...
[`http_archive`](/versions/UNSAFE_release-9.1.0/rules/lib/repo/http#http_archive), which downloads an archive
from a URL and extracts it, and
[`local_repository`](/versions/UNSAFE_release-9.1.0/reference/be/workspace#local_repository), which symlinks a
...
See [fetch options](/versions/UNSAFE_release-9.1.0/reference/command-line-reference#fetch-options) for more
information about controlling fetch.

### Directory layout

After being fetched, the repo can be found in the subdirectory `external` in the
[output base](/versions/UNSAFE_release-9.1.0/remote/output-directories), under its canonical name.
...
The [`REPO.bazel`](/versions/UNSAFE_release-9.1.0/rules/lib/globals/repo) file is used to mark the topmost
...
See the [full list](/versions/UNSAFE_release-9.1.0/rules/lib/globals/workspace) of functions available in
`WORKSPACE` files.
