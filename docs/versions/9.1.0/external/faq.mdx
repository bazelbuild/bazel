...
The [`compatibility_level`](/versions/9.1.0/external/module#compatibility_level) of a Bazel module
...
*   Ensuring that the version listed in MODULE.bazel is consistent with build
    ...
    by using the
    [`native.module_version`](/versions/9.1.0/rules/lib/toplevel/native#module_version) method
...
*   Splitting up a very large MODULE.bazel file into manageable sections,
    ...
    [`include`](/versions/9.1.0/rules/lib/globals/module#include) directive to split its
...
*   Users of the old WORKSPACE system might remember declaring a repo, and then
    ...
    capability has been replaced by [module extensions](/versions/9.1.0/external/extension).
...
Bazel instead uses [Minimal Version Selection](/versions/9.1.0/external/module#version-selection) like
...
SemVer](/versions/9.1.0/external/module#version-format), so what makes sense in a strict SemVer
...
an [`extension_metadata`](/versions/9.1.0/rules/lib/builtins/module_ctx#extension_metadata)
...
[canonical name](/versions/9.1.0/external/overview#canonical-repo-name) `@@foo`, what does the [apparent
repo name](/versions/9.1.0/external/overview#apparent-repo-name) `@bar` resolve to? Alternatively, what
...
visibility" table](/versions/9.1.0/external/migration#repository-visibility) in the migration guide can
...
*   If the context repo is the main repo (`@@`):
    1.  If `bar` is an apparent repo name introduced by the root module's
        ...
        [`bazel_dep`](/versions/9.1.0/rules/lib/globals/module#bazel_dep.repo_name),
        [`use_repo`](/versions/9.1.0/rules/lib/globals/module#use_repo),
        [`module`](/versions/9.1.0/rules/lib/globals/module#module.repo_name),
        [`use_repo_rule`](/versions/9.1.0/rules/lib/globals/module#use_repo_rule)), then `@bar`
...
the [`bazel vendor`](/versions/9.1.0/external/vendor) command.
...
*   Use `--host_jvm_args=-Djava.net.preferIPv6Addresses=true` [startup
    option](/versions/9.1.0/docs/user-manual#startup-options), for example by adding the
    following line in your [`.bazelrc` file](/versions/9.1.0/run/bazelrc):
...
*   When running Java build targets that need to connect to the internet (such
    ...
    flag](/versions/9.1.0/docs/user-manual#jvmopt). For example, include in your [`.bazelrc`
    file](/versions/9.1.0/run/bazelrc):
