...
*   Ensuring that the version listed in MODULE.bazel is consistent with build
    ...
    by using the
    [`native.module_version`](/versions/UNSAFE_release-9.1.0/rules/lib/toplevel/native#module_version) method
...
*   Splitting up a very large MODULE.bazel file into manageable sections,
    ...
    [`include`](/versions/UNSAFE_release-9.1.0/rules/lib/globals/module#include) directive to split its
...
an [`extension_metadata`](/versions/UNSAFE_release-9.1.0/rules/lib/builtins/module_ctx#extension_metadata)
...
*   If the context repo is the main repo (`@@`):
    1.  If `bar` is an apparent repo name introduced by the root module's
        ...
        [`bazel_dep`](/versions/UNSAFE_release-9.1.0/rules/lib/globals/module#bazel_dep.repo_name),
        [`use_repo`](/versions/UNSAFE_release-9.1.0/rules/lib/globals/module#use_repo),
        [`module`](/versions/UNSAFE_release-9.1.0/rules/lib/globals/module#module.repo_name),
        [`use_repo_rule`](/versions/UNSAFE_release-9.1.0/rules/lib/globals/module#use_repo_rule)), then `@bar`
...
*   Use `--host_jvm_args=-Djava.net.preferIPv6Addresses=true` [startup
    ...
    following line in your [`.bazelrc` file](/versions/UNSAFE_release-9.1.0/run/bazelrc):
