...
IMPORTANT: This tutorial is for [*legacy macros*](/versions/9.1.0/extending/legacy-macros). If
you only need to support Bazel 8 or newer, we recommend using [symbolic
macros](/versions/9.1.0/extending/macros) instead; take a look at [Creating a Symbolic
Macro](/versions/9.1.0/rules/macro-tutorial).
...
creating a [rule](/versions/9.1.0/extending/rules). Rules give you more control and flexibility.
...
"/versions/9.1.0//other/package:image.png"`, or even `src = select(...)`?
...
if possible, instead using [symbolic macros](/versions/9.1.0/extending/macros), which have
