...
*elements*. The [type name](/versions/9.1.0/rules/lib/globals/all#type) of a set is `"set"`.
...
[dictionary](/versions/9.1.0/rules/lib/core/dict), elements of a set must be hashable. A value may be used as an
element of a set if and only if it may be used as a key of a dictionary.

Sets may be constructed using the [`set()`](/versions/9.1.0/rules/lib/globals/all#set) built-in
...
retrieved using the [`len()`](/versions/9.1.0/rules/lib/globals/all#len) built-in function, and the
