...
A pseudo-struct containing all the predeclared output files, represented by [`File`](../builtins/File) objects. See the [Rules page](https://bazel.build/versions/9.1.0/extending/rules#files) for more information and examples.
...
| `execution_requirements` | [dict](../core/dict); default is `&lcub;&rcub;`  Information for scheduling the action to resolve this command. See [tags](/versions/9.1.0/reference/be/common-definitions#common.tags) for useful keys. |
...
| `collect_data` | [bool](../core/bool); default is `False`  **Use of this parameter is not recommended. See [runfiles guide](https://bazel.build/versions/9.1.0/extending/rules#runfiles)**. Whether to collect the data runfiles from the dependencies in srcs, data and deps attributes. |
| `collect_default` | [bool](../core/bool); default is `False`  **Use of this parameter is not recommended. See [runfiles guide](https://bazel.build/versions/9.1.0/extending/rules#runfiles)**. Whether to collect the default runfiles from the dependencies in srcs, data and deps attributes. |
