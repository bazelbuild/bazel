...
more information see [here](/versions/9.1.0/extending/depsets).

The elements of a depset must be hashable and all of the same type (as defined by the built-in
[`type(x)`](/versions/9.1.0/rules/lib/globals/all#type) function), but depsets are not simply hash
sets and do not support fast membership tests. If you need a general set datatype, use the core
[Starlark set](/versions/9.1.0/rules/lib/core/set) type (available since Bazel 8.1); if your .bzl file needs to
...
[constructor function](/versions/9.1.0/rules/lib/globals/bzl#depset) and merged or augmented with other
