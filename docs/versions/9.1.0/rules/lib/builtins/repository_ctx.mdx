...
| `auth` | [dict](../core/dict); default is `&lcub;&rcub;`  An optional dict specifying authentication information for some of the URLs. |
| `headers` | [dict](../core/dict); default is `&lcub;&rcub;`  An optional dict specifying http headers for all URLs. |
...
| `environment` | [dict](../core/dict); default is `&lcub;&rcub;`  Force some environment variables to be set to be passed to the process. The value can be `None` to remove the environment variable. |
...
| `attrs_for_reproducibility` | [dict](../core/dict); default is `&lcub;&rcub;`  If `reproducible` is False, this can be specified to tell Bazel which attributes of the original repo rule to change to make it reproducible. |
