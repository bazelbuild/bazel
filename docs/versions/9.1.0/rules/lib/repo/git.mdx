...
Prefer [`http_archive`](/versions/UNSAFE_release-9.1.0/rules/lib/repo/http#http_archive) to `git_repository`.
The reasons are:
...
* `http_archive` supports a list of `urls` as mirrors, and `git_repository` supports only
  a single `remote`.
* `http_archive` works with the [repository cache](/versions/UNSAFE_release-9.1.0/run/build#repository-cache), but not
  `git_repository`. See
