...
functions and types, see the [Bazel API reference](/versions/UNSAFE_release-9.1.0/rules/lib/overview).
...
[Native functions](/versions/UNSAFE_release-9.1.0/reference/be/functions) and [native rules](
...
* `import` (see [`load`](/versions/UNSAFE_release-9.1.0/extending/concepts#loading-an-extension) statement).
* `while`, `yield`.
* float and set types.
