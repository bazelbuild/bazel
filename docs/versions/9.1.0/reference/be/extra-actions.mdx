...
[aspects](https://bazel.build/versions/UNSAFE_release-9.1.0/rules/aspects)
instead.

An `action_listener` rule doesn't produce any output itself.
Instead, it allows tool developers to insert
[`extra_action`](/versions/UNSAFE_release-9.1.0/reference/be/extra-actions#extra_action)s into the build system,
by providing a mapping from action to [`extra_action`](/versions/UNSAFE_release-9.1.0/reference/be/extra-actions#extra_action).

This rule's arguments map action mnemonics to
[`extra_action`](/versions/UNSAFE_release-9.1.0/reference/be/extra-actions#extra_action) rules.

By specifying the option [`--experimental_action_listener=&lt;label&gt;`](/versions/UNSAFE_release-9.1.0/docs/user-manual#flag--experimental_action_listener),
the build will use the specified `action_listener` to insert
[`extra_action`](/versions/UNSAFE_release-9.1.0/reference/be/extra-actions#extra_action)s into the build graph.
...
| `extra_actions` | List of [labels](/versions/UNSAFE_release-9.1.0/concepts/labels); required A list of `extra_action` targets this `action_listener` should add to the build graph. E.g. `[ "//my/tools:analyzer" ]`. |
...
See [`action_listener`](/versions/UNSAFE_release-9.1.0/reference/be/extra-actions#action_listener) for details
