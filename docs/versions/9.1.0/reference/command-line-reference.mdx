...
See the [User's Guide](/versions/9.1.0/docs/build#specifying-build-targets) for the
...
:   If true, enables the &lt;code&gt;isolate&lt;/code&gt; parameter in the &lt;a href="https://bazel.build/versions/9.1.0/rules/lib/globals/module#use_extension"&gt;&lt;code&gt;use_extension&lt;/code&gt;&lt;/a&gt; function.
...
:   If true, native rules add `DefaultInfo.files` of data dependencies to their runfiles,
    which matches the recommended behavior for Starlark rules ([runfiles features to avoid](https://bazel.build/versions/9.1.0/extending/rules#runfiles_features_to_avoid)).
...
:   Skip incompatible targets that are explicitly listed on the command line.
    ...
    silently skipped when this option is enabled. See: [Skipping incompatible targets](https://bazel.build/versions/9.1.0/extending/platforms#skipping-incompatible-targets)
...
:   Whether to run validation actions as part of the build. See [Validation Actions](https://bazel.build/versions/9.1.0/extending/rules#validation_actions).
