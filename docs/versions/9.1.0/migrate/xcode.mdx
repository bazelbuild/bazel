...
1.  [Install Bazel](/versions/UNSAFE_release-9.1.0/install) if you have not already done so.
...
Add an [`objc_library`](/versions/UNSAFE_release-9.1.0/reference/be/objective-c#objc_library) target for each
...
Note: You can use the [`glob`](/versions/UNSAFE_release-9.1.0/reference/be/functions#glob) function to include
