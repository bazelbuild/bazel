...
[estimating cache hits](/versions/9.1.0/remote/cache-local).
...
4. Connect to Cloud Storage by adding the following flags to your Bazel command:
   * Pass the following URL to Bazel by using the flag:
       `--remote_cache=https://storage.googleapis.com/bucket-name` where `bucket-name` is the name of your storage bucket.
...
* [Adapting Rules for Remote Execution](/versions/9.1.0/remote/rules)
* [Troubleshooting Remote Execution](/versions/9.1.0/remote/sandbox)
