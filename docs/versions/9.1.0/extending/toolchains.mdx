...
recommended to read the [rules](/versions/UNSAFE_release-9.1.0/extending/rules) and [platforms](/versions/UNSAFE_release-9.1.0/extending/platforms)
...
[based on the platform](/versions/UNSAFE_release-9.1.0/docs/configurable-attributes):
...
[`ToolchainInfo` provider](/versions/UNSAFE_release-9.1.0/rules/lib/toplevel/platform_common#ToolchainInfo)
...
The [`config_common.toolchain_type`](/versions/UNSAFE_release-9.1.0/rules/lib/toplevel/config_common#toolchain_type)
...
3. For each such target, an associated target of the generic
  [`toolchain`](/versions/UNSAFE_release-9.1.0/reference/be/platforms-and-toolchains#toolchain)
...
analyzed, what [configuration](/versions/UNSAFE_release-9.1.0/reference/glossary#configuration) does it see, and what transitions
...
The use of [`attr.label`](/versions/UNSAFE_release-9.1.0/rules/lib/toplevel/attr#label) is the same as for a standard rule,
...
e.g. [`alias()`](/versions/UNSAFE_release-9.1.0/reference/be/general#alias).
...
Note: [Some Bazel rules](/versions/UNSAFE_release-9.1.0/concepts/platforms#status) do not yet support
toolchain resolution.
...
and
[`register_toolchains`](/versions/UNSAFE_release-9.1.0/rules/lib/globals/module#register_toolchains) calls in
`MODULE.bazel` files.
Additional execution platforms and toolchains may also be specified on the
command line via
[`--extra_execution_platforms`](/versions/UNSAFE_release-9.1.0/reference/command-line-reference#flag--extra_execution_platforms)
and
[`--extra_toolchains`](/versions/UNSAFE_release-9.1.0/reference/command-line-reference#flag--extra_toolchains).
...
1. If the target being built specifies the
   [`exec_compatible_with` attribute](/versions/UNSAFE_release-9.1.0/reference/be/common-definitions#common.exec_compatible_with)
   (or its rule definition specifies the
   [`exec_compatible_with` argument](/versions/UNSAFE_release-9.1.0/rules/lib/globals/bzl#rule.exec_compatible_with)),
...
If the rule uses [execution groups](/versions/UNSAFE_release-9.1.0/extending/exec-groups), each execution
...
If you'd like to see which [`cquery`](/versions/UNSAFE_release-9.1.0/query/cquery) dependencies are from toolchain
resolution, use `cquery`'s [`--transitions`](/versions/UNSAFE_release-9.1.0/query/cquery#transitions) flag:
