...
[aspects](/versions/UNSAFE_release-9.1.0/rules/lib/globals/bzl#aspect) and provides simple and advanced
examples.
...
![Build graph](/versions/UNSAFE_release-9.1.0/rules/build-graph.png "Build graph")

**Figure 1.** `BUILD` file dependency graph.

Bazel analyzes this dependency graph by calling an implementation function of
the corresponding [rule](/versions/UNSAFE_release-9.1.0/extending/rules) (in this case "java_library") for every
...
![Build Graph with Aspect](/versions/UNSAFE_release-9.1.0/rules/build-graph-aspects.png "Build graph with aspects")
...
the [`aspect`](/versions/UNSAFE_release-9.1.0/rules/lib/globals/bzl#aspect) function.
...
[the documentation of the aspect function](/versions/UNSAFE_release-9.1.0/rules/lib/globals/bzl#aspect).
...
functions. They return [providers](/versions/UNSAFE_release-9.1.0/extending/rules#providers), can generate
[actions](/versions/UNSAFE_release-9.1.0/extending/rules#actions), and take two arguments:

*  `target`: the [target](/versions/UNSAFE_release-9.1.0/rules/lib/builtins/Target) the aspect is being applied to.
*   `ctx`: [`ctx`](/versions/UNSAFE_release-9.1.0/rules/lib/builtins/ctx) object that can be used to access attributes
    and generate outputs and actions.

The implementation function can access the attributes of the target rule via
[`ctx.rule.attr`](/versions/UNSAFE_release-9.1.0/rules/lib/builtins/ctx#rule). It can examine providers that are
...
[`--aspects`](/versions/UNSAFE_release-9.1.0/reference/command-line-reference#flag--aspects)
...
[``--aspects_parameters``](/versions/UNSAFE_release-9.1.0/reference/command-line-reference#flag--aspects_parameters)
...
never return [`DefaultInfo`](/versions/UNSAFE_release-9.1.0/rules/lib/providers/DefaultInfo).
...
* [`aspect` API reference](/versions/UNSAFE_release-9.1.0/rules/lib/globals/bzl#aspect)
