...
releases, check the [Bzlmod migration guide](/versions/7.6.0/external/migration) on how to
migrate.
...
See [Repository rules](/versions/7.6.0/extending/repo) for more information about how to write
your own repository rules.

The most common repo rules by far are
[`http_archive`](/versions/7.6.0/rules/lib/repo/http#http_archive), which downloads an archive
from a URL and extracts it, and
[`local_repository`](/versions/7.6.0/reference/be/workspace#local_repository), which symlinks a
...
See [fetch options](/versions/7.6.0/reference/command-line-reference#fetch-options) for more
information about controlling fetch.

### Directory layout {:#directory-layout}

After being fetched, the repo can be found in the subdirectory `external` in the
[output base](/versions/7.6.0/remote/output-directories), under its canonical name.
...
A [Bazel **module**](/versions/7.6.0/external/module) is a Bazel project that can have multiple
...
[Bazel registry](/versions/7.6.0/external/registry) — by default, the [Bazel Central
...
consumed by [**module extensions**](/versions/7.6.0/external/extension) after module resolution
...
See the [full list](/versions/7.6.0/rules/lib/globals/workspace) of functions available in
`WORKSPACE` files.
