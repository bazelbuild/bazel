...
* Run steps to
  [debug local cache hits](/versions/7.6.0/remote/cache-remote#troubleshooting-cache-hits)
...
* Discover and fix hermeticity problems using
  [remote execution rules](/versions/7.6.0/remote/rules#overview).
* Enable strict [sandboxing](/versions/7.6.0/docs/sandboxing)
  at the per-action level, since actions in a build can be stateful and affect
  the build or the output.
* [Workspace rules](/versions/7.6.0/remote/workspace)
