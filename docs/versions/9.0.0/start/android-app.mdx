...
[Android rules](/versions/9.0.0/reference/be/android).
...
* **Bazel.** To install, follow the [installation instructions](/versions/9.0.0/install).
...
![Button greeting](/versions/9.0.0/docs/images/android_tutorial_app.png "Tutorial app button greeting")
...
A [workspace](/versions/9.0.0/concepts/build-ref#workspace) is a directory that contains the
source files for one or more software projects, and has a `MODULE.bazel` file at
its root.

The `MODULE.bazel` file may be empty or may contain references to [external dependencies](/versions/9.0.0/external/overview) required to build your project.
...
For more information, read [Using the Android Native Development Kit with Bazel](/versions/9.0.0/docs/android-ndk).
...
A [`BUILD` file](/versions/9.0.0/concepts/build-files) describes the relationship
...
structure in your workspace. Each [package](/versions/9.0.0/concepts/build-ref#packages) is a
...
[build rule](/versions/9.0.0/concepts/build-files#types-of-build-rules), which tells
...
[`android_library`](/versions/9.0.0/reference/be/android#android_library) and
[`android_binary`](/versions/9.0.0/reference/be/android#android_binary), that you can use to
...
The [`android_binary`](/versions/9.0.0/reference/be/android#android_binary) rule builds
...
The [`build`](/versions/9.0.0/docs/user-manual#build) subcommand instructs Bazel to build the
...
For more details about target labels and paths, see [Labels](/versions/9.0.0/concepts/labels).
...
project's [bazelrc file](/versions/9.0.0/run/bazelrc). The mobile-install-specific flags
...
![Bazel tutorial app](/versions/9.0.0/docs/images/android_tutorial_before.png "Bazel tutorial app")
...
* More information on [mobile-install](/versions/9.0.0/docs/mobile-install)
...
* Run Robolectric tests with the [robolectric-bazel](https://github.com/robolectric/robolectric-bazel)
  integration.
* Integrating C and C++ code into your Android app with the [NDK](/versions/9.0.0/docs/android-ndk)
