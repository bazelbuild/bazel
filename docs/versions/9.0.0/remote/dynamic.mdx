...
remote execution set up, go to the Bazel [Remote Execution Overview](/versions/9.0.0/remote/rbe) first.
...
to add roundtrip latency. You can use the [Bazel profiling features](/versions/9.0.0/rules/performance#performance-profiling) to look at how long typical
cache hits take.

Dynamic execution can be used with local sandboxed strategy as well as with
[persistent workers](/versions/9.0.0/remote/persistent). Persistent workers will automatically
run with sandboxing when used with dynamic execution, and cannot use [multiplex workers](/versions/9.0.0/remote/multiplex). On Darwin and Windows systems, the sandboxed
...
Dynamic execution requires some form of [remote execution system](/versions/9.0.0/remote/rbe).
...
the use of [persistent workers](/versions/9.0.0/remote/persistent), or those that run fast
...
[performance profiling](/versions/9.0.0/rules/performance#performance-profiling) contains data
...
<img width="596px" alt="Profiling data with poor dynamic execution performance"
 src="/versions/9.0.0/docs/images/dyn-trace-alldynamic.png" />
</p>
...
<img width="596px" alt="Profiling data with better dynamic execution performance"
 src="/versions/9.0.0/docs/images/dyn-trace-javaconly.png" />
</p>
...
[`--experimental_worker_memory_limit_mb`](https://bazel.build/versions/9.0.0/reference/command-line-reference#flag--experimental_worker_memory_limit_mb),
[`--experimental_worker_sandbox_hardening`](https://bazel.build/versions/9.0.0/reference/command-line-reference#flag--experimental_worker_sandbox_hardening),
and
[`--experimental_sandbox_memory_limit_mb`)](https://bazel.build/versions/9.0.0/reference/command-line-reference#flag--experimental_sandbox_memory_limit_mb)),
where worker processes may be killed when they use too many resources.

The [JSON trace profile](/versions/9.0.0/advanced/performance/json-trace-profile) contains a
