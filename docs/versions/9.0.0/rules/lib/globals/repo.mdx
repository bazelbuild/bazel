...
| --- | --- |
| `dirs` | [sequence](/versions/9.0.0/rules/lib/core/list) of [string](/versions/9.0.0/rules/lib/core/string)s; required |
...
| Parameter | Description |
| --- | --- |
| `kwargs` | required  The `repo()` function accepts exactly the same arguments as the [`package()`](/versions/9.0.0/reference/be/functions#package) function in BUILD files. |