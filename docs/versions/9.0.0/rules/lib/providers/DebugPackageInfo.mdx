...
| --- | --- |
| `target_label` | [Label](/versions/9.0.0/rules/lib/builtins/Label); required  The label for the \*\_binary target |
| `stripped_file` | [File](/versions/9.0.0/rules/lib/builtins/File); or `None`; default is `None`  The stripped file (the explicit ".stripped" target) |
| `unstripped_file` | [File](/versions/9.0.0/rules/lib/builtins/File); required  The unstripped file (the default executable target). |
| `dwp_file` | [File](/versions/9.0.0/rules/lib/builtins/File); or `None`; default is `None`  The .dwp file (for fission builds) or null if --fission=no. |
