...
*   `function(...)`: the standard [functions](/versions/9.0.0/query/language#functions)
...
[options](/versions/9.0.0/reference/command-line-reference#build-options)
...
see the [core query ordering contract](/versions/9.0.0/query/language#graph-order).
...
[Skyframe](/versions/9.0.0/reference/skyframe) is the evaluation and
incrementality model of Bazel. On each instance of Bazel server, Skyframe stores the dependency graph
constructed from the previous runs of the [Analysis phase](/versions/9.0.0/run/build#analysis).
...
It is possible for [Aspects](/versions/9.0.0/extending/aspects)
...
[dependency graph](/versions/9.0.0/extending/aspects#aspect_basics).
...
their inputs/outputs), the [JSON profile](/versions/9.0.0/rules/performance#performance-profiling)
