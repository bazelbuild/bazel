...
[loading phase](/versions/9.0.0/extending/concepts#evaluation-model), legacy macros don't exist
...
Where possible you should use [symbolic macros](/versions/9.0.0/extending/macros#macros).
...
*   Take typed attributes, which in turn means automatic label and select
    conversion.
*   Are more readable
*   Will soon have [lazy evaluation](/versions/9.0.0/extending/macros#laziness)
...
Note: `$@` is a
[Make variable](/versions/9.0.0/reference/be/make-variables#predefined_genrule_variables) that
...
from the [native](/versions/9.0.0/rules/lib/toplevel/native) module:
...
[native.package_name()](/versions/9.0.0/rules/lib/toplevel/native#package_name). Note that
...
[loading phase](/versions/9.0.0/extending/concepts#evaluation-model), label strings such as
...
[`Label`](/versions/9.0.0/rules/lib/builtins/Label#Label) constructor:
...
[native.package_relative_label()](/versions/9.0.0/rules/lib/toplevel/native#package_relative_label).
...
*   You can also use [print](/versions/9.0.0/rules/lib/globals/all#print) for debugging. It
...
If you want to throw an error, use the [fail](/versions/9.0.0/rules/lib/globals/all#fail)
