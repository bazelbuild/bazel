...
*   define custom flags for your project, obsoleting the need for
     [`--define`](/versions/9.0.0/docs/configurable-attributes#custom-keys)
*   write
    [transitions](/versions/9.0.0/rules/lib/builtins/transition#transition) to configure deps in
...
[configuration](/versions/9.0.0/extending/rules#configurations)
...
[attribute](/versions/9.0.0/rules/lib/globals/bzl#rule.build_setting).
...
[documentation](/versions/9.0.0/rules/lib/toplevel/config)  for details. More complicated typing can be
...
via default values and [transitions](/versions/9.0.0/rules/lib/builtins/transition#transition).
...
Like all rules, build setting rules have [implementation functions](/versions/9.0.0/extending/rules#implementation-function).
...
[`ctx`](/versions/9.0.0/rules/lib/builtins/ctx) objects of build setting rules. These implementation
...
the [`configuration_field`](/versions/9.0.0/rules/lib/globals/bzl#configuration_field)
 API.
...
Users can configure attributes on build settings by using
 [`select()`](/versions/9.0.0/reference/be/functions#select). Build setting targets can be passed to the `flag_values` attribute of
...
A configuration
[transition](/versions/9.0.0/rules/lib/builtins/transition#transition)
...
`transition()`
[function](/versions/9.0.0/rules/lib/builtins/transition#transition)
...
such as
[`--spawn_strategy`](/versions/9.0.0/docs/user-manual#spawn-strategy)
...
[target platform](/versions/9.0.0/extending/platforms).
...
![Scalability graph](/versions/9.0.0/rules/scalability-graph.png "Scalability graph")
...
This produces the following [configured targets](/versions/9.0.0/reference/glossary#configured-target):
