...
[`--generate_json_trace_profile`](/versions/9.0.0/reference/command-line-reference#flag--generate_json_trace_profile)
flag, and the location it is written to with the
[`--profile`](/versions/9.0.0/docs/user-manual#profile) flag. Locations ending with `.gz` are
compressed with GZIP. Bazel keeps the last 5 profiles, configurable by
[`--profiles_to_retain`](/versions/9.0.0/reference/command-line-reference#flag--generate_json_trace_profile),
...
![Example profile](/versions/9.0.0/docs/images/json-trace-profile.png "Example profile")
...
*   `action count`: Displays how many concurrent actions were in flight. Click
    ...
    [`--jobs`](/versions/9.0.0/reference/command-line-reference#flag--jobs) in clean
    builds.
...
each other; pass
[`--noslim_profile`](/versions/9.0.0/reference/command-line-reference#flag--slim_profile)
