...
Tests are run using [`bazel test`](/versions/7.7.1/docs/user-manual#test).
...
[`timeout`](/versions/7.7.1/reference/be/common-definitions#test.timeout) attribute according
...
test's [`size`](/versions/7.7.1/reference/be/common-definitions#test.size) as follows:
...
[Common definitions](/versions/7.7.1/reference/be/common-definitions#common-attributes-tests).
...
specification [here](/versions/7.7.1/reference/be/common-definitions#common-attributes-tests). If
...
[`--test_sharding_strategy`](/versions/7.7.1/reference/command-line-reference#flag--test_sharding_strategy)
and [`shard_count`](/versions/7.7.1/reference/be/common-definitions#common-attributes-tests) to
...
[`--incompatible_check_sharding_support`](/versions/7.7.1/reference/command-line-reference#flag--incompatible_check_sharding_support)
...
<table>
  ...
  <tr>
    ...
    <td>total
      <a href="/versions/7.7.1/reference/be/common-definitions#test.shard_count"><code>shard count</code></a>,
  ...
  <tr>
    ...
    <td>value of
      <a href="/versions/7.7.1/docs/user-manual#flag--test_filter"><code>--test_filter</code></a>,
      if specified</td>
    <td>optional</td>
  </tr>
  <tr>
...
[Bazel Build Encyclopedia on the `tags` attribute](/versions/7.7.1/reference/be/common-definitions#common.tags).
...
In the following, assume there is a &ast;_binary() rule labeled
...
`&ast;_binary()` rule. The runfiles directory itself depends on the set of BUILD
files that affect the `&ast;_binary()` rule or any of its compile-time or run-time
...
* **Symlinks to run-time dependencies**: each OutputFile and CommandRule that
    is a run-time dependency of the `&ast;_binary()` rule is represented by one
...
* **Symlinks to sub-runfiles**: for every `&ast;_binary()` Z that is a run-time
    dependency of `&ast;_binary()` C, there is a second link in the runfiles
