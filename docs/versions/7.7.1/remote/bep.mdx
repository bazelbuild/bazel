...
1. The root event is always a [`BuildStarted`](/versions/7.7.1/remote/bep-glossary#buildstarted)
...
1. Events containing data produced by the command, such as files built and test
   results, appear before the [`BuildFinished`](/versions/7.7.1/remote/bep-glossary#buildfinished)
   event.
1. The [`BuildFinished`](/versions/7.7.1/remote/bep-glossary#buildfinished) event *may* be followed
...
[Command-Line Reference](/versions/7.7.1/reference/command-line-reference).
...
work around this issue is to use Bazel with [remote caching](/versions/7.7.1/remote/caching).
