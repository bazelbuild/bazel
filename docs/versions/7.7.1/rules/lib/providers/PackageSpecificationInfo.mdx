...
| --- | --- |
| `target` | [Label](/versions/7.7.1/rules/lib/builtins/Label); or [string](/versions/7.7.1/rules/lib/core/string); required  A target which is checked if it exists inside the package group. |

## to\_json

```
string PackageSpecificationInfo.to_json()
```
...
```
string PackageSpecificationInfo.to_proto()
```
