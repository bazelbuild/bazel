...
[Optimizing Performance](/versions/7.7.1/rules/performance).

## Recommended flags

The flags are in the
[`bazelrc` configuration syntax](/versions/7.7.1/run/bazelrc#bazelrc-syntax-semantics), so
...
See the [JSON Profile documentation](/versions/7.7.1/rules/performance#performance-profiling) for
