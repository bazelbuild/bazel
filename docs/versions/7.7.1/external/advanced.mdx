...
use the
[`--override_repository`](/versions/7.7.1/reference/command-line-reference#flag--override_repository)
...
Note: With [Bzlmod](/versions/7.7.1/external/overview#bzlmod), remember to use canonical repo
names here. Alternatively, use the
[`--override_module`](/versions/7.7.1/reference/command-line-reference#flag--override_module)
flag to override a module to a local directory, similar to the
[`local_path_override`](/versions/7.7.1/rules/lib/globals/module#local_path_override) directive in
`MODULE.bazel`.
...
fetch a file with [`ctx.download`](/versions/7.7.1/rules/lib/builtins/repository_ctx#download) or
[`ctx.download_and_extract`](/versions/7.7.1/rules/lib/builtins/repository_ctx#download_and_extract). By
