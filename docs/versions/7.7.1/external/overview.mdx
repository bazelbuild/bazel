...
function](/versions/7.7.1/reference/be/functions#package) in `BUILD` files; whereas `package()`
...
[Bazel registry](/versions/7.7.1/external/registry) — by default, the [Bazel Central
...
guide](/versions/7.7.1/external/migration) on how to migrate to Bzlmod.