...
[creating a toolchain rule](/versions/6.3.0/docs/toolchains#creating-a-toolchain-rule).
...
identifying and resolving dependency-related build errors. See [Troubleshooting Bazel Remote Execution with Docker Sandbox](/versions/6.3.0/docs/remote-execution-sandbox)
...
*   **Symlinking to local tools or artifacts.** Symlinks to tools or libraries
    ...
    tree. Do not use [`repository_ctx.symlink`](/versions/6.3.0/rules/lib/repository_ctx#symlink)
...
To help find potential non-hermetic behavior you can use [Workspace rules log](/versions/6.3.0/docs/workspace-log).
