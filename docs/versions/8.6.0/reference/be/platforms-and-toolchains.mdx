...
2. If `remote_execution_property` is set on the child platform, and contains the
   literal string &lcub;PARENT_REMOTE_EXECUTION_PROPERTIES&rcub;, that macro will be
...
| `remote_execution_properties` | String; [nonconfigurable](/versions/8.6.0/reference/be/common-definitions#configurable-attributes); default is `""` DEPRECATED. Please use exec_properties attribute instead. A string used to configure a remote execution platform. Actual builds make no attempt to interpret this, it is treated as opaque data that can be used by a specific SpawnRunner. This can include data from the parent platform's "remote_execution_properties" attribute, by using the macro "&lcub;PARENT_REMOTE_EXECUTION_PROPERTIES&rcub;". See the section on [Platform Inheritance](#platform_inheritance) for details. |
