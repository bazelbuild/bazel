...
| `attrs_for_reproducibility` | [dict](/versions/8.6.0/rules/lib/core/dict); default is `&lcub;&rcub;`  If `reproducible` is False, this can be specified to tell Bazel which attributes of the original repo rule to change to make it reproducible. |
