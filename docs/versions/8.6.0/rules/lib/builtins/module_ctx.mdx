...
| `auth` | [dict](/versions/8.6.0/rules/lib/core/dict); default is `&lcub;&rcub;`  An optional dict specifying authentication information for some of the URLs. |
| `headers` | [dict](/versions/8.6.0/rules/lib/core/dict); default is `&lcub;&rcub;`  An optional dict specifying http headers for all URLs. |
...
| `environment` | [dict](/versions/8.6.0/rules/lib/core/dict); default is `&lcub;&rcub;`  Force some environment variables to be set to be passed to the process. The value can be `None` to remove the environment variable. |
