...
to the list of Bazel's commands in [Build with Bazel](/versions/8.6.0/run/build).
...
[Specifying targets to build](/versions/8.6.0/run/build#specifying-build-targets).
...
[Bazel configuration file](/versions/8.6.0/run/bazelrc) for convenience.
...
[cc_library.linkopts](/versions/8.6.0/reference/be/c-cpp#cc_library.linkopts).
...
a [`*.stripped` binary](/versions/8.6.0/reference/be/c-cpp#cc_binary_implicit_outputs). The default
...
[aspects](/versions/8.6.0/extending/aspects)
instead.

The `experimental_action_listener` option instructs Bazel to use
details from the [`action_listener`](/versions/8.6.0/reference/be/extra-actions#action_listener) rule specified by {{ "<var>" }}label{{ "</var>" }} to
insert [`extra_actions`](/versions/8.6.0/reference/be/extra-actions#extra_action) into the build graph.

#### `--[no]experimental_extra_action_top_level_only`

Warning: Extra actions are deprecated. Use
[aspects](/versions/8.6.0/extending/aspects) instead.
...
the [linkstatic attribute](/versions/8.6.0/reference/be/c-cpp#cc_binary.linkstatic) on build rules.

Modes:

* `default`: Allows bazel to choose whether to link dynamically.
  See [linkstatic](/versions/8.6.0/reference/be/c-cpp#cc_binary.linkstatic) for more
  information.
...
* `off`: Links all targets in
  [mostly static](/versions/8.6.0/reference/be/c-cpp#cc_binary.linkstatic) mode.
...
[shrink_resources attribute](/versions/8.6.0/reference/be/android#android_binary.shrink_resources) on
...
(around 10%) for the [execution phase](/versions/8.6.0/run/build#execution).
The drawback is that further incremental builds will be slower. See also
[memory-saving mode](/versions/8.6.0/configure/memory).
...
the [`.bazelrc`](/versions/8.6.0/run/bazelrc) file.
...
[--execution_log_json_file](/versions/8.6.0/reference/command-line-reference#flag--execution_log_json_file)
and
[--execution_log_binary_file](/versions/8.6.0/reference/command-line-reference#flag--execution_log_binary_file).
...
Please see background information on [Platforms](/versions/8.6.0/extending/platforms) and [Toolchains](/versions/8.6.0/extending/toolchains).
...
[register_execution_platforms()](/versions/8.6.0/rules/lib/globals/module#register_execution_platforms).
...
[register_toolchains()](/versions/8.6.0/rules/lib/globals/module#register_toolchains).
...
details, see the
[Starlark Configurations](/versions/8.6.0/extending/config#using-build-setting-aliases).
...
[Call Bazel from scripts](/versions/8.6.0/run/scripts). In particular, the following options
are strongly recommended:

*   [`--bazelrc=/dev/null`](/versions/8.6.0/run/bazelrc)
*   [`--nokeep_state_after_build`](/versions/8.6.0/reference/command-line-reference#flag--keep_state_after_build)
...
defined by the page [Writing Tests](/versions/8.6.0/reference/test-encyclopedia).
...
[`bazel build`](/versions/8.6.0/run/build).
...
[Writing Tests](/versions/8.6.0/reference/test-encyclopedia). Note that none of the
...
two commands is that query runs after the [loading phase](/versions/8.6.0/run/build#loading)
and cquery runs after the [analysis phase](/versions/8.6.0/run/build#analysis). These tools are an
...
[Bazel Query Reference](/versions/8.6.0/query/language).
...
For more details, see [Action Graph Query](/versions/8.6.0/query/aquery).
...
[Building with Bazel](/versions/8.6.0/run/build#quickstart).
...
[Client/server implementation](/versions/8.6.0/run/client-server).
...
*   The complete
    ["Make" environment](/versions/8.6.0/reference/be/make-variables). If the `--show_make_env` flag is
...
See [bazel mobile-install](/versions/8.6.0/docs/mobile-install) for more information.
...
[JSON trace profile](/versions/8.6.0/advanced/performance/json-trace-profile) previously
gathered during a Bazel invocation.

### `canonicalize-flags`

The [`canonicalize-flags`](/versions/8.6.0/reference/command-line-reference#canonicalize-flags-options)
...
[standard client/server mode](/versions/8.6.0/run/client-server), but instead runs a bazel
...
[the rc files](/versions/8.6.0/run/bazelrc#bazelrc-file-locations); for the current `command`,
