...
Methods available in BUILD files. See also the Build Encyclopedia for extra [functions](/versions/8.7.0/reference/be/functions) and build rules, which can also be used in BUILD files.
...
| Parameter | Description |
| --- | --- |
| `kwargs` | required  See the [`package()`](/versions/8.7.0/reference/be/functions#package) function in the Build Encyclopedia for applicable arguments. |
