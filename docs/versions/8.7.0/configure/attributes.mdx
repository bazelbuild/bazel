...
*configuration conditions*, which are labels referencing [`config_setting`](/versions/8.7.0/reference/be/general#config_setting)
...
[`values`](/versions/8.7.0/reference/be/general#config_setting.values) attribute is non-configurable.
...
[configuration transition](/versions/8.7.0/reference/glossary#transition).
...
[`config_setting`](/versions/8.7.0/reference/be/general#config_setting) or
[`constraint_value`](/versions/8.7.0/reference/be/platforms-and-toolchains#constraint_value).
...
[`config_setting`](/versions/8.7.0/reference/be/general#config_setting)'s
[`values`](/versions/8.7.0/reference/be/general#config_setting.values) attribute:
...
[`--show_progress`](/versions/8.7.0/docs/user-manual#show-progress) isn't allowed because
...
These are triggered with [`config_setting`](/versions/8.7.0/reference/be/general#config_setting)'s
[`flag_values`](/versions/8.7.0/reference/be/general#config_setting.flag_values)
attribute:
...
[`--define`](/versions/8.7.0/reference/command-line-reference#flag--define)
...
[define_values](/versions/8.7.0/reference/be/general#config_setting.define_values) attribute
...
you want to build a target.
   [Platforms](/versions/8.7.0/extending/platforms)
...
[documentation](/versions/8.7.0/concepts/platforms) for details.
...
This can be customized with the [`no_match_error`](/versions/8.7.0/reference/be/functions#select)
attribute:
...
because macros are evaluated in Bazel's [loading phase](/versions/8.7.0/run/build#loading),
...
Bazel [`query`](/versions/8.7.0/query/guide) operates over Bazel's
[loading phase](/versions/8.7.0/reference/glossary#loading-phase).
...
[analysis phase](/versions/8.7.0/reference/glossary#analysis-phase)).
So it can't determine which `select()` branches are chosen.

Bazel [`cquery`](/versions/8.7.0/query/cquery) operates after Bazel's analysis phase, so it has
...
documentation on [rules](/versions/8.7.0/extending/rules) and [macros](/versions/8.7.0/extending/macros)
...
The technical answer is that [`bind()`](/versions/8.7.0/reference/be/workspace#bind) is a repo
...
Instead, you should use [`alias()`](/versions/8.7.0/reference/be/general#alias), with a `select()` in
...
use [cquery](/versions/8.7.0/query/cquery) and `bazel config` to debug:
...
[cquery docs](/versions/8.7.0/query/cquery) for guidance on using `somepath` to get the right
one.
