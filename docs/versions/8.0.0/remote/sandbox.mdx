...
causes of such failures are described in [Adapting Bazel Rules for Remote Execution](/versions/8.0.0/remote/rules).
...
into into your build, especially if it uses [configure-style WORKSPACE rules](/versions/8.0.0/remote/rules#manage-workspace-rules).
...
*  **A file, tool, binary, or resource referenced by the Bazel runfiles tree is
   ...
   [explicitly declared](/versions/8.0.0/concepts/dependencies). See
   [Managing implicit dependencies](/versions/8.0.0/remote/rules#manage-dependencies)
...
*  **A file, tool, binary, or resource referenced by an absolute path or the `PATH`
   ...
   the toolchain container and use [toolchain rules](/versions/8.0.0/extending/toolchains) to properly
   declare dependencies pointing to the missing resource. See
   [Invoking build tools through toolchain rules](/versions/8.0.0/remote/rules#invoking-build-tools-through-toolchain-rules)
   for more information.
...
*  **A binary execution fails.** One of the build rules is referencing a binary
   ...
   [Managing platform-dependent binaries](/versions/8.0.0/remote/rules#manage-binaries)
...
*  **A file from `@local-jdk` is missing or causing errors.** The Java binaries
   ...
   it. Use [`java_toolchain`](/versions/8.0.0/reference/be/java#java_toolchain)
...
*   If the build fails during the analysis or loading phases, one or more of
    ...
    remote execution. See [Adapting Bazel Rules for Remote Execution](/versions/8.0.0/remote/rules)
