...
[strategy flags](/versions/8.2.1/docs/user-manual#strategy-options). Using the `sandboxed`
...
[Persistent workers](/versions/8.2.1/remote/persistent) run in a generic sandbox if you pass
...
sandboxes [persistent workers](/versions/8.2.1/remote/persistent).
...
-   Sandboxing effectively disables any cache the tool may have. You can
    mitigate this by using [persistent workers](/versions/8.2.1/remote/persistent), at
    the cost of weaker sandbox guarantees.

-   [Multiplex workers](/versions/8.2.1/remote/multiplex) require explicit worker support
