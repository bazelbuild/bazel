...
functions and types, see the [Bazel API reference](/versions/8.2.1/rules/lib/overview).
...
* [None](/versions/8.2.1/rules/lib/globals#None)
...
[lists](/versions/8.2.1/rules/lib/core/list) and [dicts](/versions/8.2.1/rules/lib/core/dict). Changes to mutable
...
[Native functions](/versions/8.2.1/reference/be/functions) and [native rules](/versions/8.2.1/reference/be/overview#language-specific-native-rules) are global symbols in
`BUILD` files. `bzl` files need to load them using the [`native` module](/versions/8.2.1/rules/lib/toplevel/native).
...
* Strings are represented with double-quotes (such as when you call
  [repr](/versions/8.2.1/rules/lib/globals#repr)).
...
* `class` (see [`struct`](/versions/8.2.1/rules/lib/builtins/struct#struct) function).
* `import` (see [`load`](/versions/8.2.1/extending/concepts#loading-an-extension) statement).
* `while`, `yield`.
* float and set types.
* generators and generator expressions.
* `is` (use `==` instead).
* `try`, `raise`, `except`, `finally` (see [`fail`](/versions/8.2.1/rules/lib/globals#fail) for fatal errors).
* `global`, `nonlocal`.
