...
*   It requires a fair amount of boilerplate to set up the rules under test and
    ...
    first. It helps to [keep in mind](/versions/8.2.1/extending/concepts#evaluation-model) that macros
...
[`Action`](/versions/8.2.1/rules/lib/builtins/Action) objects which represent actions registered by the
target under test.
