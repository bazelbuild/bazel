...
releases, check the [Bzlmod migration guide](/versions/8.2.1/external/migration) on how to
migrate.
...
See [Repository rules](/versions/8.2.1/extending/repo) for more information about how to write
your own repository rules.

The most common repo rules by far are
[`http_archive`](/versions/8.2.1/rules/lib/repo/http#http_archive), which downloads an archive
from a URL and extracts it, and
[`local_repository`](/versions/8.2.1/reference/be/workspace#local_repository), which symlinks a
...
See [fetch options](/versions/8.2.1/reference/command-line-reference#fetch-options) for more
information about controlling fetch.

### Directory layout {#directory-layout}

After being fetched, the repo can be found in the subdirectory `external` in the
[output base](/versions/8.2.1/remote/output-directories), under its canonical name.
...
available. `repo()` takes the same arguments as the [`package()` function](/versions/8.2.1/reference/be/functions#package) in `BUILD` files; whereas `package()`
...
A [Bazel **module**](/versions/8.2.1/external/module) is a Bazel project that can have multiple
...
[Bazel registry](/versions/8.2.1/external/registry) — by default, the [Bazel Central Registry](https://bcr.bazel.build/). The registry provides the
...
consumed by [**module extensions**](/versions/8.2.1/external/extension) after module resolution
...
See the [full list](/versions/8.2.1/rules/lib/globals/workspace) of functions available in
`WORKSPACE` files.
...
WORKSPACE system in future Bazel releases. Please read the [Bzlmod migration guide](/versions/8.2.1/external/migration) on how to migrate to Bzlmod.
