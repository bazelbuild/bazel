...
[target](https://bazel.build/versions/8.2.1/reference/glossary#target)
`//mypkg:target`.
...
| `exec_compatible_with` | List of [labels](/versions/8.2.1/concepts/labels); [nonconfigurable](#configurable-attributes); default is `[]`  A list of `constraint_values` that must be present in the execution platform for this target. This is in addition to any constraints already set by the rule type. Constraints are used to restrict the list of available execution platforms. For more details, see the description of [toolchain resolution](/versions/8.2.1/docs/toolchains#toolchain-resolution). |
...
The [`select()`](/versions/8.2.1/reference/be/functions#select) function
chooses among different alternative values for a configurable attribute based
on which [`config_setting`](/versions/8.2.1/reference/be/general#config_setting)
or [`constraint_value`](/versions/8.2.1/reference/be/platforms-and-toolchains#constraint_value)
...
[loading and analysis phases](https://bazel.build/versions/8.2.1/rules/concepts#evaluation-model)).
...
[this FAQ](https://bazel.build/versions/8.2.1/docs/configurable-attributes#faq)
...
See [Configurable Build Attributes](https://bazel.build/versions/8.2.1/docs/configurable-attributes) for a detailed overview.
...
[labels](/versions/8.2.1/concepts/labels) identify *targets*,
...
the [BUILD Concept Reference](/versions/8.2.1/concepts/build-ref).