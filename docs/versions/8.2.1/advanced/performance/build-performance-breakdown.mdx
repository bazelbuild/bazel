...
issues through [extracting metrics](/versions/8.2.1/configure/build-performance-metrics)
...
1. `PackageMetrics.packages_loaded`: the number of packages successfully loaded.
  ...
  each additional BUILD file in the loading phase.
   - This is often due to the addition of dependencies and having to load their
     transitive closure.
   - Use [query](/versions/8.2.1/query/quickstart) / [cquery](/versions/8.2.1/query/cquery) to find
...
2. `TargetMetrics.targets_configured`: representing the number of targets and
  ...
  constructing and traversing the configured target graph.
   ...
   - Use [cquery](/versions/8.2.1/query/cquery) to find where new
...
3. `ActionSummary.actions_created`: represents the actions created in the build,
  ...
  that this also includes unused actions that might not have been executed.
   - Use [aquery](/versions/8.2.1/query/aquery) for debugging regressions;
     we suggest starting with
     [`--output=summary`](/versions/8.2.1/reference/command-line-reference#flag--output)
     before further drilling down with
     [`--skyframe_state`](/versions/8.2.1/reference/command-line-reference#flag--skyframe_state).
...
4. `ActionSummary.actions_executed`: the number of actions executed, a
  regression directly represents more work in executing these actions.
   - The [BEP](/versions/8.2.1/remote/bep) writes out the action statistics
     ...
     [`--experimental_record_metrics_for_all_mnemonics`](/versions/8.2.1/reference/command-line-reference#flag--experimental_record_metrics_for_all_mnemonics)
...
- **Wall time** is the real world time elapsed.
   - If _only_ wall time regresses, we suggest collecting a
     [JSON trace profile](/versions/8.2.1/advanced/performance/json-trace-profile) and looking
...
[JSON trace profiler](/versions/8.2.1/advanced/performance/json-trace-profile) collects the
system load average during the invocation.

![Profile that includes system load average](/versions/8.2.1/docs/images/json-trace-profile-system-load-average.png "Profile that includes system load average")
...
adjusting
[`--local_cpu_resources`](/versions/8.2.1/reference/command-line-reference#flag--local_cpu_resources)
and [`--local_ram_resources`](/versions/8.2.1/reference/command-line-reference#flag--local_ram_resources),
...
- `bazel info used-heap-size-after-gc`: The amount of used memory in bytes after
  a call to `System.gc()`.
   ...
   - Additionally, there are `peak-heap-size`, `max-heap-size`, `used-heap-size`
     ...
     [documentation](/versions/8.2.1/docs/user-manual#configuration-independent-data)), but are
     less relevant.

- [BEP](/versions/8.2.1/remote/bep)’s
  `MemoryMetrics.peak_post_gc_heap_size`: Size of the peak JVM heap size in
  bytes post GC (requires setting
  [`--memory_profile`](/versions/8.2.1/reference/command-line-reference#flag--memory_profile)
...
the [built-in memory profiler](/versions/8.2.1/rules/performance#memory-profiling)
for rules.

#### Memory profiling of persistent workers

While [persistent workers](/versions/8.2.1/remote/persistent) can help to speed up builds
...
The [JSON trace profiler](/versions/8.2.1/advanced/performance/json-trace-profile) also
...
![Profile that includes workers memory usage](/versions/8.2.1/docs/images/json-trace-profile-workers-memory-usage.png "Profile that includes workers memory usage")

**Figure 2.** Profile that includes workers memory usage.

Lowering the value of
[`--worker_max_instances`](/versions/8.2.1/reference/command-line-reference#flag--worker_max_instances)
...
`NetworkMetrics.SystemNetworkStats` proto from the [BEP](/versions/8.2.1/remote/bep)
(requires passing `--experimental_collect_system_network_usage`).

Furthermore, [JSON trace profiles](/versions/8.2.1/advanced/performance/json-trace-profile)
...
![Profile that includes system-wide network usage](/versions/8.2.1/docs/images/json-trace-profile-network-usage.png "Profile that includes system-wide network usage")
...
[`--remote_download_minimal`](/versions/8.2.1/reference/command-line-reference#flag--remote_download_minimal).
...
[disk cache](/versions/8.2.1/reference/command-line-reference#flag--disk_cache) to save on
download bandwidth.
