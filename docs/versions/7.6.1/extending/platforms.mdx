...
appropriate
[toolchains](/versions/7.6.1/extending/toolchains)
...
rule to write [configurable attributes](/versions/7.6.1/docs/configurable-attributes).
...
[Migrating to Platforms](/versions/7.6.1/concepts/platforms).
...
The [`platform`](/versions/7.6.1/reference/be/platforms-and-toolchains#platform) rule introduces a new platform with
...
[`target_compatible_with`](/versions/7.6.1/reference/be/common-definitions#common.target_compatible_with)
...
Incompatible tests in a [`test_suite`](/versions/7.6.1/reference/be/general#test_suite) are
similarly skipped if the `test_suite` is specified on the command line with
[`--expand_test_suites`](/versions/7.6.1/reference/command-line-reference#flag--expand_test_suites).
...
Use [`select()`](/versions/7.6.1/reference/be/functions#select) in combination with
...
You can use the
[`IncompatiblePlatformProvider`](/versions/7.6.1/rules/lib/providers/IncompatiblePlatformProvider)
in `bazel cquery`'s [Starlark output format](/versions/7.6.1/query/cquery#output-format-definition) to distinguish
