...
[loading phase](/versions/7.6.1/extending/concepts#evaluation-model), macros don't exist anymore,
...
Note: `$@` is a [Make variable](/versions/7.6.1/reference/be/make-variables#predefined_genrule_variables)
...
instantiated from the [native](/versions/7.6.1/rules/lib/toplevel/native) module:
...
macro), use the function [native.package_name()](/versions/7.6.1/rules/lib/toplevel/native#package_name).
Note that `native` can only be used in `.bzl` files, and not in `BUILD` files.

## Label resolution in macros

Since macros are evaluated in the [loading phase](/versions/7.6.1/extending/concepts#evaluation-model),
...
[`Label`](/versions/7.6.1/rules/lib/builtins/Label#Label) constructor:
...
*   You can also use [print](/versions/7.6.1/rules/lib/globals/all#print) for debugging. It displays
...
If you want to throw an error, use the [fail](/versions/7.6.1/rules/lib/globals/all#fail) function.
