...
[analysis](/versions/7.6.1/reference/glossary#analysis-phase).
...
[Concepts and terminology](/versions/7.6.1/concepts/build-ref).
...
[`visibility`](/versions/7.6.1/reference/be/common-definitions#common.visibility) attribute on
...
*   `"//some_pkg:my_package_group"`: Grants access to all of the packages that
    are part of the given [`package_group`](/versions/7.6.1/reference/be/functions#package_group).

    *   Package groups use a
        [different syntax](/versions/7.6.1/reference/be/functions#package_group.packages) for
...
2. The value of the
[`default_visibility`](/versions/7.6.1/reference/be/functions#package.default_visibility)
argument of the [`package`](/versions/7.6.1/reference/be/functions#package) statement in the
...
[`exports_files`](/versions/7.6.1/reference/be/functions#exports_files). When no `visibility`
...
[`config_setting`](/versions/7.6.1/reference/be/general#config_setting) targets that are
referenced in the keys of a [`select()`](/versions/7.6.1/reference/be/functions#select). There
...
Some rules have [implicit dependencies](/versions/7.6.1/extending/rules#private_attributes_and_implicit_dependencies) —
...
`.bzl` file but not list it in the `srcs` of a [`filegroup`](/versions/7.6.1/reference/be/general#filegroup),
...
[`visibility()`](/versions/7.6.1/rules/lib/globals/bzl#visibility) function from within the file.
The argument to `visibility()` is a list of package specifications, just like
the [`packages`](/versions/7.6.1/reference/be/functions#package_group.packages) attribute of
...
[`includes`](/versions/7.6.1/reference/be/functions#package_group.includes) attribute.
