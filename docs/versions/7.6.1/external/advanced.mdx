...
system](/versions/7.6.1/external/overview#workspace-system) only. For
[Bzlmod](/versions/7.6.1/external/overview#bzlmod), use a [multiple-version
override](/versions/7.6.1/external/module#multiple-version_override).
...
*   Use `--host_jvm_args=-Djava.net.preferIPv6Addresses=true` [startup
    option](/versions/7.6.1/docs/user-manual#startup-options), for example by adding the
...
*   When running Java build targets that need to connect to the internet (such
    ...
    flag](/versions/7.6.1/docs/user-manual#jvmopt). For example, include in your [`.bazelrc`
    file](/versions/7.6.1/run/bazelrc):
