...
*   `function(...)`: the standard [functions](/versions/7.4.0/query/language#functions)
...
[options](/versions/7.4.0/reference/command-line-reference#build-options)
...
[Skyframe](/versions/7.4.0/reference/skyframe) is the evaluation and
incrementality model of Bazel. On each instance of Bazel server, Skyframe stores the dependency graph
constructed from the previous runs of the [Analysis phase](/versions/7.4.0/run/build#analysis).
...
It is possible for [Aspects](/versions/7.4.0/extending/aspects)
...
[dependency graph](/versions/7.4.0/extending/aspects#aspect_basics).
...
their inputs/outputs), the [JSON profile](/versions/7.4.0/rules/performance#performance-profiling)
