...
to the list of Bazel's commands in [Build with Bazel](/versions/6.2.0/docs/build).
...
[Specifying targets to build](/versions/6.2.0/docs/build#specifying-build-targets).
...
[Bazel configuration file](/versions/6.2.0/docs/bazelrc) for convenience.
...
[cc_library.linkopts](/versions/6.2.0/reference/be/c-cpp#cc_library.linkopts).
...
a [`*.stripped` binary](/versions/6.2.0/reference/be/c-cpp#cc_binary_implicit_outputs). The default
...
[aspects](/versions/6.2.0/rules/aspects)
instead.

The `experimental_action_listener` option instructs Bazel to use
details from the [`action_listener`](/versions/6.2.0/reference/be/extra-actions#action_listener) rule specified by {{ "<var>" }}label{{ "</var>" }} to
insert [`extra_actions`](/versions/6.2.0/reference/be/extra-actions#extra_action) into the build graph.

#### `--[no]experimental_extra_action_top_level_only` {:experimental-extra-action-top-level-only}

Warning: Extra actions are deprecated. Use
[aspects](/versions/6.2.0/rules/aspects) instead.
...
the [linkstatic attribute](/versions/6.2.0/reference/be/c-cpp#cc_binary.linkstatic) on build rules.
...
* `default`: Allows bazel to choose whether to link dynamically.
  See [linkstatic](/versions/6.2.0/reference/be/c-cpp#cc_binary.linkstatic) for more
  information.
...
* `off`: Links all targets in
  [mostly static](/versions/6.2.0/reference/be/c-cpp#cc_binary.linkstatic) mode.
...
[shrink_resources attribute](/versions/6.2.0/reference/be/android#android_binary.shrink_resources) on
...
["Make" variables](/versions/6.2.0/reference/be/make-variables) are derived from this option.
...
(around 10%) for the [execution phase](/versions/6.2.0/docs/build#execution).
The drawback is that further incremental builds will be slower. See also
[memory-saving mode](/versions/6.2.0/docs/memory-saving-mode).
...
(see [build encyclopedia](/versions/6.2.0/reference/be/overview) for a full list of these). Each
...
the [`.bazelrc`](/versions/6.2.0/docs/bazelrc) file.
...
[--execution_log_json_file](/versions/6.2.0/reference/command-line-reference#flag--execution_log_json_file)
and
[--execution_log_binary_file](/versions/6.2.0/reference/command-line-reference#flag--execution_log_binary_file).
...
Please see background information on [Platforms](/versions/6.2.0/docs/platforms) and [Toolchains](/versions/6.2.0/docs/toolchains).
...
[register_execution_platforms()](/versions/6.2.0/rules/lib/globals#register_execution_platforms).
...
[register_toolchains()](/versions/6.2.0/rules/lib/globals#register_toolchains).
...
details, see the
[Starlark Configurations](/versions/6.2.0/rules/config#using-build-setting-aliases).
...
[Call Bazel from scripts](/versions/6.2.0/docs/scripts). In particular, the following options
are strongly recommended:

*   [`--bazelrc=/dev/null`](/versions/6.2.0/docs/bazelrc)
*   [`--nokeep_state_after_build`](/versions/6.2.0/reference/command-line-reference#flag--keep_state_after_build)
...
defined by the page [Writing Tests](/versions/6.2.0/reference/test-encyclopedia).
...
[`bazel build`](/versions/6.2.0/docs/build).
...
[Writing Tests](/versions/6.2.0/reference/test-encyclopedia). Note that none of the
...
two commands is that query runs after the [loading phase](/versions/6.2.0/docs/build#loading)
and cquery runs after the [analysis phase](/versions/6.2.0/docs/build#analysis). These tools are an
...
[Bazel Query Reference](/versions/6.2.0/reference/query).
...
For more details, see [Action Graph Query](/versions/6.2.0/docs/aquery).
...
[Building with Bazel](/versions/6.2.0/docs/build#quickstart).
...
[Client/server implementation](/versions/6.2.0/docs/client-server).
...
*   The complete
    ["Make" environment](/versions/6.2.0/reference/be/make-variables). If the `--show_make_env` flag is
...
See [bazel mobile-install](/versions/6.2.0/docs/mobile-install) for more information.
...
[Troubleshooting performance by profiling](/versions/6.2.0/rules/performance#performance-profiling).

### `canonicalize-flags` {:#canonicalize-flags}

The [`canonicalize-flags`](/versions/6.2.0/reference/command-line-reference#canonicalize-flags-options)
...
[standard client/server mode](/versions/6.2.0/docs/client-server), but instead runs a bazel
...
[the rc files](/versions/6.2.0/docs/bazelrc#bazelrc-file-locations); for the current `command`,
