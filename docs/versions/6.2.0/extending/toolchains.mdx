...
recommended to read the [rules](/versions/6.2.0/rules/rules) and [platforms](/versions/6.2.0/docs/platforms)
...
[based on the platform](/versions/6.2.0/docs/configurable-attributes):
...
[`ToolchainInfo` provider](/versions/6.2.0/rules/lib/platform_common#ToolchainInfo)
...
The [`config_common.toolchain_type`](/versions/6.2.0/rules/lib/config_common#toolchain_type)
...
3. For each such target, an associated target of the generic
  [`toolchain`](/versions/6.2.0/reference/be/platform#toolchain)
...
analyzed, what [configuration](/versions/6.2.0/reference/glossary#configuration) does it see, and what transitions
...
The use of [`attr.label`](/versions/6.2.0/rules/lib/attr#label) is the same as for a standard rule,
...
Note: [Some Bazel rules](/versions/6.2.0/concepts/platforms-intro#status) do not yet support
...
command line via
[`--extra_execution_platforms`](/versions/6.2.0/reference/command-line-reference#flag--extra_execution_platforms)
and
[`--extra_toolchains`](/versions/6.2.0/reference/command-line-reference#flag--extra_toolchains).
...
1. If the target being built specifies the
   [`exec_compatible_with` attribute](/versions/6.2.0/reference/be/common-definitions#common.exec_compatible_with)
   (or its rule definition specifies the
   [`exec_compatible_with` argument](/versions/6.2.0/rules/lib/globals#rule.exec_compatible_with)),
...
If the rule uses [execution groups](/versions/6.2.0/reference/exec-groups), each execution
...
If you'd like to see which [`cquery`](/versions/6.2.0/docs/cquery) dependencies are from toolchain
resolution, use `cquery`'s [`--transitions`](/versions/6.2.0/docs/cquery#transitions) flag:
