...
An [external repository](/versions/6.2.0/docs/external) is a rule that can be used only
...
[repository_rule](/versions/6.2.0/rules/lib/globals#repository_rule) function to create a new
...
[`local_repository`](https://bazel.build/versions/6.2.0/reference/be/workspace#local_repository.repo_mapping)
and
[`new_local_repository`](https://bazel.build/versions/6.2.0/reference/be/workspace#new_local_repository.repo_mapping).
...
from the Internet). See [the library](/versions/6.2.0/rules/lib/repository_ctx) for more
context. Example:
...
attribute of the
[`repository_rule`](/versions/6.2.0/rules/lib/globals#repository_rule)
function. The value of those environment variable can be enforced from
the command line with the
[`--action_env`](/versions/6.2.0/reference/command-line-reference#flag--action_env)
