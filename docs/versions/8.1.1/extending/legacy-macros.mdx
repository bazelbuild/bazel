...
[loading phase](/versions/8.1.1/extending/concepts#evaluation-model), legacy macros don't exist
...
Where possible you should use [symbolic macros](/versions/8.1.1/extending/macros#macros).
...
*   Take typed attributes, which in turn means automatic label and select
    conversion.
*   Are more readable
*   Will soon have [lazy evaluation](/versions/8.1.1/extending/macros#laziness)
...
Note: `$@` is a
[Make variable](/versions/8.1.1/reference/be/make-variables#predefined_genrule_variables) that
...
from the [native](/versions/8.1.1/rules/lib/toplevel/native) module:
...
[native.package_name()](/versions/8.1.1/rules/lib/toplevel/native#package_name). Note that
...
[loading phase](/versions/8.1.1/extending/concepts#evaluation-model), label strings such as
...
[`Label`](/versions/8.1.1/rules/lib/builtins/Label#Label) constructor:
...
*   You can also use [print](/versions/8.1.1/rules/lib/globals/all#print) for debugging. It
...
If you want to throw an error, use the [fail](/versions/8.1.1/rules/lib/globals/all#fail)
