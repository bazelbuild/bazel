...
| --- | --- |
| `dirs` | [sequence](/versions/8.1.1/rules/lib/core/list) of [string](/versions/8.1.1/rules/lib/core/string)s; required |
...
| Parameter | Description |
| --- | --- |
| `kwargs` | required  The `repo()` function accepts exactly the same arguments as the [`package()`](/versions/8.1.1/reference/be/functions#package) function in BUILD files. |