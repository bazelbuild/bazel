...
* Run steps to
  [debug local cache hits](/versions/8.1.1/remote/cache-remote#troubleshooting-cache-hits)
...
* Discover and fix hermeticity problems using
  [remote execution rules](/versions/8.1.1/remote/rules#overview).
* Enable strict [sandboxing](/versions/8.1.1/docs/sandboxing)
  at the per-action level, since actions in a build can be stateful and affect
  the build or the output.
* [Workspace rules](/versions/8.1.1/remote/workspace)
