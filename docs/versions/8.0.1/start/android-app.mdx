...
[Android rules](/versions/8.0.1/reference/be/android).
...
* **Bazel.** To install, follow the [installation instructions](/versions/8.0.1/install).
...
![Button greeting](/versions/8.0.1/docs/images/android_tutorial_app.png "Tutorial app button greeting")
...
A [workspace](/versions/8.0.1/concepts/build-ref#workspace) is a directory that contains the
source files for one or more software projects, and has a `MODULE.bazel` file at
its root.

The `MODULE.bazel` file may be empty or may contain references to [external dependencies](/versions/8.0.1/external/overview) required to build your project.
...
For more information, read [Using the Android Native Development Kit with Bazel](/versions/8.0.1/docs/android-ndk).
...
A [`BUILD` file](/versions/8.0.1/concepts/build-files) describes the relationship
...
structure in your workspace. Each [package](/versions/8.0.1/concepts/build-ref#packages) is a
...
[build rule](/versions/8.0.1/concepts/build-files#types-of-build-rules), which tells
...
[`android_library`](/versions/8.0.1/reference/be/android#android_library) and
[`android_binary`](/versions/8.0.1/reference/be/android#android_binary), that you can use to
...
The [`android_binary`](/versions/8.0.1/reference/be/android#android_binary) rule builds
...
The [`build`](/versions/8.0.1/docs/user-manual#build) subcommand instructs Bazel to build the
...
For more details about target labels and paths, see [Labels](/versions/8.0.1/concepts/labels).
...
command line using the [`bazel mobile-install`](/versions/8.0.1/docs/user-manual#mobile-install) command. This command uses
...
![Bazel tutorial app](/versions/8.0.1/docs/images/android_tutorial_before.png "Bazel tutorial app")
...
[`--incremental`](/versions/8.0.1/docs/user-manual#mobile-install) flag that can be used to
...
* More information on [mobile-install](/versions/8.0.1/docs/mobile-install)
...
* Run Robolectric tests with the [robolectric-bazel](https://github.com/robolectric/robolectric-bazel)
  integration.
* Testing your app with [Android instrumentation tests](/versions/8.0.1/docs/android-instrumentation-test)
* Integrating C and C++ code into your Android app with the [NDK](/versions/8.0.1/docs/android-ndk)
