...
See [Java and Bazel](/versions/8.0.1/docs/bazel-and-java) for details.
...
1. The toolchains you want to use must exist. If using stock toolchains, the
   ...
   writing your own custom toolchains, you need to [register](https://bazel.build/versions/8.0.1/extending/toolchains#registering-building-toolchains) them in your
   `MODULE.bazel` file or with [`--extra_toolchains`](https://bazel.build/versions/8.0.1/reference/command-line-reference#flag--extra_toolchains).
...
1. Define standard toolchains and make them accessible to users through your
   rule's registration instructions ([details](https://bazel.build/versions/8.0.1/extending/toolchains#registering-building-toolchains))
