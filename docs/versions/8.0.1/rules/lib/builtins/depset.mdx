...
A specialized data structure that supports efficient merge operations and has a defined traversal order. Commonly used for accumulating data from transitive dependencies in rules and aspects. For more information see [here](/versions/8.0.1/extending/depsets).
...
Depsets are immutable. They should be created using their [constructor function](/versions/8.0.1/rules/lib/globals/bzl#depset) and merged or augmented with other depsets via the `transitive` argument.
