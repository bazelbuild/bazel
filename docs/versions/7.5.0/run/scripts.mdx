...
By default, Bazel uses a long-running [server process](/versions/7.5.0/run/client-server) as an
...
By default, Bazel reads the [`.bazelrc` file](/versions/7.5.0/run/bazelrc) from the base
...
See the [Performance Profiling](/versions/7.5.0/rules/performance#performance-profiling) section.
