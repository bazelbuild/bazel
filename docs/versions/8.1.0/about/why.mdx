...
same computer or on [remote build nodes](/versions/8.1.0/remote/rbe). If the structure of build
...
[same source code always results in the same binary](/versions/8.1.0/basics/hermeticity), bit
by bit.
...
Bazel rules are written in [Starlark](/versions/8.1.0/rules/language), our
...
Bazel's [integrated test runner](/versions/8.1.0/docs/user-manual#running-tests)
...
Bazel [provides facilities](/versions/8.1.0/remote/bep) to upload test results to a central
...
Bazel can [gather source code from multiple locations](/versions/8.1.0/external/overview): you
...
[cross-compilation capabilities](/versions/8.1.0/extending/platforms) to build code for one
platform while running the build on another.

## Wide ecosystem {:#ecosystem}

[Industry leaders](/versions/8.1.0/community/users) love Bazel, building a large
community of developers who use and contribute to Bazel. Find a tools, services
and documentation, including [consulting and SaaS offerings](/versions/8.1.0/community/experts)
Bazel can use. Explore extensions like support for programming languages in
our [open source software repositories](/versions/8.1.0/rules).