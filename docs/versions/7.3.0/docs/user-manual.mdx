...
to the list of Bazel's commands in [Build with Bazel](/versions/7.3.0/run/build).
...
[Specifying targets to build](/versions/7.3.0/run/build#specifying-build-targets).
...
[Bazel configuration file](/versions/7.3.0/run/bazelrc) for convenience.
...
[cc_library.linkopts](/versions/7.3.0/reference/be/c-cpp#cc_library.linkopts).
...
a [`*.stripped` binary](/versions/7.3.0/reference/be/c-cpp#cc_binary_implicit_outputs). The default
...
[aspects](/versions/7.3.0/extending/aspects)
instead.

The `experimental_action_listener` option instructs Bazel to use
details from the [`action_listener`](/versions/7.3.0/reference/be/extra-actions#action_listener) rule specified by {{ "<var>" }}label{{ "</var>" }} to
insert [`extra_actions`](/versions/7.3.0/reference/be/extra-actions#extra_action) into the build graph.

#### `--[no]experimental_extra_action_top_level_only` {:experimental-extra-action-top-level-only}

Warning: Extra actions are deprecated. Use
[aspects](/versions/7.3.0/extending/aspects) instead.
...
the [linkstatic attribute](/versions/7.3.0/reference/be/c-cpp#cc_binary.linkstatic) on build rules.
...
* `default`: Allows bazel to choose whether to link dynamically.
  See [linkstatic](/versions/7.3.0/reference/be/c-cpp#cc_binary.linkstatic) for more
  information.
...
* `off`: Links all targets in
  [mostly static](/versions/7.3.0/reference/be/c-cpp#cc_binary.linkstatic) mode.
...
[shrink_resources attribute](/versions/7.3.0/reference/be/android#android_binary.shrink_resources) on
...
["Make" variables](/versions/7.3.0/reference/be/make-variables) are derived from this option.
...
(around 10%) for the [execution phase](/versions/7.3.0/run/build#execution).
The drawback is that further incremental builds will be slower. See also
[memory-saving mode](/versions/7.3.0/configure/memory).
...
the [`.bazelrc`](/versions/7.3.0/run/bazelrc) file.
...
[--execution_log_json_file](/versions/7.3.0/reference/command-line-reference#flag--execution_log_json_file)
and
[--execution_log_binary_file](/versions/7.3.0/reference/command-line-reference#flag--execution_log_binary_file).
...
Please see background information on [Platforms](/versions/7.3.0/extending/platforms) and [Toolchains](/versions/7.3.0/extending/toolchains).
...
details, see the
[Starlark Configurations](/versions/7.3.0/extending/config#using-build-setting-aliases).
...
[Call Bazel from scripts](/versions/7.3.0/run/scripts). In particular, the following options
are strongly recommended:

*   [`--bazelrc=/dev/null`](/versions/7.3.0/run/bazelrc)
*   [`--nokeep_state_after_build`](/versions/7.3.0/reference/command-line-reference#flag--keep_state_after_build)
...
defined by the page [Writing Tests](/versions/7.3.0/reference/test-encyclopedia).
...
[`bazel build`](/versions/7.3.0/run/build).
...
[Writing Tests](/versions/7.3.0/reference/test-encyclopedia). Note that none of the
...
two commands is that query runs after the [loading phase](/versions/7.3.0/run/build#loading)
and cquery runs after the [analysis phase](/versions/7.3.0/run/build#analysis). These tools are an
...
[Bazel Query Reference](/versions/7.3.0/query/language).
...
For more details, see [Action Graph Query](/versions/7.3.0/query/aquery).
...
[Building with Bazel](/versions/7.3.0/run/build#quickstart).
...
[Client/server implementation](/versions/7.3.0/run/client-server).
...
*   The complete
    ["Make" environment](/versions/7.3.0/reference/be/make-variables). If the `--show_make_env` flag is
...
See [bazel mobile-install](/versions/7.3.0/docs/mobile-install) for more information.
...
[JSON trace profile](/versions/7.3.0/advanced/performance/json-trace-profile) previously
gathered during a Bazel invocation.

### `canonicalize-flags` {:#canonicalize-flags}

The [`canonicalize-flags`](/versions/7.3.0/reference/command-line-reference#canonicalize-flags-options)
...
[standard client/server mode](/versions/7.3.0/run/client-server), but instead runs a bazel
...
[the rc files](/versions/7.3.0/run/bazelrc#bazelrc-file-locations); for the current `command`,
