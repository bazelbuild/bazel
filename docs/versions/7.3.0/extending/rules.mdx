...
[build phases](/versions/7.3.0/extending/concepts). It is important to understand the three
phases of a build (loading, analysis, and execution). It is also useful to
learn about [macros](/versions/7.3.0/extending/macros) to understand the difference between rules and
macros. To get started, first review the [Rules Tutorial](/versions/7.3.0/rules/rules-tutorial).
...
[Starlark](/versions/7.3.0/rules/language) language. These rules are written in `.bzl` files, which
...
[analysis phase](/versions/7.3.0/extending/concepts#evaluation-model). This function does not run any
...
In a `.bzl` file, use the [rule](/versions/7.3.0/rules/lib/globals/bzl#rule) function to define a new
...
This defines a [kind of rule](/versions/7.3.0/query/language#kind) named `example_library`.
...
Rules can be [loaded](/versions/7.3.0/concepts/build-files#load) and called in `BUILD` files:
...
Starlark functions that call rules are called [Starlark macros](/versions/7.3.0/extending/macros).
Starlark macros must ultimately be called from `BUILD` files, and can only be
called during the [loading phase](/versions/7.3.0/extending/concepts#evaluation-model), when `BUILD`
...
from attribute names to schemas (created using the [`attr`](/versions/7.3.0/rules/lib/toplevel/attr)
module) to the `attrs` parameter of `rule`.
[Common attributes](/versions/7.3.0/reference/be/common-definitions#common-attributes), such as
...
various [types of dependencies](/versions/7.3.0/concepts/dependencies#types_of_dependencies):
...
[`attr.label_list`](/versions/7.3.0/rules/lib/toplevel/attr#label_list),
[`attr.label`](/versions/7.3.0/rules/lib/toplevel/attr#label), or
[`attr.label_keyed_string_dict`](/versions/7.3.0/rules/lib/toplevel/attr#label_keyed_string_dict))
specifies dependencies of a certain type
between a target and the targets whose labels (or the corresponding
[`Label`](/versions/7.3.0/rules/lib/builtins/Label) objects) are listed in that attribute when the target
...
[execution platform](/versions/7.3.0/extending/platforms) or a different repository instead, the
rule should obtain that tool from a [toolchain](/versions/7.3.0/extending/toolchains).

### Output attributes

*Output attributes*, such as [`attr.output`](/versions/7.3.0/rules/lib/toplevel/attr#output) and
[`attr.output_list`](/versions/7.3.0/rules/lib/toplevel/attr#output_list), declare an output file that the
...
strictly in the [analysis phase](/versions/7.3.0/extending/concepts#evaluation-model) and transform the
...
[rule context](/versions/7.3.0/rules/lib/builtins/ctx), conventionally named `ctx`. They return a list of
[providers](#providers).

### Targets

Dependencies are represented at analysis time as [`Target`](/versions/7.3.0/rules/lib/builtins/Target)
objects. These objects contain the [providers](#providers) generated when the
target's implementation function was executed.

[`ctx.attr`](/versions/7.3.0/rules/lib/builtins/ctx#attr) has fields corresponding to the names of each
...
[providers for native rules](/versions/7.3.0/rules/lib/providers) available as Starlark
global variables.
...
For the legacy style in which a [`struct`](/versions/7.3.0/rules/lib/builtins/struct) is returned from a
...
Files are represented by [`File`](/versions/7.3.0/rules/lib/builtins/File) objects. Since Bazel does not
...
functions (see [`ctx.actions`](/versions/7.3.0/rules/lib/builtins/actions)) to construct pieces of the
action graph.
...
[`ctx.files`](/versions/7.3.0/rules/lib/builtins/ctx#files) contains a list of the default outputs of all
...
[`ctx.file`](/versions/7.3.0/rules/lib/builtins/ctx#file) contains a single `File` or `None` for
dependency attributes whose specs set `allow_single_file=True`.
[`ctx.executable`](/versions/7.3.0/rules/lib/builtins/ctx#executable) behaves the same as `ctx.file`, but only
...
[`ctx.actions.declare_file`](/versions/7.3.0/rules/lib/builtins/actions#declare_file) and
[`ctx.actions.declare_directory`](/versions/7.3.0/rules/lib/builtins/actions#declare_directory). Often,
the names of outputs are based on the target's name,
[`ctx.label.name`](/versions/7.3.0/rules/lib/builtins/ctx#label):
...
from the corresponding fields of [`ctx.outputs`](/versions/7.3.0/rules/lib/builtins/ctx#outputs).
...
[`ctx.actions`](/versions/7.3.0/rules/lib/builtins/actions):

*   [`ctx.actions.run`](/versions/7.3.0/rules/lib/builtins/actions#run), to run an executable.
*   [`ctx.actions.run_shell`](/versions/7.3.0/rules/lib/builtins/actions#run_shell), to run a shell
    command.
*   [`ctx.actions.write`](/versions/7.3.0/rules/lib/builtins/actions#write), to write a string to a file.
*   [`ctx.actions.expand_template`](/versions/7.3.0/rules/lib/builtins/actions#expand_template), to
    generate a file from a template.

[`ctx.actions.args`](/versions/7.3.0/rules/lib/builtins/actions#args) can be used to efficiently
...
[analysis phase](/versions/7.3.0/extending/concepts#evaluation-model). It might depend on the value of
...
consumers, generally by accumulating that into a [`depset`](/versions/7.3.0/rules/lib/builtins/depset).
...
implementation function returns a [`struct`](/versions/7.3.0/rules/lib/builtins/struct) instead of list of
...
[`DefaultInfo`](/versions/7.3.0/rules/lib/providers/DefaultInfo):
...
time). During the [execution phase](/versions/7.3.0/extending/concepts#evaluation-model), Bazel creates
...
[`runfiles`](/versions/7.3.0/rules/lib/builtins/runfiles) objects can be created by the `runfiles` method
on the rule context, [`ctx.runfiles`](/versions/7.3.0/rules/lib/builtins/ctx#runfiles) and passed to the
...
[`data`](/versions/7.3.0/reference/be/common-definitions#common.data), whose outputs are added to
...
Providers can be defined using the [`provider`](/versions/7.3.0/rules/lib/globals/bzl#provider)
...
[`provider`](/versions/7.3.0/rules/lib/globals/bzl.html#provider) function. If this callback is given, the
...
setting the respective [`executable`](/versions/7.3.0/rules/lib/globals/bzl#rule.executable) or
[`test`](/versions/7.3.0/rules/lib/globals/bzl#rule.test) argument to `True` in the call to `rule`:
...
`executable` argument of a returned [`DefaultInfo`](/versions/7.3.0/rules/lib/providers/DefaultInfo)
...
a [`ctx.actions.run`](/versions/7.3.0/rules/lib/builtins/actions#run) or
[`ctx.actions.run_shell`](/versions/7.3.0/rules/lib/builtins/actions#run_shell) action this should be done
by the underlying tool that is invoked by the action. For a
[`ctx.actions.write`](/versions/7.3.0/rules/lib/builtins/actions#write) action, pass `is_executable=True`.
...
[Executable rules](/versions/7.3.0/reference/be/common-definitions#common-attributes-binaries) and
[test rules](/versions/7.3.0/reference/be/common-definitions#common-attributes-tests) have additional
attributes implicitly defined, in addition to those added for
[all rules](/versions/7.3.0/reference/be/common-definitions#common-attributes). The defaults of
implicitly-added attributes cannot be changed, though this can be worked around
by wrapping a private rule in a [Starlark macro](/versions/7.3.0/extending/macros) which alters the
default:
...
[`File.short_path`](/versions/7.3.0/rules/lib/builtins/File#short_path).
...
[`File`](/versions/7.3.0/rules/lib/builtins/File) objects for output attributes, use the corresponding
attribute of [`ctx.outputs`](/versions/7.3.0/rules/lib/builtins/ctx#outputs). Rules can
...
[`--output_groups`](/versions/7.3.0/reference/command-line-reference#flag--output_groups). For
...
[`OutputGroupInfo`](/versions/7.3.0/rules/lib/providers/OutputGroupInfo) provider. Note that unlike many
...
[aspect](/versions/7.3.0/extending/aspects) and the rule target to which that aspect is applied, as
...
[the execution platform](/versions/7.3.0/extending/platforms#overview) (which could be your host,
...
[user-defined transitions](/versions/7.3.0/extending/config#user-defined-transitions), which allow
rule authors a great deal of flexibility in changing configurations, with the
drawback of
[making the build graph larger and less comprehensible](/versions/7.3.0/extending/config#memory-and-performance-considerations).
...
Rules may access
[configuration fragments](/versions/7.3.0/rules/lib/fragments) such as
...
When the [`coverage`](/versions/7.3.0/reference/command-line-reference#coverage) command is run,
...
[`--instrumentation_filter`](/versions/7.3.0/reference/command-line-reference#flag--instrumentation_filter).
Test targets are excluded, unless
[`--instrument_test_targets`](/versions/7.3.0/reference/command-line-reference#flag--instrument_test_targets)
is specified.
...
[ctx.coverage_instrumented](/versions/7.3.0/rules/lib/builtins/ctx#coverage_instrumented) returns true in
...
[ctx.configuration.coverage_enabled](/versions/7.3.0/rules/lib/builtins/configuration#coverage_enabled).
...
[`coverage_common.instrumented_files_info`](/versions/7.3.0/rules/lib/toplevel/coverage_common#instrumented_files_info).
...
<img src="/versions/7.3.0/rules/validation_action_historical.svg" width="35%" />
...
<img src="/versions/7.3.0/rules/validation_action.svg" width="35%" />
...
*   The [`outputs`](/versions/7.3.0/rules/lib/globals/bzl#rule.outputs) parameter of `rule` specifies
...
[`ctx.runfiles`](/versions/7.3.0/rules/lib/builtins/ctx#runfiles) and the [`runfiles`](/versions/7.3.0/rules/lib/builtins/runfiles)
...
*   **Avoid** use of the `collect_data` and `collect_default` modes of
    [`ctx.runfiles`](/versions/7.3.0/rules/lib/builtins/ctx#runfiles). These modes implicitly collect
...
*   **Avoid** use of the `data_runfiles` and `default_runfiles` of the
    ...
    [`data`](/versions/7.3.0/reference/be/common-definitions#common-attributes.data)).
...
*   The fields `files`, `runfiles`, `data_runfiles`, `default_runfiles`, and
    ...
    [`DefaultInfo`](/versions/7.3.0/rules/lib/providers/DefaultInfo). It is not allowed to specify any of
...
*   The field `output_groups` takes a struct value and corresponds to an
    [`OutputGroupInfo`](/versions/7.3.0/rules/lib/providers/OutputGroupInfo).

In [`provides`](/versions/7.3.0/rules/lib/globals/bzl#rule.provides) declarations of rules, and in
[`providers`](/versions/7.3.0/rules/lib/toplevel/attr#label_list.providers) declarations of dependency
