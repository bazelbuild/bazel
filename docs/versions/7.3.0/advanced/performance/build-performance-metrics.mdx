...
[Breaking down build performance](/versions/7.3.0/configure/build-performance-breakdown) showcases
...
through the [Build Event Protocol (BEP)](/versions/7.3.0/remote/bep), which
...
Bazel provides 3 different query modes ([query](/versions/7.3.0/query/quickstart),
[cquery](/versions/7.3.0/query/cquery) and [aquery](/versions/7.3.0/query/aquery)) that allow users
to query the target graph, configured target graph and action graph
respectively. The query language provides a
[suite of functions](/versions/7.3.0/query/language#functions) usable across the different
...
format. The [JSON trace profile](/versions/7.3.0/advanced/performance/json-trace-profile) can
be very useful to quickly understand what Bazel spent time on during the
invocation.

## Execution Log {:#execution-log}

The [execution log](/versions/7.3.0/remote/cache-remote) can help you to troubleshoot and fix
missing remote cache hits due to machine and environment differences or
non-deterministic actions. If you pass the flag
[`--experimental_execution_log_spawn_metrics`](/versions/7.3.0/reference/command-line-reference#flag--experimental_execution_log_spawn_metrics)
