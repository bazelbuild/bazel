...
functions and types, see the [Bazel API reference](/versions/7.3.0/rules/lib/overview).
...
[Native functions](/versions/7.3.0/reference/be/functions) and [native rules](
...
* `import` (see [`load`](/versions/7.3.0/extending/concepts#loading-an-extension) statement).
* `while`, `yield`.
* float and set types.
