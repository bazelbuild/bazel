...
*   The `bazel` command line interface (CLI) is the user-facing frontend tool
    and receives commands from the user.

*   The CLI tool starts a [*Bazel server*](https://bazel.build/versions/7.7.0/run/client-server)
    for each distinct [output base](https://bazel.build/versions/7.7.0/remote/output-directories).
...
is also *invalidated*: for example, many `bazel` command line flags
cause the cache to be discarded. This is because many flags affect the build
graph (e.g. because of
[configurable attributes](https://bazel.build/versions/7.7.0/configure/attributes)). Some flag
changes can also cause the Bazel server to be restarted (e.g. changing
[startup options](https://bazel.build/versions/7.7.0/docs/user-manual#startup-options)).

A good execution cache is also valuable for build performance. An execution
cache can be kept locally
[on disk](https://bazel.build/versions/7.7.0/remote/caching#disk-cache), or
[remotely](https://bazel.build/versions/7.7.0/remote/caching). The cache can be shared among
...
*   Losing the Bazel server loses the analysis cache. The Bazel server has a
    [configurable](https://bazel.build/versions/7.7.0/docs/user-manual#max-idle-secs) idle
    time, after which it shuts down. You can configure this time using your
...
*   Beware that the Bazel server is killed if you press
    ...
    Ctrl-C once to request a graceful end of the current invocation. {#avoid-ctrl-c}
