...
## Running Bazel with Limited RAM {:#running-bazel-with-limited-ram}

In certain situations, you may want Bazel to use minimal memory. You can set the
maximum heap using the startup flag
[`--host_jvm_args`](/versions/7.7.0/docs/user-manual#host-jvm-args),
...
[`--discard_analysis_cache`](/versions/7.7.0/docs/user-manual#discard-analysis-cache),
[`--nokeep_state_after_build`](/versions/7.7.0/reference/command-line-reference#flag--keep_state_after_build),
and
[`--notrack_incremental_state`](/versions/7.7.0/reference/command-line-reference#flag--track_incremental_state).
...
[Memory Profiling section](/versions/7.7.0/rules/performance#memory-profiling) of our
