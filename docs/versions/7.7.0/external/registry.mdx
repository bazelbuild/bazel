...
*   If `type` is `archive` (the default), this module version is backed by an
    [`http_archive`](/versions/7.7.0/rules/lib/repo/http#http_archive) repo rule; it's fetched
    ...
    *   `archive_type`: A string, the archive type of the downloaded file (Same
        as [`type` on `http_archive`](/versions/7.7.0/rules/lib/repo/http#http_archive-type)).
*   If `type` is `git_repository`, this module version is backed by a
    [`git_repository`](/versions/7.7.0/rules/lib/repo/git#git_repository) repo rule; it's
...
*   If `type` is `local_path`, this module version is backed by a
    [`local_repository`](/versions/7.7.0/rules/lib/repo/local#local_repository) repo rule;
