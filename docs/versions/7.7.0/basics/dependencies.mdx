...
## Dealing with Modules and Dependencies {:#modules-dependencies}

Projects that use artifact-based build systems like Bazel are broken into a set
of modules, with modules expressing dependencies on one another using `BUILD`
...
## Using Fine-Grained Modules and the 1:1:1 Rule {:#fine-grained-modules-111}
...
## Minimizing Module Visibility {:#min-module-visibility}
...
[![Transitive dependencies](/versions/7.7.0/images/transitive-dependencies.png)](/images/transitive-dependencies.png)
...
target that used C using a dependency on B would break. Effectively, a target's
...
### Automatic versus manual dependency management {:#automatic-vs-manual-dependency-management}
...
### The One-Version Rule {:#one-version-rule}
...
### Transitive external dependencies {:#transitive-external-dependencies}
...
### Caching build results using external dependencies {:#cache-build-external}
...
### Security and reliability of external dependencies {:#security-external-deps}
