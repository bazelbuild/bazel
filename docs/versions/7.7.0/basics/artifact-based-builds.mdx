...
systems are good step over build scripts, they give too much power to
...
[build configurations](/versions/7.7.0/run/build#build-config-cross-compilation)
.
Rather than targets depending directly on their tools, they depend on types of
configurations:
...
[adding custom rules](/versions/7.7.0/extending/rules).
...
conflicts impossible by using _[sandboxing](/versions/7.7.0/docs/sandboxing)_.
On supported
