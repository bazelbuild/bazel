...
`cquery` is a variant of [`query`](/versions/7.7.0/query/language) that correctly handles
[`select()`](/versions/7.7.0/docs/configurable-attributes) and build options' effects on the build
graph.
...
the [configuration](/versions/7.7.0/reference/glossary#configuration) the
target is built with.

Since `cquery` runs over the configured target graph. it doesn't have insight
into artifacts like build actions nor access to [`test_suite`](/versions/7.7.0/reference/be/general#test_suite)
rules as they are not configured targets. For the former, see [`aquery`](/versions/7.7.0/query/aquery).
...
*   **`function(...)`** is the function to run on the target. `cquery`
    supports most
    of `query`'s [functions](/versions/7.7.0/query/language#functions), plus a
    few new ones.
*   **`//target`** is the expression fed to the function. In this example, the
    ...
    See the [Query guide](/versions/7.7.0/query/guide) for examples.

`cquery` requires a target to run through the [loading and analysis](/versions/7.7.0/extending/concepts#evaluation-model)
...
[core query ordering contract](/versions/7.7.0/query/language#graph-order).
...
Of the [set of functions](/versions/7.7.0/query/language#functions "list of query functions")
supported by `query`, `cquery` supports all but [`visible`](/versions/7.7.0/query/language#visible),
[`siblings`](/versions/7.7.0/query/language#siblings), [`buildfiles`](/versions/7.7.0/query/language#buildfiles),
and [`tests`](/versions/7.7.0/query/language#tests).
...
[options](/versions/7.7.0/reference/command-line-reference#build-options) available during a build.
...
[transitions](/versions/7.7.0/extending/rules#configurations),
...
[exec configuration](/versions/7.7.0/extending/rules#configurations)
...
[non-target configurations](/versions/7.7.0/extending/rules#configurations).
...
Include dependencies added by [aspects](/versions/7.7.0/extending/aspects).
...
Configuration [transitions](/versions/7.7.0/extending/rules#configurations)
...
See [query's proto output documentation](/versions/7.7.0/query/language#output-formats)
...
[graph output documentation](/versions/7.7.0/query/language#display-result-graph) for details. `cquery`
also supports [`--graph:node_limit`](/versions/7.7.0/query/language#graph-nodelimit) and
[`--graph:factored`](/versions/7.7.0/query/language#graph-factored).
...
[`--output_groups`](/versions/7.7.0/reference/command-line-reference#flag--output_groups) flag.
It does include source files.

All paths emitted by this output format are relative to the
[execroot](https://bazel.build/versions/7.7.0/remote/output-directories), which can be obtained
...
This output format calls a [Starlark](/versions/7.7.0/rules/language)
...
`target`. This function is called for each [Target](/versions/7.7.0/rules/lib/builtins/Target)
...
[Configurations](/versions/7.7.0/extending/config))
...
*   `cquery`'s interpretation of
    the [query language](/versions/7.7.0/query/language) introduces ambiguity
...
[incoming edge transitions](/versions/7.7.0/extending/config#incoming-edge-transitions).
...
**No support for [`--output=xml`](/versions/7.7.0/query/language#xml).**
