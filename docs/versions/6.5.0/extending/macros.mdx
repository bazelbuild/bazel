...
[loading phase](/versions/6.5.0/rules/concepts#evaluation-model), macros don't exist anymore,
...
Note: `$@` is a [Make variable](/versions/6.5.0/reference/be/make-variables#predefined_genrule_variables)
...
instantiated from the [native](/versions/6.5.0/rules/lib/native) module:
...
macro), use the function [native.package_name()](/versions/6.5.0/rules/lib/native#package_name).
Note that `native` can only be used in `.bzl` files, and not in `WORKSPACE` or
`BUILD` files.

## Label resolution in macros

Since macros are evaluated in the [loading phase](/versions/6.5.0/extending/concepts#evaluation-model),
...
[`Label`](/versions/6.5.0/rules/lib/Label#Label) constructor:
...
*   You can also use [print](/versions/6.5.0/rules/lib/globals#print) for debugging. It displays
...
If you want to throw an error, use the [fail](/versions/6.5.0/rules/lib/globals#fail) function.
