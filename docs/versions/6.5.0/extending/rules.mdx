...
from attribute names to schemas (created using the [`attr`](/versions/6.5.0/rules/lib/attr)
...
[`attr.label_list`](/versions/6.5.0/rules/lib/attr#label_list),
[`attr.label`](/versions/6.5.0/rules/lib/attr#label), or
[`attr.label_keyed_string_dict`](/versions/6.5.0/rules/lib/attr#label_keyed_string_dict))
specifies dependencies of a certain type
between a target and the targets whose labels (or the corresponding
[`Label`](/versions/6.5.0/rules/lib/Label) objects) are listed in that attribute when the target
...
*Output attributes*, such as [`attr.output`](/versions/6.5.0/rules/lib/attr#output) and
[`attr.output_list`](/versions/6.5.0/rules/lib/attr#output_list), declare an output file that the
...
[rule context](/versions/6.5.0/rules/lib/ctx), conventionally named `ctx`. They return a list of
[providers](#providers).

### Targets

Dependencies are represented at analysis time as [`Target`](/versions/6.5.0/rules/lib/Target)
objects. These objects contain the [providers](#providers) generated when the
target's implementation function was executed.

[`ctx.attr`](/versions/6.5.0/rules/lib/ctx#attr) has fields corresponding to the names of each
...
For the legacy style in which a [`struct`](/versions/6.5.0/rules/lib/struct) is returned from a
...
Files are represented by [`File`](/versions/6.5.0/rules/lib/File) objects. Since Bazel does not
...
functions (see [`ctx.actions`](/versions/6.5.0/rules/lib/actions)) to construct pieces of the
action graph.
...
[`ctx.files`](/versions/6.5.0/rules/lib/ctx#files) contains a list of the default outputs of all
...
[`ctx.file`](/versions/6.5.0/rules/lib/ctx#file) contains a single `File` or `None` for
dependency attributes whose specs set `allow_single_file=True`.
[`ctx.executable`](/versions/6.5.0/rules/lib/ctx#executable) behaves the same as `ctx.file`, but only
...
[`ctx.actions.declare_file`](/versions/6.5.0/rules/lib/actions#declare_file) and
[`ctx.actions.declare_directory`](/versions/6.5.0/rules/lib/actions#declare_directory). Often,
the names of outputs are based on the target's name,
[`ctx.label.name`](/versions/6.5.0/rules/lib/ctx#label):
...
from the corresponding fields of [`ctx.outputs`](/versions/6.5.0/rules/lib/ctx#outputs).
...
[`ctx.actions`](/versions/6.5.0/rules/lib/actions):

*   [`ctx.actions.run`](/versions/6.5.0/rules/lib/actions#run), to run an executable.
*   [`ctx.actions.run_shell`](/versions/6.5.0/rules/lib/actions#run_shell), to run a shell
    command.
*   [`ctx.actions.write`](/versions/6.5.0/rules/lib/actions#write), to write a string to a file.
*   [`ctx.actions.expand_template`](/versions/6.5.0/rules/lib/actions#expand_template), to
    generate a file from a template.

[`ctx.actions.args`](/versions/6.5.0/rules/lib/actions#args) can be used to efficiently
...
consumers, generally by accumulating that into a [`depset`](/versions/6.5.0/rules/lib/depset).
...
implementation function returns a [`struct`](/versions/6.5.0/rules/lib/struct) instead of list of
...
[`DefaultInfo`](/versions/6.5.0/rules/lib/DefaultInfo):
...
[`runfiles`](/versions/6.5.0/rules/lib/runfiles) objects can be created by the `runfiles` method
on the rule context, [`ctx.runfiles`](/versions/6.5.0/rules/lib/ctx#runfiles) and passed to the
...
[`provider`](/versions/6.5.0/rules/lib/globals.html#provider) function. If this callback is given, the
...
`executable` argument of a returned [`DefaultInfo`](/versions/6.5.0/rules/lib/DefaultInfo)
...
a [`ctx.actions.run`](/versions/6.5.0/rules/lib/actions#run) or
[`ctx.actions.run_shell`](/versions/6.5.0/rules/lib/actions#run_shell) action this should be done
by the underlying tool that is invoked by the action. For a
[`ctx.actions.write`](/versions/6.5.0/rules/lib/actions#write) action, pass `is_executable=True`.
...
[`File.short_path`](/versions/6.5.0/rules/lib/File#short_path).
...
[`File`](/versions/6.5.0/rules/lib/File) objects for output attributes, use the corresponding
attribute of [`ctx.outputs`](/versions/6.5.0/rules/lib/ctx#outputs). Rules can
...
[`OutputGroupInfo`](/versions/6.5.0/rules/lib/OutputGroupInfo) provider. Note that unlike many
...
[ctx.coverage_instrumented](/versions/6.5.0/rules/lib/ctx#coverage_instrumented) returns true in
...
[ctx.configuration.coverage_enabled](/versions/6.5.0/rules/lib/configuration#coverage_enabled).
...
[`coverage_common.instrumented_files_info`](/versions/6.5.0/rules/lib/coverage_common#instrumented_files_info).
...
<img src="/versions/6.5.0/rules/validation_action_historical.svg" width="35%" />
...
<img src="/versions/6.5.0/rules/validation_action.svg" width="35%" />
...
[`ctx.runfiles`](/versions/6.5.0/rules/lib/ctx#runfiles) and the [`runfiles`](/versions/6.5.0/rules/lib/runfiles)
...
*   **Avoid** use of the `collect_data` and `collect_default` modes of
    [`ctx.runfiles`](/versions/6.5.0/rules/lib/ctx#runfiles). These modes implicitly collect
...
*   The fields `files`, `runfiles`, `data_runfiles`, `default_runfiles`, and
    ...
    [`DefaultInfo`](/versions/6.5.0/rules/lib/DefaultInfo). It is not allowed to specify any of
...
[`providers`](/versions/6.5.0/rules/lib/attr#label_list.providers) declarations of dependency
