...
*  `target`: the [target](/versions/6.5.0/rules/lib/Target) the aspect is being applied to.
*   `ctx`: [`ctx`](/versions/6.5.0/rules/lib/ctx) object that can be used to access attributes
    and generate outputs and actions.

The implementation function can access the attributes of the target rule via
[`ctx.rule.attr`](/versions/6.5.0/rules/lib/ctx#rule). It can examine providers that are
...
[`OutputGroupInfo`](/versions/6.5.0/rules/lib/OutputGroupInfo)
(which is merged, so long as the
rule and aspect specify different output groups) and
[`InstrumentedFilesInfo`](/versions/6.5.0/rules/lib/InstrumentedFilesInfo)
(which is taken from the aspect). This means that aspect implementations may
never return [`DefaultInfo`](/versions/6.5.0/rules/lib/DefaultInfo).
