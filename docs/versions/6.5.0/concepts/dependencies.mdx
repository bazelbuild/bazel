...
<table class="cyan">
  ...
  <tr>
    <td><code>rule(     name = "a",     srcs = "a.in",     deps = "//b:b", )       </code></td>
    <td><code> rule(     name = "b",     srcs = "b.in",     deps = "//c:c", )       </code></td>
  </tr>
  <tr class="alt">
  ...
  <tr>
    <td><code> import b; b.foo();     </code></td>
    <td><code> import c; function foo() {{}"}   c.bar(); {{}"}       </code></td>
  </tr>
  <tr>
    <td><figure>         <img src="/versions/6.5.0/docs/images/a_b_c.svg"              alt="Declared dependency graph with arrows connecting a, b, and c" />         <figcaption><b>Declared</b> dependency graph</figcaption>       </figure></td>
    <td><figure>         <img src="/versions/6.5.0/docs/images/a_b_c.svg"              alt="Actual dependency graph that matches the declared dependency                   graph with arrows connecting a, b, and c" />         <figcaption><b>Actual</b> dependency graph</figcaption>       </figure></td>
  </tr>
</table>
...
<table class="cyan">
  ...
  <tr>
    <td><code>         import b;         import c;         b.foo();         c.garply();       </code></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td><figure>         <img src="/versions/6.5.0/docs/images/a_b_c.svg"              alt="Declared dependency graph with arrows connecting a, b, and c" />         <figcaption><b>Declared</b> dependency graph</figcaption>       </figure></td>
    <td><figure>         <img src="/versions/6.5.0/docs/images/a_b_c_ac.svg"              alt="Actual dependency graph with arrows connecting a, b, and c. An                   arrow now connects A to C as well. This does not match the                   declared dependency graph" />         <figcaption><b>Actual</b> dependency graph</figcaption>       </figure></td>
  </tr>
</table>
...
<table class="cyan">
  ...
  <tr>
    <td>&nbsp;</td>
    <td><code>rule(     name = "b",     srcs = "b.in",     <strong>deps = "//d:d",</strong> )       </code></td>
  </tr>
  <tr class="alt">
    <td>&nbsp;</td>
    <td><code>b / b.in</code></td>
  ...
  <tr>
    <td>&nbsp;</td>
    <td><code>       import d;       function foo() {{}"}         d.baz();       {{}"}       </code></td>
  </tr>
  <tr>
    <td><figure>         <img src="/versions/6.5.0/docs/images/ab_c.svg"              alt="Declared dependency graph with arrows connecting a and b.                   b no longer connects to c, which breaks a's connection to c" />         <figcaption><b>Declared</b> dependency graph</figcaption>       </figure></td>
    <td><figure>         <img src="/versions/6.5.0/docs/images/a_b_a_c.svg"              alt="Actual dependency graph that shows a connecting to b and c,                   but b no longer connects to c" />         <figcaption><b>Actual</b> dependency graph</figcaption>       </figure></td>
  </tr>
</table>
...
more details, see
[Attributes common to all rules](/versions/6.5.0/reference/be/common-definitions).
...
[Build Encyclopedia](/versions/6.5.0/reference/be/).
...
<p><span class="compare-worse">Not recommended</span> —   <code>data = ["//data/regression:unittest/."]</code></p>

<p><span class="compare-worse">Not recommended</span> —   <code>data = ["testdata/."]</code></p>
...
[`glob()`](/versions/6.5.0/reference/be/functions#glob) function. (Use `**` to force the
`glob()` to be recursive.)

<p><span class="compare-better">Recommended</span> —   <code>data = glob(["testdata/{{}"*}{{}"*}"])</code></p>
...
conform to the [label syntax](/versions/6.5.0/concepts/labels#labels-lexical-specification),
then explicit enumeration of files, or use of the
[`glob()`](/versions/6.5.0/reference/be/functions#glob) function produces an invalid labels
