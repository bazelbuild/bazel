...
* Targets can be [skipped](/versions/6.5.0/docs/platforms#skipping-incompatible-targets) in the
...
To learn how to use the configuration flags, see the [Bazel and Java](/versions/6.5.0/docs/bazel-and-java) manual.
...
can mix Apple projects with platorm-enabled C++ with [platform mappings](#platform-mappings)
...
"//command_line_option:platforms": "//:my_arm_platform" }` or use [platform mappings](#platform-mappings) to support both styles through the migration
window.
...
1. Flip on the "use platforms" flag for your project's languages ([if they have one](#status)) and do whatever testing you need to see if the projects you care
   about work.
