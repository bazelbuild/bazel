...
*   It requires a fair amount of boilerplate to set up the rules under test and
    ...
    first. It helps to [keep in mind](/versions/6.5.0/rules/concepts#evaluation-model) that macros
...
[`Action`](/versions/6.5.0/rules/lib/Action) objects which represent actions registered by the
target under test.
