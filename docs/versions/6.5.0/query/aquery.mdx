...
*   `function(...)`: the standard [functions](/versions/6.5.0/reference/query#functions)
...
[options](/versions/6.5.0/reference/command-line-reference#build-options)
...
[Skyframe](/versions/6.5.0/reference/skyframe) is the evaluation and
incrementality model of Bazel. On each instance of Bazel server, Skyframe stores the dependency graph
constructed from the previous runs of the [Analysis phase](/versions/6.5.0/docs/build#analysis).
...
It is possible for [Aspects](/versions/6.5.0/rules/aspects)
...
[dependency graph](/versions/6.5.0/rules/aspects#aspect_basics).
...
their inputs/outputs), the [JSON profile](/versions/6.5.0/rules/performance#performance-profiling)
