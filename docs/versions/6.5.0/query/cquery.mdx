...
It achieves this by running over the results of Bazel's [analysis phase](/rules/concepts#evaluation-model),
...
See `query`'s [target pattern documentation](/reference/query#target-patterns) for more information on target patterns.
