...
1.  [Install Bazel](/versions/6.5.0/install) if you have not already done so.

2.  If you're not familiar with Bazel and its concepts, complete the
    [iOS app tutorial](/versions/6.5.0/tutorials/ios-app). You should understand the Bazel
...
[Working with external dependencies](/versions/6.5.0/docs/external).
...
[Workspaces, packages, and targets](/versions/6.5.0/concepts/build-ref).
...
Add an [`objc_library`](/versions/6.5.0/reference/be/objective-c#objc_library)
...
Note: You can use the [`glob`](/versions/6.5.0/reference/be/functions#glob)
