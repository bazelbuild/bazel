...
To run Bazel, go to your base [workspace](/versions/6.5.0/concepts/build-ref#workspace) directory
...
* [`analyze-profile`](/versions/6.5.0/docs/user-manual#analyze-profile): Analyzes build profile data.
* [`aquery`](/versions/6.5.0/docs/user-manual#aquery): Executes a query on the [post-analysis](#analysis) action graph.
* [`build`](#bazel-build): Builds the specified targets.
* [`canonicalize-flags`](/versions/6.5.0/docs/user-manual#canonicalize-flags): Canonicalize Bazel flags.
* [`clean`](/versions/6.5.0/docs/user-manual#clean): Removes output files and optionally stops the server.
* [`cquery`](/versions/6.5.0/docs/cquery): Executes a [post-analysis](#analysis) dependency graph query.
* [`dump`](/versions/6.5.0/docs/user-manual#dump): Dumps the internal state of the Bazel server process.
* [`help`](/versions/6.5.0/docs/user-manual#help): Prints help for commands, or the index.
* [`info`](/versions/6.5.0/docs/user-manual#info): Displays runtime info about the bazel server.
* [`fetch`](#fetching-external-dependencies): Fetches all external dependencies of a target.
* [`mobile-install`](/versions/6.5.0/docs/user-manual#mobile-install): Installs apps on mobile devices.
* [`query`](/versions/6.5.0/docs/query-how-to): Executes a dependency graph query.
* [`run`](/versions/6.5.0/docs/user-manual#running-executables): Runs the specified target.
* [`shutdown`](/versions/6.5.0/docs/user-manual#shutdown): Stops the Bazel server.
* [`test`](/versions/6.5.0/docs/user-manual#running-tests): Builds and runs the specified test targets.
* [`version`](/versions/6.5.0/docs/user-manual#version): Prints version information for Bazel.

### Getting help 

* `bazel help <var>command</var>`: Prints help and options for
  `<var>command</var>`.
* `bazel help `[`startup_options`](/versions/6.5.0/docs/user-manual#startup-options): Options for the JVM hosting Bazel.
...
[`--show_result`](/versions/6.5.0/docs/user-manual#show-result) for more details.
...
Whereas [labels](/versions/6.5.0/concepts/labels) are used to specify individual
...
<tr>
  <td><code>//...</code></td>
  <td>All targets in packages in the workspace. This does not include targets   from <a href="/versions/6.5.0/docs/external">external repositories</a>.</td>
</tr>
<tr>
  <td><code>//:all</code></td>
  <td>All targets in the top-level package, if there is a `BUILD` file at the   root of the workspace.</td>
</tr>
</table>
...
Using the
[`--distdir=/path/to-directory`](/versions/6.5.0/reference/command-line-reference#flag--distdir)
...
[`--distdir`](/versions/6.5.0/reference/command-line-reference#flag--distdir)
...
output files. See [Hermeticity](/versions/6.5.0/concepts/hermeticity) for more details.
...
such as Bazel's [query](/versions/6.5.0/docs/query-how-to) command, which is implemented atop the loading
phase.
