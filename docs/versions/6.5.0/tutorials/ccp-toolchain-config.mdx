...
1.  If you have not already done so,
   [download and install Bazel 0.23](/versions/6.5.0/install/ubuntu) or later.
...
[`crosstool_top`](/versions/6.5.0/docs/user-manual#crosstool-top),
[`cpu`](/versions/6.5.0/docs/user-manual#cpu) and
[`host_crosstool_top`](/versions/6.5.0/docs/user-manual#host-crosstool-top).

When you build your [target](/versions/6.5.0/concepts/build-ref#targets)
with `bazel build --config=clang_config //main:hello-world`, Bazel uses your
custom toolchain from the
[cc_toolchain_suite](/versions/6.5.0/reference/be/c-cpp#cc_toolchain_suite)
`//toolchain:clang_suite`. The suite may list different
[toolchains](/versions/6.5.0/reference/be/c-cpp#cc_toolchain) for different CPUs,
...
3.  Run the build again. Bazel throws the following error:
    ...
    doesn't provide the necessary [`ToolchainInfo`](/versions/6.5.0/rules/lib/ToolchainInfo)
...
8.  Run the build again. Bazel throws the following error:
     ...
     [`cxx_builtin_include_directories`](/versions/6.5.0/rules/lib/cc_common#create_cc_toolchain_config_info)
...
- You have to let the toolchain know where the tools live. In this tutorial
  ...
  workspaces [here](/versions/6.5.0/reference/be/workspace). Your tools could come from a
