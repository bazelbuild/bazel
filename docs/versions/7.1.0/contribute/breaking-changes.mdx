...
[backward compatibility policy](/versions/7.1.0/release/backward-compatibility).
This document describes the process for Bazel contributors to make a breaking
change in Bazel to adhere to this policy.

1. Follow the [design document policy](/versions/7.1.0/contribute/design-documents).
