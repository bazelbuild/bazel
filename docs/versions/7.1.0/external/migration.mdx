...
*   **WORKSPACE**
    ...
    The [`workspace`](/versions/7.1.0/rules/lib/globals/workspace#workspace) function is used
...
*   **Bzlmod**
    ...
    [`module`](/versions/7.1.0/rules/lib/globals/module#module) function as the repository
    name. If the module name is different from the needed repository name, you
    can use `repo_name` attribute of the
    [`module`](/versions/7.1.0/rules/lib/globals/module#module) function to override the
    repository name.
...
*   **WORKSPACE**
    ...
    [`http_archive`](/versions/7.1.0/rules/lib/repo/http#http_archive) or
    [`git_repository`](/versions/7.1.0/rules/lib/repo/git#git_repository) repository rules to
...
*   **Bzlmod**
    ...
    [`bazel_dep`](/versions/7.1.0/rules/lib/globals/module#bazel_dep) directive.
...
Please read the [overrides](/versions/7.1.0/external/module#overrides) section for more
information.
...
[`use_repo_rule`](/versions/7.1.0/external/module#use_repo_rule) or [module
extensions](/external/extension).

*   **WORKSPACE**

    Download a file using the [`http_file`](/versions/7.1.0/rules/lib/repo/http#http_file)
    repository rule.
...
Please read the [module extensions](/versions/7.1.0/external/extension) page to learn more
...
*   **Bzlmod**
    ...
    With Bzlmod, the
    [`register_toolchains`](/versions/7.1.0/rules/lib/globals/module#register_toolchains) and
...
4. when not using `WORKSPACE.bzlmod`: toolchains registered in the `WORKSPACE`
   [suffix](/versions/7.1.0/external/migration#builtin-default-deps).
...
*   **WORKSPACE**
    ...
    [`local_repository`](/versions/7.1.0/reference/be/workspace#local_repository) and
...
*   **Bzlmod**
    ...
    [`local_path_override`](/versions/7.1.0/rules/lib/globals/module#local_path_override) to
...
The [`bind`](/versions/7.1.0/reference/be/workspace#bind) rule in WORKSPACE is deprecated and
...
*   Or use the [`alias`](/versions/7.1.0/reference/be/general#alias) build rule
...
*   No [prefixes or suffixes](/versions/7.1.0/external/migration#builtin-default-deps) are
...
an [apparent repository name](/versions/7.1.0/external/overview#apparent-repo-name) of
...
*   **Make sure the source tree follows the layout of the original repository.**
    ...
    version by [`archive_override`](/versions/7.1.0/rules/lib/globals/module#archive_override)
    and [`git_override`](/versions/7.1.0/rules/lib/globals/module#git_override).
...
[`bazel_dep`](/versions/7.1.0/rules/lib/globals/module#bazel_dep) and
[`use_extension`](/versions/7.1.0/rules/lib/globals/module#use_extension) directives so that
