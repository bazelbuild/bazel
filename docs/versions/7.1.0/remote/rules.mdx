...
[creating a toolchain rule](/versions/7.1.0/extending/toolchains#creating-a-toolchain-rule).
...
identifying and resolving dependency-related build errors. See [Troubleshooting Bazel Remote Execution with Docker Sandbox](/versions/7.1.0/remote/sandbox)
...
*   **Symlinking to local tools or artifacts.** Symlinks to tools or libraries
    ...
    tree. Do not use [`repository_ctx.symlink`](/versions/7.1.0/rules/lib/builtins/repository_ctx#symlink)
...
To help find potential non-hermetic behavior you can use [Workspace rules log](/versions/7.1.0/remote/workspace).
