...
![Build graph](/versions/6.0.0/rules/build-graph.png "Build graph")

**Figure 1.** `BUILD` file dependency graph.

Bazel analyzes this dependency graph by calling an implementation function of
the corresponding [rule](/versions/6.0.0/extending/rules) (in this case "java_library") for every
...
![Build Graph with Aspect](/versions/6.0.0/rules/build-graph-aspects.png "Build graph with aspects")
...
the [`aspect`](/versions/6.0.0/rules/lib/globals#aspect) function.
...
functions. They return [providers](/versions/6.0.0/extending/rules#providers), can generate
[actions](/versions/6.0.0/extending/rules#actions), and take two arguments:

*  `target`: the [target](/versions/6.0.0/rules/lib/Target) the aspect is being applied to.
*   `ctx`: [`ctx`](/versions/6.0.0/rules/lib/ctx) object that can be used to access attributes
    and generate outputs and actions.

The implementation function can access the attributes of the target rule via
[`ctx.rule.attr`](/versions/6.0.0/rules/lib/ctx#rule). It can examine providers that are
...
[`--aspects`](/versions/6.0.0/reference/command-line-reference#flag--aspects)
...
[`InstrumentedFilesInfo`](/versions/6.0.0/rules/lib/InstrumentedFilesInfo)
(which is taken from the aspect). This means that aspect implementations may
never return [`DefaultInfo`](/versions/6.0.0/rules/lib/DefaultInfo).
