...
*   define custom flags for your project, obsoleting the need for
     [`--define`](/versions/6.0.0/docs/configurable-attributes#custom-keys)
*   write
    [transitions](/versions/6.0.0/rules/lib/transition#transition) to configure deps in
...
[configuration](/versions/6.0.0/extending/rules#configurations)
...
[attribute](/versions/6.0.0/rules/lib/globals#rule.build_setting).
...
[documentation](/versions/6.0.0/rules/lib/config)  for details. More complicated typing can be
...
via default values and [transitions](/versions/6.0.0/rules/lib/transition#transition).
...
Like all rules, build setting rules have [implementation functions](/versions/6.0.0/extending/rules#implementation-function).
...
[`ctx`](/versions/6.0.0/rules/lib/ctx) objects of build setting rules. These implementation
...
the [`configuration_field`](/versions/6.0.0/rules/lib/globals#configuration_field)
 API.
...
Users can configure attributes on build settings by using
 [`select()`](/versions/6.0.0/reference/be/functions#select). Build setting targets can be passed to the `flag_values` attribute of
...
A configuration
[transition](/versions/6.0.0/rules/lib/transition#transition)
...
`transition()`
[function](/versions/6.0.0/rules/lib/transition#transition)
...
such as
[`--spawn_strategy`](/versions/6.0.0/docs/user-manual#spawn-strategy)
...
[target platform](/versions/6.0.0/extending/platforms).
...
![Scalability graph](/versions/6.0.0/rules/scalability-graph.png "Scalability graph")
...
This produces the following [configured targets](/versions/6.0.0/reference/glossary#configured-target):
 ...
 * [Bazel Configurability Roadmap](https://bazel.build/versions/6.0.0/community/roadmaps-configurability){: .external}
