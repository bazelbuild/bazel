...
[loading phase](/versions/6.0.0/extending/concepts#evaluation-model), macros don't exist anymore,
...
Note: `$@` is a [Make variable](/versions/6.0.0/reference/be/make-variables#predefined_genrule_variables)
...
instantiated from the [native](/versions/6.0.0/rules/lib/native) module:
...
macro), use the function [native.package_name()](/versions/6.0.0/rules/lib/native#package_name).
...
[`Label`](/versions/6.0.0/rules/lib/Label#Label) constructor:
...
*   You can also use [print](/versions/6.0.0/rules/lib/globals#print) for debugging. It displays
...
If you want to throw an error, use the [fail](/versions/6.0.0/rules/lib/globals#fail) function.
