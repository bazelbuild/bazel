...
1. Read the [Bazel Contribution policy](/versions/6.0.0/contribute/policy).
...
1. If you're proposing significant changes, write a
   [design document](/versions/6.0.0/contribute/design-documents).
...
1. Prepare a git commit that implements the feature. Don't forget to add tests
   ...
   [add release notes](/versions/6.0.0/contribute/release-notes). If it is an incompatible change,
   read the [guide for rolling out breaking changes](/versions/6.0.0/contribute/breaking-changes).
