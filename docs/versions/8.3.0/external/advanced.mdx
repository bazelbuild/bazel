...
[Bzlmod](/versions/8.3.0/external/overview#bzlmod), use a [multiple-version
...
use the
[`--override_repository`](/versions/8.3.0/reference/command-line-reference#flag--override_repository)
...
Note: With [Bzlmod](/versions/8.3.0/external/overview#bzlmod), remember to use canonical repo
names here. Alternatively, use the
[`--override_module`](/versions/8.3.0/reference/command-line-reference#flag--override_module)
flag to override a module to a local directory, similar to the
[`local_path_override`](/versions/8.3.0/rules/lib/globals/module#local_path_override) directive in
`MODULE.bazel`.
...
*   Use `--host_jvm_args=-Djava.net.preferIPv6Addresses=true` [startup
    ...
    following line in your [`.bazelrc` file](/versions/8.3.0/run/bazelrc):
...
fetch a file with [`ctx.download`](/versions/8.3.0/rules/lib/builtins/repository_ctx#download) or
[`ctx.download_and_extract`](/versions/8.3.0/rules/lib/builtins/repository_ctx#download_and_extract). By
