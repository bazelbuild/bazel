...
Methods available in BUILD files. See also the Build Encyclopedia for extra <a href="/versions/8.3.0/reference/be/functions">functions</a> and build rules, which can also be used in BUILD files.
    ...
    Declares metadata that applies to every rule in the package. It must be called at most once within a package (BUILD file). If called, it should be the first call in the BUILD file, right after the <code>load()</code> statements.
      ...
      <table class="table table-bordered table-condensed table-params">
        ...
        <tbody>
            ...
            <td>
                                     required<br/>
                          See the <a href="/versions/8.3.0/reference/be/functions#package"><code>package()</code></a> function in the Build Encyclopedia for applicable arguments.
            </td>
          </tr>
                </tbody>
      </table>
