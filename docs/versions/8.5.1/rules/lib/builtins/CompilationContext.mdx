...
Returns the set of search paths (as strings) for header files referenced by angle brackets, e.g. <code>#include &lt;foo/bar/header.h&gt;</code>. They can be either relative to the exec root or absolute. Usually passed with -isystem.
