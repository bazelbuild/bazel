...
When a `File` is passed to an [`Args`](/versions/8.5.1/rules/lib/builtins/Args) object without using a `map_each` function, it is converted to a string by taking the value of its `path` field.
...
The path of this file relative to the root of the ancestor's tree, if the ancestor's [is\_directory](#is_directory) field is true. `tree_relative_path` is only available for expanded files of a directory in an action command, i.e. [Args.add\_all()](/versions/8.5.1/rules/lib/builtins/Args#add_all). For other types of files, it is an error to access this field.