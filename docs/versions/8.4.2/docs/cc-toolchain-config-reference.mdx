...
[`CcToolchainConfigInfo`](/versions/8.4.2/rules/lib/providers/CcToolchainConfigInfo) is a provider that provides the necessary level of
...
[`toolchain_config`](/versions/8.4.2/reference/be/c-cpp#cc_toolchain.toolchain_config) attribute of the
[`cc_toolchain`](/versions/8.4.2/reference/be/c-cpp#cc_toolchain) to your rule.
You can create the `CcToolchainConfigInfo` by calling
[`cc_common.create_cc_toolchain_config_info()`](/versions/8.4.2/rules/lib/toplevel/cc_common#create_cc_toolchain_config_info).
...
1.  User specifies a `cc_toolchain_suite` target in the `BUILD` file and points
    ...
    [`--crosstool_top` option](/versions/8.4.2/docs/user-manual#flag--crosstool_top).
...
<td><code>implies = ['feature']</code></td>
   <td><p>Feature-level. This feature implies the specified feature(s).
       ...
       features cannot be disabled.</p></td>
  </tr>
  <tr>
<td><code>provides = ['feature']</code></td>
   <td><p>Feature-level. Indicates that this feature is one of several mutually
       ...
       for two or more mutually exclusive features at once.</p></td>
  </tr>
  <tr>
<td><code>with_features = [ with_feature_set( features = ['feature-1'], not_features = ['feature-2'], ), ]</code></td>
   <td>Flag set-level. A feature can specify multiple flag sets with multiple.
     ...
     applied unconditionally for every action specified.</td>
  </tr>
</table>
...
<table>
  ...
  <tr>
   ...
   <td>Run the compiler's parser on a header file to ensure that the header is
     ...
     only to toolchains that support modules.</td>
  </tr>
</table>
...
<table>
  ...
  <tr>
   ...
   <td>The executable to invoke. The tool applied to the action will be the
       ...
       configuration. Default value must be provided.</td>
  </tr>
  <tr>
...
<table>
  ...
  <tr>
   ...
   <td>Sequence of files the compiler must
       unconditionally include in the compiled source.</td>
  </tr>
  <tr>
   ...
   <td>Sequence directories in which the compiler
       ...
       and <code>#include "foo.h"</code>.</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>Sequence of flags from legacy
       ...
       <code>optional_cxx_flag</code>.</td>
  </tr>
  <tr>
   ...
   <td>Sequence of flags from either the
       ...
       <code>--cxxopt</code>, and <code>--conlyopt</code> flags.</td>
  </tr>
  <tr>
   ...
   <td>Sequence of flags from the
     <code>unfiltered_cxx_flag</code> legacy <code>CROSSTOOL</code> field or the
       ...
       the <code>nocopts</code> rule attribute.</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>Location of linker param file created by bazel to
       overcome command line length limit.</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>Linker flags coming from the <code>--linkopt</code>
       or <code>linkopts</code> attribute.</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>Presence of this variable indicates that fission (per-object debug info)
     is activated. Debug info will be in <code>.dwo</code> files instead
       of <code>.o</code> files and the compiler and linker need to know this.</td>
  </tr>
  <tr>
...
<table>
  ...
  <tr>
   ...
   <td>If enabled (and the option <code>--interface_shared_objects</code> is
     ...
     libraries. This makes incremental relinking faster.</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>If enabled, Bazel will link the C++ runtime statically in static linking
     ...
     linking mode) will be added to the linking actions.</td>
  </tr>
  <tr>
   ...
   <td>If enabled, toolchain will know to use PIC objects for dynamic libraries.
     ...
     validate that the feature is enabled. If the feature is missing, or couldn't
      be enabled, `--force_pic` cannot be used.</td>
  </tr>
  <tr>
    <td><strong><code>static_linking_mode | dynamic_linking_mode</code></strong></td>
    <td>Enabled by default based on linking mode.</td>
  ...
  <tr>
     ...
     <td>Prevents Bazel from adding legacy features to
