...
To run Bazel, go to your base [workspace](/versions/8.4.2/concepts/build-ref#workspace) directory
...
* [`analyze-profile`](/versions/8.4.2/docs/user-manual#analyze-profile): Analyzes build profile data.
* [`aquery`](/versions/8.4.2/docs/user-manual#aquery): Executes a query on the [post-analysis](#analysis) action graph.
* [`build`](#bazel-build): Builds the specified targets.
* [`canonicalize-flags`](/versions/8.4.2/docs/user-manual#canonicalize-flags): Canonicalize Bazel flags.
* [`clean`](/versions/8.4.2/docs/user-manual#clean): Removes output files and optionally stops the server.
* [`cquery`](/versions/8.4.2/query/cquery): Executes a [post-analysis](#analysis) dependency graph query.
* [`dump`](/versions/8.4.2/docs/user-manual#dump): Dumps the internal state of the Bazel server process.
* [`help`](/versions/8.4.2/docs/user-manual#help): Prints help for commands, or the index.
* [`info`](/versions/8.4.2/docs/user-manual#info): Displays runtime info about the bazel server.
* [`fetch`](#fetching-external-dependencies): Fetches all external dependencies of a target.
* [`mobile-install`](/versions/8.4.2/docs/user-manual#mobile-install): Installs apps on mobile devices.
* [`query`](/versions/8.4.2/query/guide): Executes a dependency graph query.
* [`run`](/versions/8.4.2/docs/user-manual#running-executables): Runs the specified target.
* [`shutdown`](/versions/8.4.2/docs/user-manual#shutdown): Stops the Bazel server.
* [`test`](/versions/8.4.2/docs/user-manual#running-tests): Builds and runs the specified test targets.
* [`version`](/versions/8.4.2/docs/user-manual#version): Prints version information for Bazel.

### Getting help

* `bazel help {{ '<var>' }}command{{ '</var>' }}`: Prints help and options for
  `{{ '<var>' }}command{{ '</var>' }}`.
* `bazel help `[`startup_options`](/versions/8.4.2/docs/user-manual#startup-options): Options for the JVM hosting Bazel.
...
[`--show_result`](/versions/8.4.2/docs/user-manual#show-result) for more details.
...
Whereas [labels](/versions/8.4.2/concepts/labels) are used to specify individual
...
<tr>
  ...
  from <a href="/versions/8.4.2/docs/external">external repositories</a>.</td>
</tr>
<tr>
  <td><code>//:all</code></td>
...
Using the
[`--distdir=/path/to-directory`](/versions/8.4.2/reference/command-line-reference#flag--distdir)
...
[`--repository_cache`](/versions/8.4.2/reference/command-line-reference#flag--repository_cache)
...
[`--distdir`](/versions/8.4.2/reference/command-line-reference#flag--distdir)
flag. You will need to do this once for every new Bazel binary version, since
the implicit dependencies can be different for every release.
...
output files. See [Hermeticity](/versions/8.4.2/basics/hermeticity) for more details.
...
such as Bazel's [query](/versions/8.4.2/query/guide) command, which is implemented atop the loading
phase.
