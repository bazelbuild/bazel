...
invocation [at a time](/versions/8.4.2/run/client-server#clientserver-implementation).
...
By default, Bazel uses a long-running [server process](/versions/8.4.2/run/client-server) as an
...
By default, Bazel reads the [`.bazelrc` file](/versions/8.4.2/run/bazelrc) from the base
...
See the [Performance Profiling](/versions/8.4.2/rules/performance#performance-profiling) section.
