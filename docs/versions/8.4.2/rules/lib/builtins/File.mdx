...
When a `File` is passed to an [`Args`](/versions/8.4.2/rules/lib/builtins/Args) object without using a `map_each` function, it is converted to a string by taking the value of its `path` field.
