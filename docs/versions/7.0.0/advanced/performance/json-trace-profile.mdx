...
[`--generate_json_trace_profile`](/versions/7.0.0/reference/command-line-reference#flag--generate_json_trace_profile)
flag, and the location it is written to with the
[`--profile`](/versions/7.0.0/docs/user-manual#profile) flag. Locations ending with `.gz` are
compressed with GZIP. Use the flag
[`--experimental_announce_profile_path`](/versions/7.0.0/reference/command-line-reference#flag--experimental_announce_profile_path)
...
![Example profile](/versions/7.0.0/docs/images/json-trace-profile.png "Example profile")
...
The Bazel subcommand [`analyze-profile`](/versions/7.0.0/docs/user-manual#analyze-profile)
...
*   `action count`: Displays how many concurrent actions were in flight. Click
    ...
    [`--jobs`](/versions/7.0.0/reference/command-line-reference#flag--jobs) in clean
    builds.
...
each other; pass
[`--noslim_json_profile`](/versions/7.0.0/reference/command-line-reference#flag--slim_profile)
