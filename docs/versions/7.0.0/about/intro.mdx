...
*   **Bazel is extensible.** Many [languages](/versions/7.0.0/rules) are
...
1.  **Set up Bazel.** Download and [install Bazel](/versions/7.0.0/install).

2.  **Set up a project [workspace](/versions/7.0.0/concepts/build-ref#workspaces)**, which is a
...
3.  **Write a `BUILD` file**, which tells Bazel what to build and how to
    ...
    [Starlark](/versions/7.0.0/rules/language), a domain-specific language. (See example
...
4. **Run Bazel** from the [command line](/versions/7.0.0/reference/command-line-reference). Bazel
   places your outputs within the workspace.

In addition to building, you can also use Bazel to run
[tests](/versions/7.0.0/reference/test-encyclopedia) and [query](/versions/7.0.0/query/guide) the build
...
2.  **Analyzes** the inputs and their
    [dependencies](/versions/7.0.0/concepts/dependencies), applies the specified build
    rules, and produces an [action](/versions/7.0.0/extending/concepts#evaluation-model)
    graph.
...
[hermetically](/versions/7.0.0/basics/hermeticity) through sandboxing, minimizing skew
and maximizing [reproducibility](/versions/7.0.0/run/build#correct-incremental-rebuilds).
...
[track](/versions/7.0.0/run/build#build-consistency) changes to
...
easily [trace dependencies](/versions/7.0.0/query/guide) in your code.

## Getting started tutorials {:#getting-started-tutorials}

To get started with Bazel, see [Getting Started](/versions/7.0.0/start/) or jump
directly to the Bazel tutorials:

*   [Tutorial: Build a C++ Project](/versions/7.0.0/start/cpp)
*   [Tutorial: Build a Java Project](/versions/7.0.0/start/java)
*   [Tutorial: Build an Android Application](/versions/7.0.0/start/android-app)
*   [Tutorial: Build an iOS Application](/versions/7.0.0/start/ios-app)
