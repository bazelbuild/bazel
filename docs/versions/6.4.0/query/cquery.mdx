...
`cquery` is a variant of [`query`](/versions/6.4.0/reference/query) that correctly handles
[`select()`](/versions/6.4.0/docs/configurable-attributes) and build options' effects on the build
graph.
...
the [configuration](/versions/6.4.0/reference/glossary#configuration) the
target is built with.

Since `cquery` runs over the configured target graph. it doesn't have insight
into artifacts like build actions nor access to `[test_suite](/versions/6.4.0/reference/be/general#test_suite)`
rules as they are not configured targets. For the former, see `[aquery](/versions/6.4.0/docs/aquery)`.
...
*   **`function(...)`** is the function to run on the target. `cquery`
    supports most
    of `query`'s [functions](/versions/6.4.0/reference/query#functions), plus a
    few new ones.
*   **`//target`** is the expression fed to the function. In this example, the
    ...
    See the [Query How-To](/versions/6.4.0/query/quickstart) for examples.

`cquery` requires a target to run through the [loading and analysis](/versions/6.4.0/rules/concepts#evaluation-model)
...
[core query ordering contract](/versions/6.4.0/reference/query#graph-order).
...
Of the [set of functions](/versions/6.4.0/reference/query#functions "list of query functions")
supported by `query`, `cquery` supports all but
[`allrdeps`](/versions/6.4.0/query/language#allrdeps),
[`buildfiles`](/versions/6.4.0/query/language#buildfiles),
[`rbuildfiles`](/versions/6.4.0/query/language#rbuildfiles),
[`siblings`](/versions/6.4.0/query/language#siblings),
[`tests`](/versions/6.4.0/query/language#tests), and
[`visible`](/versions/6.4.0/query/language#visible).
...
[options](/versions/6.4.0/reference/command-line-reference#build-options) available during a build.
...
[transitions](/versions/6.4.0/rules/rules#configurations),
...
[host configuration](/versions/6.4.0/rules/rules#configurations)
...
[non-target configurations](/versions/6.4.0/rules/rules#configurations).
...
[Aspects](/versions/6.4.0/rules/aspects) can add
...
Configuration [transitions](/versions/6.4.0/rules/rules#configurations)
...
See [query's proto output documentation](/versions/6.4.0/reference/query#output-formats)
...
[graph output documentation](/versions/6.4.0/reference/query#display-result-graph) for details. `cquery`
also supports [`--graph:node_limit`](/versions/6.4.0/reference/query#graph-nodelimit) and
[`--graph:factored`](/versions/6.4.0/reference/query#graph-factored).
...
[`--output_groups`](/versions/6.4.0/reference/command-line-reference#flag--output_groups) flag.
...
This output format calls a [Starlark](/versions/6.4.0/rules/language)
...
`target`. This function is called for each [Target](/versions/6.4.0/rules/lib/Target)
...
[Configurations](/versions/6.4.0/rules/config))
...
[providers](/versions/6.4.0/rules/rules#providers)
...
*   `cquery`'s interpretation of
    the [query language](/versions/6.4.0/reference/query) introduces ambiguity
...
[incoming edge transitions](/versions/6.4.0/rules/config#incoming-edge-transitions).
...
**No support for [`--output=xml`](/versions/6.4.0/reference/query#xml).**
