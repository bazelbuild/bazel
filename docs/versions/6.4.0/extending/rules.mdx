...
[build phases](/versions/6.4.0/rules/concepts). It is important to understand the three
phases of a build (loading, analysis, and execution). It is also useful to
learn about [macros](/versions/6.4.0/rules/macros) to understand the difference between rules and
macros. To get started, first review the [Rules Tutorial](/versions/6.4.0/rules/rules-tutorial).
...
[Starlark](/versions/6.4.0/rules/language) language. These rules are written in `.bzl` files, which
...
[analysis phase](/versions/6.4.0/rules/concepts#evaluation-model). This function does not run any
...
In a `.bzl` file, use the [rule](/versions/6.4.0/rules/lib/globals#rule) function to define a new
...
This defines a [kind of rule](/versions/6.4.0/reference/query#kind) named `example_library`.
...
Rules can be [loaded](/versions/6.4.0/concepts/build-files#load) and called in `BUILD` files:
...
Starlark functions that call rules are called [Starlark macros](/versions/6.4.0/rules/macros).
Starlark macros must ultimately be called from `BUILD` files, and can only be
called during the [loading phase](/versions/6.4.0/rules/concepts#evaluation-model), when `BUILD`
...
from attribute names to schemas (created using the [`attr`](/versions/6.4.0/rules/lib/attr)
module) to the `attrs` parameter of `rule`.
[Common attributes](/versions/6.4.0/reference/be/common-definitions#common-attributes), such as
...
various [types of dependencies](/versions/6.4.0/concepts/dependencies#types_of_dependencies):
...
[`attr.label_list`](/versions/6.4.0/rules/lib/attr#label_list),
[`attr.label`](/versions/6.4.0/rules/lib/attr#label), or
[`attr.label_keyed_string_dict`](/versions/6.4.0/rules/lib/attr#label_keyed_string_dict))
specifies dependencies of a certain type
between a target and the targets whose labels (or the corresponding
[`Label`](/versions/6.4.0/rules/lib/Label) objects) are listed in that attribute when the target
...
[execution platform](/versions/6.4.0/docs/platforms) or a different repository instead, the
rule should obtain that tool from a [toolchain](/versions/6.4.0/docs/toolchains).

### Output attributes

*Output attributes*, such as [`attr.output`](/versions/6.4.0/rules/lib/attr#output) and
[`attr.output_list`](/versions/6.4.0/rules/lib/attr#output_list), declare an output file that the
...
strictly in the [analysis phase](/versions/6.4.0/rules/concepts#evaluation-model) and transform the
...
[rule context](/versions/6.4.0/rules/lib/ctx), conventionally named `ctx`. They return a list of
[providers](#providers).

### Targets

Dependencies are represented at analysis time as [`Target`](/versions/6.4.0/rules/lib/Target)
objects. These objects contain the [providers](#providers) generated when the
target's implementation function was executed.

[`ctx.attr`](/versions/6.4.0/rules/lib/ctx#attr) has fields corresponding to the names of each
...
[providers for native rules](/versions/6.4.0/rules/lib/starlark-provider) available as Starlark
global variables.
...
For the legacy style in which a [`struct`](/versions/6.4.0/rules/lib/struct) is returned from a
...
Files are represented by [`File`](/versions/6.4.0/rules/lib/File) objects. Since Bazel does not
...
functions (see [`ctx.actions`](/versions/6.4.0/rules/lib/actions)) to construct pieces of the
action graph.
...
[`ctx.files`](/versions/6.4.0/rules/lib/ctx#files) contains a list of the default outputs of all
...
[`ctx.file`](/versions/6.4.0/rules/lib/ctx#file) contains a single `File` or `None` for
dependency attributes whose specs set `allow_single_file=True`.
[`ctx.executable`](/versions/6.4.0/rules/lib/ctx#executable) behaves the same as `ctx.file`, but only
...
[`ctx.actions.declare_file`](/versions/6.4.0/rules/lib/actions#declare_file) and
[`ctx.actions.declare_directory`](/versions/6.4.0/rules/lib/actions#declare_directory). Often,
the names of outputs are based on the target's name,
[`ctx.label.name`](/versions/6.4.0/rules/lib/ctx#label):
...
from the corresponding fields of [`ctx.outputs`](/versions/6.4.0/rules/lib/ctx#outputs).
...
[`ctx.actions`](/versions/6.4.0/rules/lib/actions):

*   [`ctx.actions.run`](/versions/6.4.0/rules/lib/actions#run), to run an executable.
*   [`ctx.actions.run_shell`](/versions/6.4.0/rules/lib/actions#run_shell), to run a shell
    command.
*   [`ctx.actions.write`](/versions/6.4.0/rules/lib/actions#write), to write a string to a file.
*   [`ctx.actions.expand_template`](/versions/6.4.0/rules/lib/actions#expand_template), to
    generate a file from a template.

[`ctx.actions.args`](/versions/6.4.0/rules/lib/actions#args) can be used to efficiently
...
[analysis phase](/versions/6.4.0/rules/concepts#evaluation-model). It might depend on the value of
...
consumers, generally by accumulating that into a [`depset`](/versions/6.4.0/rules/lib/depset).
...
implementation function returns a [`struct`](/versions/6.4.0/rules/lib/struct) instead of list of
...
[`DefaultInfo`](/versions/6.4.0/rules/lib/DefaultInfo):
...
time). During the [execution phase](/versions/6.4.0/rules/concepts#evaluation-model), Bazel creates
...
[`runfiles`](/versions/6.4.0/rules/lib/runfiles) objects can be created by the `runfiles` method
on the rule context, [`ctx.runfiles`](/versions/6.4.0/rules/lib/ctx#runfiles) and passed to the
...
[`data`](/versions/6.4.0/reference/be/common-definitions#common.data), whose outputs are added to
...
Providers can be defined using the [`provider`](/versions/6.4.0/rules/lib/globals#provider)
...
[`provider`](/versions/6.4.0/rules/lib/globals.html#provider) function. If this callback is given, the
...
setting the respective [`executable`](/versions/6.4.0/rules/lib/globals#rule.executable) or
[`test`](/versions/6.4.0/rules/lib/globals#rule.test) argument to `True` in the call to `rule`:
...
`executable` argument of a returned [`DefaultInfo`](/versions/6.4.0/rules/lib/DefaultInfo)
...
a [`ctx.actions.run`](/versions/6.4.0/rules/lib/actions#run) or
[`ctx.actions.run_shell`](/versions/6.4.0/rules/lib/actions#run_shell) action this should be done
by the underlying tool that is invoked by the action. For a
[`ctx.actions.write`](/versions/6.4.0/rules/lib/actions#write) action, pass `is_executable=True`.
...
[Executable rules](/versions/6.4.0/reference/be/common-definitions#common-attributes-binaries) and
[test rules](/versions/6.4.0/reference/be/common-definitions#common-attributes-tests) have additional
attributes implicitly defined, in addition to those added for
[all rules](/versions/6.4.0/reference/be/common-definitions#common-attributes). The defaults of
implicitly-added attributes cannot be changed, though this can be worked around
by wrapping a private rule in a [Starlark macro](/versions/6.4.0/rules/macros) which alters the
default:
...
[`File.short_path`](/versions/6.4.0/rules/lib/File#short_path).
...
[`File`](/versions/6.4.0/rules/lib/File) objects for output attributes, use the corresponding
attribute of [`ctx.outputs`](/versions/6.4.0/rules/lib/ctx#outputs). Rules can
...
[`--output_groups`](/versions/6.4.0/reference/command-line-reference#flag--output_groups). For
...
[`OutputGroupInfo`](/versions/6.4.0/rules/lib/OutputGroupInfo) provider. Note that unlike many
...
[aspect](/versions/6.4.0/rules/aspects) and the rule target to which that aspect is applied, as
...
[the execution platform](/versions/6.4.0/docs/platforms#overview) (which could be your host,
...
[user-defined transitions](/versions/6.4.0/rules/config#user-defined-transitions), which allow
rule authors a great deal of flexibility in changing configurations, with the
drawback of
[making the build graph larger and less comprehensible](/versions/6.4.0/rules/config#memory-and-performance-considerations).
...
Rules may access
[configuration fragments](/versions/6.4.0/rules/lib/starlark-configuration-fragment) such as
...
When the [`coverage`](/versions/6.4.0/reference/command-line-reference#coverage) command is run,
...
[`--instrumentation_filter`](/versions/6.4.0/reference/command-line-reference#flag--instrumentation_filter).
Test targets are excluded, unless
[`--instrument_test_targets`](/versions/6.4.0/reference/command-line-reference#flag--instrument_test_targets)
is specified.
...
[ctx.coverage_instrumented](/versions/6.4.0/rules/lib/ctx#coverage_instrumented) returns true in
...
[ctx.configuration.coverage_enabled](/versions/6.4.0/rules/lib/configuration#coverage_enabled).
...
[`coverage_common.instrumented_files_info`](/versions/6.4.0/rules/lib/coverage_common#instrumented_files_info).
...
<img src="/versions/6.4.0/rules/validation_action_historical.svg" width="35%" />
...
<img src="/versions/6.4.0/rules/validation_action.svg" width="35%" />
...
*   The [`outputs`](/versions/6.4.0/rules/lib/globals#rule.outputs) parameter of `rule` specifies
...
[`ctx.runfiles`](/versions/6.4.0/rules/lib/ctx#runfiles) and the [`runfiles`](/versions/6.4.0/rules/lib/runfiles)
...
*   **Avoid** use of the `collect_data` and `collect_default` modes of
    [`ctx.runfiles`](/versions/6.4.0/rules/lib/ctx#runfiles). These modes implicitly collect
...
*   **Avoid** use of the `data_runfiles` and `default_runfiles` of the
    ...
    [`data`](/versions/6.4.0/reference/be/common-definitions#common-attributes.data)).
...
*   The fields `files`, `runfiles`, `data_runfiles`, `default_runfiles`, and
    ...
    [`DefaultInfo`](/versions/6.4.0/rules/lib/DefaultInfo). It is not allowed to specify any of
...
*   The field `output_groups` takes a struct value and corresponds to an
    [`OutputGroupInfo`](/versions/6.4.0/rules/lib/OutputGroupInfo).

In [`provides`](/versions/6.4.0/rules/lib/globals#rule.provides) declarations of rules, and in
[`providers`](/versions/6.4.0/rules/lib/attr#label_list.providers) declarations of dependency
