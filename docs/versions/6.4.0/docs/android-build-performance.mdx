...
[Optimizing Performance](/versions/6.4.0/rules/performance).

## Recommended flags {:#recommended-flags}

The flags are in the
[`bazelrc` configuration syntax](/versions/6.4.0/docs/bazelrc#bazelrc-syntax-semantics), so
...
See the [JSON Profile documentation](/versions/6.4.0/rules/performance#performance-profiling) for
