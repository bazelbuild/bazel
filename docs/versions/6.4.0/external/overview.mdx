...
releases, check the [Bzlmod migration guide](/versions/6.4.0/external/migration) on how to
migrate.
...
See [Repository rules](/versions/6.4.0/extending/repo) for more information about how to write
your own repository rules.

The most common repo rules by far are
[`http_archive`](/versions/6.4.0/rules/lib/repo/http#http_archive), which downloads an archive
from a URL and extracts it, and
[`local_repository`](/versions/6.4.0/reference/be/workspace#local_repository), which symlinks a
...
[output base](/versions/6.4.0/remote/output-directories), under its canonical name.
...
A [Bazel **module**](/versions/6.4.0/external/module) is a Bazel project that can have multiple
...
[Bazel registry](/versions/6.4.0/external/registry) — by default, the [Bazel Central
...
consumed by [**module extensions**](/versions/6.4.0/external/extension) after module resolution
