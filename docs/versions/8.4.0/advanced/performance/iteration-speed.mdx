...
*   The CLI tool starts a [*Bazel server*](https://bazel.build/versions/8.4.0/run/client-server)
    for each distinct [output base](https://bazel.build/versions/8.4.0/remote/output-directories).
...
[configurable attributes](https://bazel.build/versions/8.4.0/configure/attributes)). Some flag
changes can also cause the Bazel server to be restarted (e.g. changing
[startup options](https://bazel.build/versions/8.4.0/docs/user-manual#startup-options)).

A good execution cache is also valuable for build performance. An execution
cache can be kept locally
[on disk](https://bazel.build/versions/8.4.0/remote/caching#disk-cache), or
[remotely](https://bazel.build/versions/8.4.0/remote/caching). The cache can be shared among
...
*   Losing the Bazel server loses the analysis cache. The Bazel server has a
    [configurable](https://bazel.build/versions/8.4.0/docs/user-manual#max-idle-secs) idle
