...
6.  Run the build again. Bazel throws the following error:
    ...
    [`cxx_builtin_include_directories`](/versions/8.4.0/rules/lib/toplevel/cc_common#create_cc_toolchain_config_info)
...
-   You have to let the toolchain know where the tools live. In this tutorial
    ...
    [external dependencies](/versions/8.4.0/external/overview). Your tools could come from a
