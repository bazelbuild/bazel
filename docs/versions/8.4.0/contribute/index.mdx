...
[patch acceptance process](/versions/8.4.0/contribute/patch-acceptance).

## Bazel's code description {:#code-description}

Bazel has a large codebase with code in multiple locations. See the [codebase guide](/versions/8.4.0/contribute/codebase) for more details.
...
*  Server code is in `src/main/java` and `src/test/java`.
   *  Core code which is mostly composed of [SkyFrame](/versions/8.4.0/reference/skyframe)
      and some utilities.
   *  Built-in rules are in `com.google.devtools.build.lib.rules` and in
     ...
     the [Challenges of Writing Rules](/versions/8.4.0/rules/challenges) first.
*  Java native interfaces are in `src/main/native`.
*  Various tooling for language support are described in the list in the
   [compiling Bazel](/versions/8.4.0/install/compile-source) section.
...
[Bazel Code Search User Guide](/versions/8.4.0/contribute/search).
