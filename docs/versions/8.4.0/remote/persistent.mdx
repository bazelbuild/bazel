...
[execution strategy](/versions/8.4.0/docs/user-manual#execution-strategy) that decreases
...
[strategy](/versions/8.4.0/docs/user-manual#execution-strategy) for the applicable tool
...
strategy, pass the
[--experimental_spawn_scheduler](/versions/8.4.0/reference/command-line-reference#flag--experimental_spawn_scheduler)
...
with the
[`worker_max_instances`](/versions/8.4.0/reference/command-line-reference#flag--worker_max_instances)
...
![Graph of performance improvements of clean builds](/versions/8.4.0/docs/images/workers-clean-chart.png "Performance improvements of clean builds")
...
![Graph of performance improvements of incremental builds](/versions/8.4.0/docs/images/workers-incremental-chart.png "Performance improvements of incremental builds")
...
You can pass the
[`--worker_extra_flag`](/versions/8.4.0/reference/command-line-reference#flag--worker_extra_flag)
...
Passing the
[`--worker_sandboxing`](/versions/8.4.0/reference/command-line-reference#flag--worker_sandboxing)
flag makes each worker request use a separate sandbox directory for all its
inputs. Setting up the [sandbox](/versions/8.4.0/docs/sandboxing) takes some extra time,
especially on macOS, but gives a better correctness guarantee.

The
[`--worker_quit_after_build`](/versions/8.4.0/reference/command-line-reference#flag--worker_quit_after_build)
...
[`--worker_verbose`](/versions/8.4.0/reference/command-line-reference#flag--worker_verbose) to
...
[Android Build Performance page](/versions/8.4.0/docs/android-build-performance).

## Implementing persistent workers {:#implementation}

See the [creating persistent workers](/versions/8.4.0/remote/creating) page for more
...
[multiplex workers](/versions/8.4.0/remote/multiplex) feature allows using multiple
...
[sandbox](/versions/8.4.0/docs/sandboxing), similar to the `local` strategy. You can set the
