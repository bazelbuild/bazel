...
| [`bazel-contrib/bazel-docs`](https://github.com/bazel-contrib/bazel-docs) | Hosting pipeline. Syncs from `bazelbuild/bazel`, defines the sidebar navigation, generates PR previews, and deploys to [preview.bazel.build](https://preview.bazel.build). |
...
2. Add your page to the sidebar navigation. This requires a separate change
   to `docs-tabs.json` in `bazel-contrib/bazel-docs`. See
   [Update the docs navigation](/contribute/docs-navigation) for instructions.
   For an initial PR, you can skip this and ask a maintainer to help. Your page
   is published either way, but it doesn't appear in the sidebar until the
   navigation change lands.
...
<Tip>
  ...
  do not appear in the sidebar until you add them to `docs-tabs.json` in
