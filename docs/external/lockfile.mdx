...
*   `error`: Like `update`, but if any information consulted during resolution
    ...
    To also check for information that would be pruned, use
    [`bazel mod tidy --diff`](#updating-and-checking).
*   `off`: The lockfile is neither checked nor updated.

### Updating and checking module files and the lockfile {#updating-and-checking}

To update `MODULE.bazel`, its included module files, and `MODULE.bazel.lock`, run:

```sh
bazel mod tidy
```

This formats module files, fixes `use_repo` calls for supported extensions, and
requests all extensions in the dependency graph, including those used only by
dependencies. The lockfile updater adds missing information, updates stale
results, and prunes information that is no longer needed. Valid cached extension
results are reused.

For a CI check that reports the same changes without writing these files, run:

```sh
bazel mod tidy --diff
```

The command prints unified diffs for module files and a short diagnostic if the
lockfile would change. It exits with code 1 if either needs updating, or code 0
if neither does. Evaluation errors also cause failure. The check may download
dependencies, evaluate extensions, and update Bazel's caches, including the
[hidden lockfile](#hidden-lockfile).

These commands require the `update` (default) or `refresh` lockfile mode. If
your `.bazelrc` sets another mode, override it with `--lockfile_mode=update`.

For CI builds and tests, use `error` to require the recorded information needed
by that invocation to be up-to-date:

```sh
bazel test --lockfile_mode=error //...
```

This complements the tidy check, particularly when build jobs use different
platforms or flags. It permits unused lockfile entries and does not check module
file formatting.

To also refresh mutable registry information, such as yanked status, use
`--lockfile_mode=refresh` in both the update and check commands. With
`--lockfile_mode=error`, `tidy --diff` rejects missing or stale required
information but does not check for pruning; with `off`, it only checks module
files.

These workflows use the current flags and host environment. Results recorded
for other operating systems or architectures may be retained without being
revalidated. The lockfile check uses the updater's comparison of parsed lockfile
contents; it does not enforce JSON whitespace or key ordering.
...
*   Regularly update the lockfile to reflect changes in project dependencies or
    ...
    up-to-date and accurate set of dependencies. Run
    [`bazel mod tidy`](#updating-and-checking) to update module files and lock
    down all extensions at once.
