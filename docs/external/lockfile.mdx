...
This formats module files, fixes supported `use_repo` calls, and evaluates all
extensions in the dependency graph. The lockfile updater adds missing
information, updates stale results, and prunes information that is no longer
needed.
...
The command prints module-file diffs and reports whether the lockfile would
change. It exits with code 1 if changes are needed and fails on evaluation
errors. It may download dependencies and update caches, including the
[hidden lockfile](#hidden-lockfile).

Checking lockfile additions and pruning requires `update` (default) or `refresh`.
If your `.bazelrc` sets another mode, override it with `--lockfile_mode=update`.
...
This complements the tidy check for jobs that use different platforms or flags.
It permits unused lockfile entries and ignores module-file formatting.
...
The check uses the current flags and host environment and compares parsed
lockfile contents, so it does not enforce JSON whitespace or key ordering.
Results for other operating systems or architectures may be retained without
being revalidated.
...
*   Regularly update the lockfile to reflect changes in project dependencies or
    ...
    up-to-date and accurate set of dependencies. Run `bazel mod tidy` to format
    module files, fix supported `use_repo` calls, and update the lockfile from
    all extensions in the dependency graph, including those used only by
    dependencies. Valid cached extension results are reused.
