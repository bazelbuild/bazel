...
*   `tidy`: Formats `MODULE.bazel` and its included module files and updates
    `use_repo` calls for supported module extensions. Pass `--diff` to print
    a unified diff of the changes without modifying them. This exits with code 1
    if changes are needed, including formatting changes, and code 0 if the files
    are already tidy. Errors evaluating extensions or running buildozer still
