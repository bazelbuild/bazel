...
*   `tidy`: Formats `MODULE.bazel` and its included module files and updates
    `use_repo` calls for supported module extensions. It evaluates all extensions
    in the dependency graph to update `MODULE.bazel.lock`. Pass `--diff` to
    report the same changes without modifying files and exit with code 1 if any
    are needed. See the
