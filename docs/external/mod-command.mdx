...
*   `show_repo <arg>...`: Displays the definition and location of the
    specified repos (see [example](#mod-example6)). This does not cause the
    repos to be materialized, use `bazel fetch` for that. Use `--all_repos` to
    show definitions of all repos in the entire dependency graph, or
    `--all_visible_repos` to show definitions of all repos visible from the
    `--base_module`.
...
6.  <span id="mod-example6"></span>See the underlying rule of some your modules'
    ...
    ```none
    ## rules_cc@0.0.1:
    # Located at: /home/user/.cache/bazel/_bazel_user/6e893e0f5a92cc4cf5909a6e4b2770f9/external/rules_cc+
    # <builtin>
    http_archive(
      name = "rules_cc+",
    ...
    ## stardoc:
    # Located at: /home/user/.cache/bazel/_bazel_user/6e893e0f5a92cc4cf5909a6e4b2770f9/external/stardoc+
    # <builtin>
    http_archive(
      name = "stardoc+",
