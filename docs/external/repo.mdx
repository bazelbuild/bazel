...
*   For Bzlmod-managed repositories, the [repository mapping](/external/overview#apparent-repo-name)
    entries recorded by the repository rule (for example, when resolving
    labels). A re-fetch is only triggered if the mapping content changes; the
    order in which these mappings are recorded is ignored.
