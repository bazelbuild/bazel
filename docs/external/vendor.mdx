...
files. Tidy also requests every module extension in the dependency graph to
update the lockfile. Vendoring only build targets and the tools filegroup may
therefore omit dependencies needed by extensions used only by dependencies.
...
Re-vendor when dependencies or extension inputs change. `--nofetch` prevents
repository fetching; extensions may still access the network themselves.
