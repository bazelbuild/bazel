...
external_directory)`. Vendoring external dependencies means moving out
...
a symlink to it under `$(bazel info external_directory)` instead of actually
...
-   Create a symlink `<vendor_dir>/bazel-external` that points to `$(bazel info
    external_directory)`. It is refreshed by every Bazel command automatically.
-   For the vendored source, rewrite all symlinks that originally point to a
    path under `$(bazel info external_directory)` to a relative path under
    `<vendor_dir>/bazel-external`.

For example, if the original symlink is

```none
<vendor_dir>/repo_foo+/link  =>  $(bazel info external_directory)/repo_bar+/file
```
...
```none
<vendor_dir>/bazel-external  =>  $(bazel info external_directory)  # This might be new if output base is changed
```
...
external_directory)` are not rewritten. Therefore, it could still break
