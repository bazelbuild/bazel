...
* `processwrapper-sandbox` causes commands to be executed inside a portable symlink-based
  sandbox without OS-specific namespace or container mechanisms.
* `linux-sandbox` causes commands to be executed inside a Linux namespace sandbox (using
  User, Mount, PID, Network, and IPC namespaces).
* `darwin-sandbox` causes commands to be executed inside a macOS sandbox using `sandbox-exec`.
* `windows-sandbox` causes commands to be executed inside a Windows sandbox.
...
and templates. Many binary or 'top-level' rules, and other rules like
`genrule`, support the `stamp` attribute, which can embed workspace status
...
Bazel specifying the time at which the message was output.
