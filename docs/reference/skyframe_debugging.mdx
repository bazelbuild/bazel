...
- `--skyframe=summary`: A count the nodes and edges in Skyframe
...
Thetefore, the skykey for the configured target for the bazel binary would be
...
- The `SkyKey.toString()` representation does not perform regex escaping. In the
  example, the characters "{[]}" are all used in regular expressions and will
  need to be escaped when used with `--skykey_filter` (and possibly escaped from
  the shell, as well).
...
Commonly interesting types are:
...
The output of `bazel dump --skyframe=rdeps` gives all skykeys that are dependedn
