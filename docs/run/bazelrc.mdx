...
Platform specific configs in the `.bazelrc` are enabled by default (controlled
by `--[no]enable_platform_specific_config`). For example, if the host OS is Linux and
