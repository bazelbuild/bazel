...
2.  [Integrate Swift package dependencies](#integrate-swiftpm)
...
### Step 2: Integrate Swift package dependencies {#integrate-swiftpm}

If the Xcode project depends on Swift packages, use [rules_swift_package_manager](https://github.com/cgrindel/rules_swift_package_manager)
to make them available as Bazel targets. It reads the `Package.swift` and
`Package.resolved` files you already know from Swift Package Manager, so you
keep declaring dependencies the same way.

First, add the ruleset and the Swift package extension to your `MODULE.bazel`
file:
...
Next, create a `Package.swift` file at the workspace root listing only the
packages your Bazel targets depend on directly:
...
Then resolve the packages and let Bazel record the resulting repositories:
...
`swift package update` writes `Package.resolved`, and `bazel mod tidy` adds each
resolved package to the `use_repo` call in `MODULE.bazel`. Each package becomes
an external repository named `swiftpkg_` plus the package identity, lowercased
with punctuation replaced by underscores, and each product in the package
becomes a target in it. Depend on those targets like any other:
...
Check `Package.swift`, `Package.resolved`, and `MODULE.bazel` into version
control. Re-run both commands above whenever you change `Package.swift`.

Once `Package.resolved` exists, `bazel run @swift_package//:update` and
`bazel run @swift_package//:resolve` replace `swift package update` and
`swift package resolve`, using the Swift toolchain Bazel is configured with.

For more examples, check `rules_swift_package_manager`'s [examples](https://github.com/cgrindel/rules_swift_package_manager/tree/main/examples)
