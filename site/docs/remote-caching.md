...
make builds significantly faster. Users have reported build time decreases
between 50% and 90%.
...
* configure the Bazel build to use the remote cache use Bazel version 0.10.0 or later
...
* only read from the remote cache, or not
* use the remote cache at all.
...
You need to set up a server to act as the cache’s backend. Any HTTP/1.1
...
You can set up any HTTP/1.1 server as the cache’s backend. Users have
reported success with caching backends such as [Hazelcast], [Apache httpd],
and [AWS S3].
...
only your CI system to have access to write to the remote cache.
...
Use the following flags to: read from the remote cache disable sandboxing
...
Deleting content from the remote cache is part of management of your server.
