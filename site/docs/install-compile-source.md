...
4.  Build Bazel using Bazel: `bazel build //src:bazel`
    (or `bazel build //src:bazel.exe` on Windows)

5.  The resulting binary is at `bazel-bin/src/bazel`
    (or `bazel-bin\src\bazel.exe` on Windows). You can copy it wherever you
...
If you want to build Bazel at an earlier commit than HEAD, you'll need the Bazel
release that was the latest one when the commit was merged. (Other Bazel
versions may work too, but are not guaranteed to.) You can either download this
version from [GitHub](https://github.com/bazelbuild/bazel/releases), or build it
from source, or bootstrap it as described below.
...
**Goal**: Run Bazel to build a custom Bazel binary (`bazel-bin\src\bazel.exe`).
...
4.  The output will be at `bazel-bin\src\bazel.exe`
...
**To build a development version** of Bazel from a GitHub source tree, you need
a working Bazel binary. [Scroll up](#build-bazel-using-bazel) to see how to
