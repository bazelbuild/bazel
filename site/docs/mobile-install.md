...
[Split .apks](http://tools.android.com/tech-docs/new-build-system/user-guide/apk-splits)
will make most of the logic above obsolete, because they allow Bazel to
distribute the code of the app into multiple .apk files that can be re-installed
independently of each other.

Android L already supports split .apks, but in L, the whole application is
recompiled on-device if a single .dex file changes, which makes most of the
optimizations of `bazel mobile-install` useless.

With Android M, only the changed .dex file will be recompiled, which means that
the complete mechanism above can be replaced by splitting the app into multiple
split .apks and installing only those that changed:

 - An .apk containing AndroidManifest.xml
 - An .apk containing native code
 - An .apk containing Android resources
 - An .apk containing Java resources
 - An .apk for each .dex shard

This will obviate the need for the `--incremental` command line option, and
remove the limitations stemming from having to manipulate the internals of the
Android framework.

This is currently implemented in Bazel behind the `--split_apks` command line
option.
