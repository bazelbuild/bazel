...
Bazel writes a JSON profile by default to the file `command.profile.gz` in the
output base. The location can be configured using the `--profile` flag, for
...
We recommend to use the keyboard controls:

*   Press `1` to switch to "select" mode. In this mode, you can select
...
*   Press `2` to switch to "pan" mode. Then drag the mouse to move the view. You
    can also use `a`/`d` to move left/right.
*   Press `3` to switch to "zoom" mode. Then drag the mouse to zoom. You can
    also use `w`/`s` to zoom in/out.
*   Press `4` to switch to "timing" mode where you can measure the distance
    between two events.
...
Each other row represents one of the Bazel threads and shows all events on that
thread.
...
```json
{
  ...
  "traceEvents": [
    ...
    {"cat":"build phase marker","name":"Launch Blaze","ph":"X","ts":-1824000,"dur":1824000,"pid":1,"tid":60},
    ...
