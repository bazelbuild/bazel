...
## Running executables with "bazel run"

Note: `bazel run` will execute the target even if a post-build task, such as uploading to the Build Event Service, fails. The command will still report a non-zero exit code to indicate the task failure.
