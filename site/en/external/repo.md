...
The cache key for a repository rule now includes the host operating system (`os.name`) and architecture (`os.arch`). This means that repository rules will be re-fetched when building on a different platform (e.g., switching from Linux to macOS), ensuring platform-specific logic is correctly executed.
