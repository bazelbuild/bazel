...
Note: The legacy flags `--ios_simulator_version` and `--ios_simulator_device` have been removed. Use `--apple_simulator_device` and `--apple_simulator_version` to target a specific simulator.
