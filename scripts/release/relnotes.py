...
def extract_relnotes(commit_message_lines, major_release):
...
def get_relnotes_between(base, head, major_release):
  ...
  for commit in commits:
    ...
    if m is not None:
      ...
      continue
    relnote = extract_relnotes(lines, major_release)
...
def get_label(issue_id):
  ...
  # auth=os.system(
      ...
      # --keyring buildkite --key github-trusted-token \
...
def get_categorized_relnotes(filtered_notes):
  ...
  return dict(sorted(categorized_relnotes.items(), key=lambda x: x))
...
if __name__ == "__main__":
  ...
  # e.g. if current_release is 5.3.3, last_release should be 5.3.2 not 6.1.1
  current_release = git("rev-parse", "--abbrev-ref", "HEAD")
  current_release = re.sub(r"rc\d", "", current_release[0].split("-")[-1])

  major = False
  if re.match(r"\d+.0.0", current_release):
    major = True
  ...
  cur_release_relnotes = get_relnotes_between(merge_base, "HEAD", major)
  last_release_relnotes = set(
      get_relnotes_between(merge_base, last_release, major)
  )
  filtered_relnotes = [
