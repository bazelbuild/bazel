...
class BazelCompletionWriter(object):
    ...
    def _get_param_types(self):
        """Generates a mapping from Bazel commands to expected param types.

        Parses completion help text provided by the Bazel executable to derive a
        parameter type string for each Bazel command.

        Returns:
            (:obj:`dict` of :obj:`str` to :obj:`str`): Mapping of Bazel commands
                to their corresponding parameter types.
        """
        param_types = {}
    ...
    def _get_subcommands(self):
        ...
        for sub in Arg.generate_from_help(
                ...
                block.group(1),
                is_subcommand=True):
            sub.sub_opts = self._get_options_from_bazel(
                ('help', sub.name), expected_subcommand=sub.name)
    ...
    def _get_params(self, subcommand):
        ...
        if param_type.startswith('label'):
            ...
            params.append("({} query -k '{}' 2>/dev/null)".format(
                _BAZEL, query))
...
class Arg(object):
    ...
    Attributes:
        ...
        expected_subcommand: Nullable string containing a subcommand that this
            option must follow. Defaults to None, indicating that this option
            or subcommand must not follow another subcommand.
    ...
    def _write_params_completion(self, output_file, command=_BAZEL):
        ...
        if self.sub_params:
            args.extend(
                ('-fa',
                 '"{}"'.format(self._escape(' '.join(self.sub_params)))))
        output_file.write(self._complete(args))
    ...
    def _get_complete_args_base(command, subcommand=None):
        ...
        args.append('-n')
        if subcommand:
            args.append('"{} {}"'.format(_FISH_SEEN_SUBCOMMAND_FROM,
                                         subcommand))
        else:
