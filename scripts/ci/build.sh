...
if ! command -v gcloud &>/dev/null; then
  echo "Required tool 'gcloud' not found. Please install it:"
...
# It requires to have gcloud storage installed.
# This methods expects the following arguments:
...
function release_to_gcs() {
  ...
  if [ -n "${release_name}" ]; then
    ...
    gcloud storage cp "${artifact_dir}/**" "gs://bazel/${release_path}"
    # Set the content type on index.html so it isn't autodetected incorrectly by the browser.
    gcloud storage objects update "gs://bazel/${release_path}/index.html" --content-type="text/html; charset=utf-8"

    if [[ "$(is_rolling_release)" -eq 1 ]] && [[ -z "${rc}" ]]; then
      gcloud storage cp "gs://bazel/rolling.html" "${artifact_dir}"
      ...
      gcloud storage cp "${artifact_dir}/rolling.html" "gs://bazel/rolling.html"
    fi
  fi
}
