...
_bazel__rc_files() {
  local workspace="$1" new_files=() processed_files=() rc_file imported file seen
  ...
  local bazelrc_pattern="--bazelrc=(\"[^\"]*\"|'[^']*'|[^[:space:]]+)"
  while [[ "$remaining" =~ $bazelrc_pattern ]]; do
    rc_file=${BASH_REMATCH[1]}
    remaining=${remaining#*"${BASH_REMATCH[0]}"}
    case "$rc_file" in
      \"*\"|\'*\') rc_file=${rc_file:1:${#rc_file}-2} ;;
    esac
    if [ -f "$rc_file" ]; then
  ...
  while (( ${#new_files[@]} > 0 )); do
    ...
    while IFS= read -r imported; do
      new_files+=("$imported")
    done < <(_bazel__rc_imports "$workspace" "$rc_file")
  done
...
_bazel__all_configs() {
  local workspace="$1" command="$2" rc_file
  ...
  while IFS= read -r rc_file; do
    cat "$rc_file"
  done < <(_bazel__rc_files "$workspace") \
      | sed 's/#.*//' \
