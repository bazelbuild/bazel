...
static vector<string> GetServerExeArgs(const blaze_util::Path &jvm_path,
  ...
  if (startup_options.use_remote_repo_contents_cache) {
    result.push_back("--experimental_use_remote_repo_contents_cache");
  } else {
    result.push_back("--noexperimental_use_remote_repo_contents_cache");
  }
