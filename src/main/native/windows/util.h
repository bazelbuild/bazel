...
//
//   The result will never have a UNC prefix. The result won't have a trailing
//   "\\" except when and only when the path is normalized to just a drive
//   specifier (e.g. when `path` is "c:/" or "c:/foo/.."). The result will
//   preserve the casing of the input, so "D:/Bar" becomes "D:\\Bar".
std::string Normalize(const std::string& p);

wstring Normalize(const wstring& p);
