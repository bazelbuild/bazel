...
int GetFdFromFileDescriptor(JNIEnv* env, jobject fdobj) {
  ...
  return env->GetIntField(fdobj, fd_field);
}

}  // namespace

// Non-blockingly drains all bytes currently readable from the fifo fd, returning the count.
// FileInputStream.available()/ioctl(FIONREAD) return 0 for fifos on macOS, so we count jobserver
// tokens by reading them out (the caller rewrites the ones to keep). O_NONBLOCK is toggled on for
// the drain and restored, so the fd stays blocking for the writer that shares it.
extern "C" JNIEXPORT jint JNICALL
Java_com_google_devtools_build_lib_unix_NativePosixFilesServiceImpl_drainFifoNonBlocking(
    JNIEnv* env, jobject instance, jobject fdobj) {
  int fd = GetFdFromFileDescriptor(env, fdobj);
  if (env->ExceptionOccurred()) {
    return -1;
  }
  if (fd < 0) {
    PostException(env, EINVAL, "drainFifoNonBlocking");
    return -1;
  }
  int flags = fcntl(fd, F_GETFL);
  if (flags == -1) {
    POST_EXCEPTION_FROM_ERRNO(env, errno, "drainFifoNonBlocking");
    return -1;
  }
  bool added_nonblock = (flags & O_NONBLOCK) == 0;
  if (added_nonblock && fcntl(fd, F_SETFL, flags | O_NONBLOCK) == -1) {
    POST_EXCEPTION_FROM_ERRNO(env, errno, "drainFifoNonBlocking");
    return -1;
  }
  int count = 0;
  ...
  while (true) {
    ssize_t n;
    RESTARTABLE(read(fd, buf, sizeof buf), n);
    if (n > 0) {
    ...
    POST_EXCEPTION_FROM_ERRNO(env, read_errno, "drainFifoNonBlocking");
    return -1;
  }
  if (added_nonblock && fcntl(fd, F_SETFL, flags) == -1) {
    POST_EXCEPTION_FROM_ERRNO(env, errno, "drainFifoNonBlocking");
    return -1;
  }
  return count;
