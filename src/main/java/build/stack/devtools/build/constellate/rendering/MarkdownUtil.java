...
public final class MarkdownUtil {
  ...
  /**
   * Return a string that formats the input string so it is displayable in a
   * markdown table cell.
   ...
   * <li>Transforms 'new paraphgraph' patterns (two or more sequential newline
   * characters) into
   * line break HTML tags.
   * <li>Turns lingering new line tags into spaces (as they generally indicate
   * intended line wrap.
   * </ul>
   */
   ...
   *
   * <p>
   * For example: 'Information with <brackets>.' becomes 'Information with
   * &lt;brackets&gt;'.
   */
  ...
  /**
   * Return a string representing the rule summary for the given rule with the
   * given name.
   *
  ...
  public String ruleSummary(String ruleName, RuleInfo ruleInfo) {
    List<String> attributeNames = ruleInfo.getAttributeList().stream()
  ...
  /**
   * Return a string representing the summary for the given provider with the
   * given name.
   *
  ...
  public String providerSummary(String providerName, ProviderInfo providerInfo) {
    List<String> fieldNames = providerInfo.getFieldInfoList().stream()
  ...
  /**
   * Return a string representing the aspect summary for the given aspect with the
   * given name.
   *
   * <p>
   * For example: 'my_aspect(foo, bar)'. The summary will contain hyperlinks for
   * each attribute.
   */
  @SuppressWarnings("unused") // Used by markdown template.
  public String aspectSummary(String aspectName, AspectInfo aspectInfo) {
    List<String> attributeNames = aspectInfo.getAttributeList().stream()
        .map(attr -> attr.getName())
        .collect(Collectors.toList());
   ...
   *
   * <p>
   * For example: 'my_func(foo, bar)'. The summary will contain hyperlinks for
   * each parameter.
   */
  @SuppressWarnings("unused") // Used by markdown template.
  public String funcSummary(StarlarkFunctionInfo funcInfo) {
    List<String> paramNames = funcInfo.getParameterList().stream()
  ...
  /**
   * Returns a string describing the given attribute's type. The description
   * consists of a hyperlink
  ...
  /**
   * Returns "required" if providing a value for this parameter is mandatory.
   * Otherwise, returns
   * "optional".
   */
  ...
  /**
   * Return a string explaining what providers an attribute requires. Adds
   * hyperlinks to providers.
   */
