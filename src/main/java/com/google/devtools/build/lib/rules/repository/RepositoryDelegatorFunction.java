...
public final class RepositoryDelegatorFunction implements SkyFunction {
  ...
  public SkyValue compute(SkyKey skyKey, Environment env)
    ...
    try (SilentCloseable c =
      ...
      ImmutableSortedMap<RepoRecordedInput.EnvVar, Optional<String>> environValues;
      if (environAttr != null && environAttr.iterator().hasNext()) {
        var rawEnvVarValues =
            RepositoryFunction.getEnvVarValues(env, ImmutableSet.copyOf(environAttr));
        if (rawEnvVarValues == null) {
          return null;
        }
        environValues = RepoRecordedInput.EnvVar.wrap(rawEnvVarValues);
      } else {
        environValues = ImmutableSortedMap.of();
      }

      DigestWriter digestWriter =
          new DigestWriter(
              directories, repositoryName, rule, starlarkSemantics, environValues);
  ...
  private static class DigestWriter {
    ...
    DigestWriter(
      ...
      predeclaredInputHash =
          computePredeclaredInputHash(rule, starlarkSemantics, environValues);
    ...
    static String computePredeclaredInputHash(
      ...
      Fingerprint fp =
          new Fingerprint()
              ...
              .addInt(environValues.size());
      environValues.forEach(
          (envVar, value) -> {
            fp.addString(envVar.toString());
            fp.addNullableString(value.orElse(null));
          });
