...
class BuildOutputFormatterCallback extends CqueryThreadsafeCallback {
  ...
  private ConfiguredAttributeMapper getAttributeMap(KeyedConfiguredTarget ct)
      throws InterruptedException {
    Rule associatedRule = accessor.getTarget(ct).getAssociatedRule();
    if (associatedRule == null) {
      return null;
    } else if (ct.configuredTarget() instanceof AliasConfiguredTarget) {
      return ConfiguredAttributeMapper.of(
          associatedRule, ((AliasConfiguredTarget) ct.configuredTarget()).getConfigConditions());
    } else if (ct.configuredTarget() instanceof OutputFileConfiguredTarget) {
      return ConfiguredAttributeMapper.of(
          associatedRule,
          accessor
              .getGeneratingConfiguredTarget((OutputFileConfiguredTarget) ct.configuredTarget())
              .getConfigConditions());
    } else {
      Verify.verify(ct.configuredTarget() instanceof RuleConfiguredTarget);
      return ConfiguredAttributeMapper.of(
          associatedRule, ((RuleConfiguredTarget) ct.configuredTarget()).getConfigConditions());
    }
  }
