...
public final class StrippingPathMapper {
  ...
  private static class StringStripper {
     ...
     *
     * <p>Doesn't strip paths that would be non-existent without config prefixes. For example,
     * these are unchanged: "bazel-out/x86-fastbuild", "bazel-out;foo", "/path/to/compiler
     * bazel-out".
     *
   ...
   * action is explicitly supported (see {@link PathMappers#SUPPORTED_MNEMONICS) and b) it's safe to
