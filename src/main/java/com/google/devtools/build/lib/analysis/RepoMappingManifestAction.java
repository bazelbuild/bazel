...
public final class RepoMappingManifestAction extends AbstractFileWriteAction
  ...
  public DeterministicWriter newDeterministicWriter() {
    return out -> {
      ...
      var sortedRepoMappings = transitivePackages.toList().stream()
          .collect(
      ...
      // map instance.
      IdentityHashMap<ImmutableMap<String, RepositoryName>, ImmutableList<Entry<String, RepositoryName>>> cachedRelevantEntries = new IdentityHashMap<>();
      for (var repoAndMapping : sortedRepoMappings.entrySet()) {
        cachedRelevantEntries
            .computeIfAbsent(repoAndMapping.getValue().entries(),
            ...
            .forEach(mappingEntry -> {
              ...
              String targetRepoDirectoryName =
                  mappingEntry.getValue().isMain() ? workspaceName
                      : mappingEntry.getValue().getName();
              writer.format("%s,%s,%s\n", repoAndMapping.getKey().getName(), mappingEntry.getKey(),
