...
public final class FilesetEntryFunction implements SkyFunction {
  ...
  public FilesetEntryFunction(Function<String, Path> getExecRoot) {
    this.getExecRoot = getExecRoot::apply;
  }
  ...
  private void maybeStoreSymlink(
    ...
    if (!result.containsKey(linkName)) {
      result.put(
          linkName,
          FilesetOutputSymlink.create(linkName, linkTarget, metadata, isGenerated,
              execRoot.asFragment()));
    }
  }
  ...
  /**
   * Returns the {@link TraversalRequest} node used to compute the
   ...
   */
  public static TraversalRequest getDependencyForRewinding(
