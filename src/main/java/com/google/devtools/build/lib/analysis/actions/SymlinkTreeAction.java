...
public final class SymlinkTreeAction extends AbstractAction {
   ...
   * @param outputManifest      the generated symlink tree manifest (must have "MANIFEST" base
   *                            name). Symlink tree root will be set to the artifact's parent
   *                            directory.
  ...
  /**
   * Creates SymlinkTreeAction instance. Prefer the constructor that takes a
   * {@link BuildConfigurationValue} instance; it is less likely to require changes in the future if
   * we add more command-line flags that affect this action.
   *
   * @param owner               action owner
   * @param inputManifest       the input runfiles manifest
   * @param runfiles            the input runfiles
