...
final class SandboxBackendManifest {
   ...
   * this map.
   *
   * <p>In a built tree the only {@code byte[]} blobs are directories — files and virtual inputs are
   * retained as {@link com.google.devtools.build.lib.actions.ActionInput}s and fetched by path, not
   * shipped here — so this is exactly the set of directories in the tree.
   */
  static Map<String, ByteString> directoryBlobs(MerkleTree.Uploadable built) {
