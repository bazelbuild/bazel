  ...
  private CacheEntry.Final fetchFinalCacheEntry(
    ...
    while (!currentHashes.isEmpty()) {
      ...
      for (var hash : currentHashes) {
        switch (fetchCacheEntry(env, context, hash, state)) {
    ...
    if (nextInputBatches == null) {
      ...
      nextInputBatches =
          stdoutFuture
              ...
              .collect(toImmutableList());
      // Cache fetched next input batches to avoid repeated downloads of the same intermediate
      // action result due to restarts.
