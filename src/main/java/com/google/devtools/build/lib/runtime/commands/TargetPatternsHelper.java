...
public final class TargetPatternsHelper {
  ...
  public static List<String> readFrom(CommandEnvironment env, OptionsParsingResult options)
    ...
    boolean hasQuery = !buildRequestOptions.getTargetQuery().isEmpty();
    boolean hasQueryFile = !buildRequestOptions.getTargetQueryFile().isEmpty();
    boolean hasTargetPatternFile = !buildRequestOptions.getTargetPatternFile().isEmpty();
    ...
    if (hasQuery || hasQueryFile) {
      ...
      if (hasQuery) {
        ...
        Path queryFilePath =
            env.getWorkingDirectory().getRelative(buildRequestOptions.getTargetQueryFile());
        try {
          env.getEventBus()
              .post(InputFileEvent.create(/* type= */ "query_file", queryFilePath.getFileSize()));
    ...
    if (!buildRequestOptions.getTargetPatternFile().isEmpty()) {
  ...
  private static ImmutableList<String> executeQuery(
    ...
    try {
      ...
      int loadingPhaseThreads = threadsOption.getThreads();
      var repoMapping =
          env.getSkyframeExecutor()
              .getMainRepoMapping(false, loadingPhaseThreads, env.getReporter());
      ...
      var starlarkSemantics =
          env.getSkyframeExecutor()
              .getEffectiveStarlarkSemantics(options.getOptions(BuildLanguageOptions.class));
      ...
      if (!result.getSuccess()) {
        throw new TargetPatternsHelperException(
            "Query evaluation failed", TargetPatterns.Code.TARGET_PATTERNS_UNKNOWN);
      }
      ...
      throw new TargetPatternsHelperException(
          e.getMessage(), TargetPatterns.Code.TARGET_PATTERNS_UNKNOWN);
    } catch (QuerySyntaxException e) {
      throw new TargetPatternsHelperException(
          "Query syntax error: " + e.getMessage(), TargetPatterns.Code.TARGET_PATTERNS_UNKNOWN);
    }
  }
}
