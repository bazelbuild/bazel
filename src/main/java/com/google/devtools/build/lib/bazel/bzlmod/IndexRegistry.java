...
public class IndexRegistry implements Registry {
  ...
  private record GitSchemeEntry(String template, String friendlyName, BiFunction<String, String, Boolean> validator) {}
  ...
  private static final Pattern GIT_SSH_SCHEME_REGEX = Pattern.compile("^git@[\\w.-]+(?::[/.\\w_-]+)?$");
  ...
  private RepoSpec createGitRepoSpec(
    ...
    if (ALLOWED_GIT_SCHEMES.stream().noneMatch((schema) -> schema.validator.apply(schema.template, sourceJson.remote))) {
      throw new IOException(
          String.format(
  ...
  /** Validates that a given URI-like git repo spec is a valid URI, returning true if valid and false otherwise. */
  ...
  /** Validates that the given URI-like git repo spec is a valid git SSH remote, returning true if valid and false otherwise. */
