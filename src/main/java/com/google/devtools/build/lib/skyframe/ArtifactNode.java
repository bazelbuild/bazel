...
/**
 * A node representing an artifact. Source artifacts are checked for existence, while output
 * artifacts imply creation of the output file.
 *
 * <p>There are effectively three kinds of output artifact nodes. The first corresponds to an
 * ordinary artifact {@link FileArtifactNode}. It stores the relevant data for the artifact --
 * digest/mtime and size. The second corresponds to an "aggregating" artifact -- the output of an
 * aggregating middleman action. It stores the relevant data of all its inputs. The third represents
 * any other middleman artifact, and is just a dummy node.
 */
@Immutable
@ThreadSafe
public abstract class ArtifactNode implements Node {

  @ThreadSafe
  static NodeKey key(Artifact artifact, boolean isMandatory) {
    return new NodeKey(NodeTypes.ARTIFACT, new OwnedArtifact(artifact, isMandatory));
  }

  private static final Function<Artifact, NodeKey> TO_MANDATORY_KEY =
      new Function<Artifact, NodeKey>() {
        @Override
        public NodeKey apply(Artifact artifact) {
          return key(artifact, true);
        }
      };

  @ThreadSafe
  public static Iterable<NodeKey> mandatoryKeys(Iterable<Artifact> artifacts) {
  ...
  public static Artifact artifact(NodeKey key) {
    return TO_ARTIFACT.apply((OwnedArtifact) key.getNodeName());
  }

  private static class MiddlemanNode extends ArtifactNode {}
  private static class MissingFileNode extends ArtifactNode {}

  /**
   * Returns an empty node. Note that we must return a new instance each call, to prevent change
   * pruning from stopping at middleman artifacts.
   */
  static ArtifactNode newEmptyNode() {
    return new MiddlemanNode();
  }

  /**
   * Returns a missing node for a non-mandatory input Artifact.
   */
  static ArtifactNode newMissingNode() {
    return new MissingFileNode();
  }
   ...
   * will result in the node entry for foo.out under configurationA being returned. This would
   * prevent caching the graph in different configurations, and also causes big problems with change
   * pruning, which assumes the invariant that a node's first dependency will always be the same. In
   * this case, the node entry's old dependency on //foo:foo in configurationA would cause it to
  ...
  public static class OwnedArtifact {
    ...
    private OwnedArtifact(Artifact artifact, boolean mandatory) {
