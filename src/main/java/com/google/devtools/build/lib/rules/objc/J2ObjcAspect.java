...
public class J2ObjcAspect extends NativeAspectClass implements ConfiguredAspectFactory {
    ...
    return ConfigAwareAspectBuilder.of(new AspectDefinition.Builder(this))
        ...
        .add(
            attr(J2OBJC_PROTO_TOOLCHAIN_ATTR, LABEL)
                ...
                .value(getProtoToolchainLabel(toolsRepository + "//tools/j2objc:j2objc_proto_toolchain")))
  ...
  private ConfiguredAspect proto(
    ...
    ProtoLangToolchainProvider protoToolchain = ruleContext.getPrerequisite(
    ...
    ProtoSourceFileBlacklist protoBlacklist =
        new ProtoSourceFileBlacklist(
            ruleContext,
            protoToolchain.blacklistedProtos().toList());
    ...
    if (Iterables.isEmpty(j2ObjcSource.getObjcSrcs())) {
      ...
      directJ2ObjcMappingFileProvider =
          createJ2ObjcProtoCompileActions(base, protoToolchain, ruleContext, filteredProtoSources, j2ObjcSource);
    }
  ...
  static ObjcCommon common(RuleContext ruleContext, Iterable<Artifact> transpiledSources,
      ...
      List<TransitiveInfoCollection> otherObjcProviders) throws InterruptedException {
    ...
    List<ObjcProvider> newOtherDeps = otherObjcProviders
        .stream()
        .map(d -> d.get(ObjcProvider.SKYLARK_CONSTRUCTOR))
        .collect(Collectors.toList());
    // We can't just use addDeps since that now takes ConfiguredTargetAndTargets and we only have TransitiveInfoCollections
    builder.addDepObjcProviders(newOtherDeps);
    builder.addRuntimeDeps(otherObjcProviders);
