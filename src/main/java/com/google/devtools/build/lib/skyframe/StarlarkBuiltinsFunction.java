...
public class StarlarkBuiltinsFunction implements SkyFunction {
  ...
  private static StarlarkBuiltinsValue computeInternal(
    ...
    if (isWithAutoloads) {
      ImmutableList<String> symbolsToLoad = starlarkSemantics.get(
              BuildLanguageOptions.INCOMPATIBLE_LOAD_SYMBOLS_EXTERNALLY).stream()
          .filter(s -> !s.startsWith("-")).collect(
              toImmutableList());
      ImmutableList<String> rulesToLoad = starlarkSemantics.get(
              BuildLanguageOptions.INCOMPATIBLE_LOAD_RULES_EXTERNALLY).stream()
          .filter(s -> !s.startsWith("-")).collect(
              toImmutableList());
      ...
      for (String symbol : symbolsToLoad) {
        ...
        loadKeysBuilder.add(BzlLoadValue.keyForBuildAutoloads(Label.parseCanonicalUnchecked(
      ...
      try {
        if (inliningState == null) {
          loadValues = BzlLoadFunction.computeBzlLoadsWithSkyframe(env, loadKeysBuilder.build(),
              additionalLoads.build());
        } else {
          loadValues = bzlLoadFunction.computeBzlLoadsWithInlining(env, loadKeysBuilder.build(),
              additionalLoads.build(), inliningState);
        }
        if (loadValues == null) {
          return null;
        }
      } catch (BzlLoadFailedException ex) {
        throw new BuiltinsFailedException(String.format(
            "There's no load specification for toplevel symbol or rule set by --incompatible_load_symbols_externally or --incompatible_load_rules_externally. Most likely a typo."),
            ex, Transience.PERSISTENT);
      }

      Fingerprint fp = new Fingerprint();
      fp.addBytes(transitiveDigest);

      String workspaceWarning = starlarkSemantics.getBool(BuildLanguageOptions.ENABLE_BZLMOD) ? "" :
          " Most likely you need to upgrade the version of rules repository providing it your WORKSPACE file.";

      // Substitute-in loaded rules and symbols (that were removed from Bazel)
      LinkedHashMap<String, Object> exportedToplevelsBuilder = new LinkedHashMap<>(
          exportedToplevels);
      int i = 0;
      for (String symbol : symbolsToLoad) {
        ...
        if (v.getModule().getGlobal(symbol) == null) {
          throw new BuiltinsFailedException(String.format(
              "The toplevel symbol '%s' set by --incompatible_load_symbols_externally couldn't be loaded.%s",
              symbol, workspaceWarning),
              null, Transience.PERSISTENT);
        }
