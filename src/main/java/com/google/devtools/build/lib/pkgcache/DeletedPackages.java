...
public record DeletedPackages(
  ...
  public record Pattern(PackageIdentifier id, boolean subtree) {
    ...
    public static Pattern parse(String input) throws LabelSyntaxException {
      if (input.equals("...")) {
        return new Pattern(PackageIdentifier.createInMainRepo(""), /* subtree= */ true);
      }
      if (!input.endsWith("/...")) {
        return new Pattern(PackageIdentifier.parse(input), /* subtree= */ false);
      }
      String pkg = input.substring(0, input.length() - "/...".length());
      if (pkg.isEmpty()) {
        throw new LabelSyntaxException("invalid package pattern '" + input + "'");
      }
      if (pkg.endsWith("/")) {
        // "//..." or "@repo//...": restore the second slash so the repo root is parsed.
        pkg += "/";
      }
      return new Pattern(PackageIdentifier.parse(pkg), /* subtree= */ true);
    }
  }
