...
public class BuildEventServiceTransport implements BuildEventTransport {
  ...
  public TransportKind kind() {
    return besClient.transportKind();
  }
  ...
  private synchronized ListenableFuture<Void> close(boolean now) {
    ...
    uploaderExecutorService.execute(
        () -> {
          try {
            sendOrderedBuildEvent(new LastOrderedBuildEvent(besProtoUtil.nextSequenceNumber()));
  ...
  public synchronized void sendBuildEvent(BuildEvent event, final ArtifactGroupNamer namer) {
    ...
    sendOrderedBuildEvent(
        new DefaultOrderedBuildEvent(event, namer, besProtoUtil.nextSequenceNumber()));
  }
  ...
  private synchronized void sendOrderedBuildEvent(OrderedBuildEvent evtAndNamer) {
  ...
  private void publishEventStream(
    ...
    try {
      @Nullable OrderedBuildEvent orderedBuildEvent;
      do {
        ...
        if (orderedBuildEvent != null) {
          ...
          try {
            pathConverter = artifactUploader.upload(orderedBuildEvent.referencedArtifacts());
          } catch (IOException e) {
            logger.log(
                Level.WARNING,
                String.format(
  ...
  private Function<PublishBuildToolEventStreamResponse, Void> ackCallback(
    ...
    return ack -> {
      ...
      OrderedBuildEvent event = pendingAck.removeFirst();
  ...
  private class DefaultOrderedBuildEvent implements OrderedBuildEvent {
    ...
    public Set<Path> referencedArtifacts() {
      return event.referencedArtifacts();
    }
  ...
  private class LastOrderedBuildEvent implements OrderedBuildEvent {

    private final int sequenceNumber;

    LastOrderedBuildEvent(int sequenceNumber) {
      this.sequenceNumber = sequenceNumber;
    }
