...
public class TerminalTestResultNotifier implements TestResultNotifier {
  ...
  private void addToWarningList(
    ...
    addToList(list, AnsiTerminalPrinter.Mode.WARNING, singularPrefix, pluralPrefix, message,
        count);
  }

  private void addToList(
      List<String> list, AnsiTerminalPrinter.Mode mode, String singularPrefix, String pluralPrefix,
      String message, int count) {
    if (count > 0) {
      list.add(String.format("%s%d %s %s%s", mode, count,
