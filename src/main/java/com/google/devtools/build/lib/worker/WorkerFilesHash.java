  ...
  public static HashCode getCombinedHash(SortedMap<PathFragment, byte[]> workerFilesMap) {
    ...
    workerFilesMap.forEach(
        (execPath, digest) -> {
          ...
          // Kept around to preserve the hash generated by Bazel 8.4.1 and earlier.
          hasher.putByte(/* String.LATIN1 */ (byte) 0);
          hasher.putInt(execPathString.length());
          hasher.putBytes(StringUnsafe.getInternalStringBytes(execPathString));
