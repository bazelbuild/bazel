            ...
            if (ret instanceof String) {
              ...
              for (Object v : ((Sequence) ret)) {
                if (!(v instanceof String)) {
                  throw Starlark.errorf("Function provided to map_each must return string, None, "
                      + "or list of strings, but returned list containing element '%s' of type %s "
                      + "for key '%s' and value: %s", v, Starlark.type(v), getKey(), val);
                }
                parts.add((String) v);
              }
              ...
              throw Starlark.errorf(
                  "Function provided to map_each must return string, None, or list of strings, but "
                      + "returned type %s for key '%s' and value: %s", Starlark.type(ret), getKey(),
                  val);
            }
