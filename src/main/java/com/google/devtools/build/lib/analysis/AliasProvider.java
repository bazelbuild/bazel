...
public final class AliasProvider implements TransitiveInfoProvider {
  ...
  public static String describeTargetWithAliases(
    ...
    return describeTargetWithAliases(target, targetMode, /* mainRepositoryMapping= */ null);
  }
   ...
   * @param target the target to describe
   * @param targetMode how to express the kind of the target
   * @param mainRepositoryMapping the repository mapping to use for displaying labels, or null
   */
  public static String describeTargetWithAliases(
    ...
    if (aliasProvider == null) {
      return kind + " '" + target.getTargetLabel().getDisplayForm(mainRepositoryMapping) + "'";
    }
    ...
    result
        .append("alias '")
        .append(aliasChain.get(0).getDisplayForm(mainRepositoryMapping))
        .append("'");
    result
        .append(" referring to ")
        .append(kind)
        .append(" '")
        .append(target.getTargetLabel().getDisplayForm(mainRepositoryMapping))
        .append("'");
    if (aliasChain.size() > 1) {
      result
          ...
          .append(
              aliasChain.stream()
                  .skip(1)
                  .map(label -> label.getDisplayForm(mainRepositoryMapping))
                  .collect(joining("' -> '")))
          .append("'");
    }
