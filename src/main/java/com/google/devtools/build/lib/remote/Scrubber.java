...
public class Scrubber {
  ...
  /**
   * Constructs a {@link Scrubber} from the given configuration file, which must contain a
   * {@link Config} protocol buffer in text format.
   */
  ...
  public static class SpawnScrubber {
    ...
    private SpawnScrubber(Config.Rule ruleProto) {
      ...
      this.omittedInputPatterns = transformProto.getOmittedInputsList().stream()
          .map(s -> Pattern.compile(s)).collect(toImmutableList());
      this.argReplacements = transformProto.getArgReplacementsList().stream()
    ...
    private boolean matches(Spawn spawn) {
      ...
      return (!isForTool || matchTools) && mnemonicPattern.matcher(mnemonic).matches()
