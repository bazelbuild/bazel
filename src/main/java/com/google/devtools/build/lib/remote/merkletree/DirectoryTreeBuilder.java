...
class DirectoryTreeBuilder {
   ...
   *
   * <p>Prefer {@link #buildFromActionInputs} if this Merkle tree is for an action spawn (as
   * opposed to repository fetching).</p>
   *
  ...
  private static <T> int build(
    ...
    for (Map.Entry<PathFragment, T> e : inputs.entrySet()) {
      ...
      if (scrubber != null && input instanceof ActionInput && scrubber.shouldOmitInput(
          (ActionInput) input)) {
        continue;
      }
