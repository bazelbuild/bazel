    ...
    for (Map.Entry<DependencyKind, Label> entry : outgoingLabels.entries()) {
      ...
      if (entry.getKey() == TOOLCHAIN_DEPENDENCY) {
        ...
        if (useToolchainTransition) {
          // We need to create an individual PRD for each distinct toolchain context that depends on this toolchain.
          for (ToolchainContext toolchainContext : toolchainContexts.getContextsForResolvedToolchain(toLabel)) {
