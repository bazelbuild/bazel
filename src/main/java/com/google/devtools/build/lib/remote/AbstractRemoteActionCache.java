...
public abstract class AbstractRemoteActionCache implements AutoCloseable {
  ...
  private List<ListenableFuture<FileMetadata>> downloadOutErr(
    ...
    if (!result.getStdoutRaw().isEmpty()) {
      ...
      downloads.add(Futures.transform(downloadBlob(result.getStdoutDigest(), outErr.getOutputStream()),
          (d) -> null, directExecutor()));
    }
    if (!result.getStderrRaw().isEmpty()) {
      result.getStderrRaw().writeTo(outErr.getErrorStream());
      outErr.getErrorStream().flush();
    } else if (result.hasStderrDigest()) {
      downloads.add(Futures.transform(downloadBlob(result.getStderrDigest(), outErr.getErrorStream()),
          (d) -> null, directExecutor()));
    }
    return downloads;
  }
  ...
  /**
   * Avoids downloading the majority of action outputs but injects their metadata using
   * {@link MetadataInjector} instead.
   *
  ...
  public InMemoryOutput downloadMinimal(
    ...
    try (SilentCloseable c = Profiler.instance().profile("Remote.downloadStdoutStderrAndInline")) {
  ...
  private void injectRemoteArtifact(Artifact output, ActionResultMetadata metadata, Path execRoot,
    ...
    if (output.isTreeArtifact()) {
      ...
      for (FileMetadata file : directory.files()) {
        ...
        RemoteFileArtifactValue r = new RemoteFileArtifactValue(
      ...
      metadataInjector.injectRemoteFile(output, DigestUtil.toBinaryDigest(outputMetadata.digest()),
          outputMetadata.digest().getSizeBytes(), 1);
    }
  }

  private DirectoryMetadata parseDirectory(Path parent, Directory dir,
      Map<Digest, Directory> childDirectoriesMap) {
    ImmutableList.Builder<FileMetadata> filesBuilder = ImmutableList.builder();
    for (FileNode file : dir.getFilesList()) {
      filesBuilder.add(new FileMetadata(parent.getRelative(file.getName()), file.getDigest(),
  ...
  private ActionResultMetadata parseActionResultMetadata(ActionResult actionResult, Path execRoot)
    ...
    for (OutputDirectory dir : actionResult.getOutputDirectoriesList()) {
      dirMetadataDownloads.put(
          ...
          Futures.transform(downloadBlob(dir.getTreeDigest()),
              (treeBytes) -> {
                try {
    ...
    for (OutputFile outputFile : actionResult.getOutputFilesList()) {
      files.put(
          ...
          new FileMetadata(execRoot.getRelative(outputFile.getPath()), outputFile.getDigest(),
