...
public final class BuildConfiguration implements Serializable {
  ...
  private final Root sourceGenDirectory; // for source files generated by annotation processors
  ...
  private final boolean showCachedAnalysisResults;
  ...
  BuildConfiguration(BlazeDirectories directories,
    ...
    this.compilationMode = options.compilationMode;
    this.showCachedAnalysisResults = options.showCachedAnalysisResults;
    ...
    this.sourceGenDirectory = Root.asDerivedRoot(execRoot, outputDir.getRelative("sourcegenfiles"));
    ...
    // Lots of packages in third_party assume that BINMODE expands to
    // either "-dbg", "-opt", or (maybe) "-coverage".  So for backwards
    // compatibility we preserve that invariant, setting BINMODE
    // to "-dbg" rather than "-fastbuild" if the compilation mode is "fastbuild".
    // We put the real compilation mode in a new variable COMPILATION_MODE.
    globalMakeEnvBuilder.put("COMPILATION_MODE", compilationMode.toString());
    globalMakeEnvBuilder.put("BINMODE", "-"
        + ((compilationMode == CompilationMode.FASTBUILD) ? "dbg" : compilationMode.toString()));
    /*
  ...
  /**
   * Returns the execution root used by this Bazel instance.
   */
  // TODO(bazel-team): Avoid using this method - absolute paths shouldn't be used in the analysis
  // phase.
  ...
  /**
   * Returns the directory where source files generated by annotation
   * processors should be stored.
   */
  public Root getSourceGenDirectory() {
   ...
   */
  public boolean showCachedAnalysisResults() {
    return showCachedAnalysisResults;
  }
