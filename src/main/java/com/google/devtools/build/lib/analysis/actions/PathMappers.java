...
public final class PathMappers {
  ...
  private static final ImmutableSet<String> SUPPORTED_MNEMONICS = ImmutableSet.of(
      "AndroidLint",
   ...
   */
  public static void addToFingerprint(String mnemonic, OutputPathsMode outputPathsMode,
   ...
   * {@link com.google.devtools.build.lib.actions.CommandLines#expand(ArtifactExpander, PathFragment, PathMapper, CommandLineLimits)}
   * or any other variants of these functions. The same instance should also be passed to the
   * {@link Spawn} constructor so that the executor can obtain it via {@link Spawn#getPathMapper()}.
   *
   ...
   * #addToFingerprint(String, OutputPathsMode, Fingerprint)} from {@link
   ...
   * @param outputPathsMode the value of {@link CoreOptions#outputPathsMode}
   * @return a {@link PathMapper} that maps paths of the action's inputs and outputs. May be
   *    {@link PathMapper#NOOP} if path mapping is not applicable to the action.
   */
  ...
  private static OutputPathsMode getEffectiveOutputPathsMode(OutputPathsMode outputPathsMode,
      String mnemonic) {
