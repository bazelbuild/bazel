...
public abstract class CcBinary implements RuleConfiguredTargetFactory {
  ...
  public static void init(
    ...
    if (featureConfiguration.isEnabled(CppRuleClasses.COPY_DYNAMIC_LIBRARIES_TO_BINARY)) {
      copiedRuntimeDynamicLibraries =
          createRuntimeDirectoryCopyActions(
              ruleContext,
    ...
    if (debugFileOutputs != null) {
      NestedSetBuilder<Artifact> debugFilesSet = new NestedSetBuilder(Order.STABLE_ORDER);
      if (featureConfiguration.isEnabled(CppRuleClasses.COPY_DYNAMIC_LIBRARIES_TO_BINARY)) {
        NestedSet<Artifact> copiedRuntimeDebugFiles = null;
        copiedRuntimeDebugFiles =
                createRuntimeDirectoryCopyActions(
                        ruleContext,
                        debugFileOutputs);
        debugFilesSet.addTransitive(copiedRuntimeDebugFiles);
      } else {
        debugFilesSet.addAll(debugFileOutputs);
      }
      if (!debugFilesSet.isEmpty()) {
        ruleBuilder.addOutputGroup("debug_files", debugFilesSet.build());
      }
    }
   ...
   *
   * @param artifactsForRuntime The artifacts to be copied.
   * @return The result artifacts of the copies.
   */
  private static NestedSet<Artifact> createRuntimeDirectoryCopyActions(
      RuleContext ruleContext, Iterable<Artifact> artifactsForRuntime) {
    NestedSetBuilder<Artifact> result = NestedSetBuilder.stableOrder();
    for (Artifact artifact : artifactsForRuntime) {
      ...
      if (!ruleContext
              ...
              .equals(artifact.getOwner().getPackageIdentifier())
        ...
        Artifact copy = ruleContext.getBinArtifact(targetSubDir.getRelative(artifact.getFilename()));
        ruleContext.registerAction(
            SymlinkAction.toArtifact(
                ruleContext.getActionOwner(), artifact, copy, "Copying Execution Dynamic Library"));
        result.add(copy);
      } else {
        // If the artifact is already in the same directory as the binary, we don't need to copy it,
