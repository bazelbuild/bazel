 ...
 *
 * Start position for reading is fixed.
 * Mutable state includes the end position, offset in case of escaped symbol,
 * error text if a lexing error occurred, and the flag indicating if zero byte was read.
 * (Zero byte determines the end of the file.)
 *
...
public class NinjaLexerStep {
  ...
  /**
   * For the quick checks outside of skipXXX and tryXXX methods of this class,
   * assume that the step takes just one symbol.
   */
