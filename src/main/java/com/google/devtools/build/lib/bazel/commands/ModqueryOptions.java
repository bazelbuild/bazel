...
public class ModqueryOptions extends OptionsBase {
  ...
  @Option(
      ...
      effectTags = {OptionEffectTag.EXECUTION},
      help =
          "Maximum display depth of the dependency tree. A depth of 1 displays the direct dependencies, for example. For tree, path and all_paths it defaults to Integer.MAX_VALUE, while for explain it defaults to 1 (only displays direct deps of the root besides the leaves and their parents).\n")
  public int depth;
  ...
  public enum QueryType {
    ...
    public static String printValues() {
      return "("
          + Arrays.stream(values()).map(QueryType::toString).collect(Collectors.joining(", "))
          + ")";
    }
  }
