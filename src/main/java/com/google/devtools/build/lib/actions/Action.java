...
public interface Action extends ActionMetadata, Describable {
  ...
  /**
   * Estimates the workload for executing the Action for reporting progress.
   *
   * The default implementation uses the number of input files as an approximation of the
   * workload adjusted by 5 to account for the action itself.
   *
   * @return an estimation of the workload used to report progress.
   */
  ...
  /**
   * Returns true if the output (stdout and stderr) and warnings generated by
   * the action are to be displayed to the user or not.
   */
  boolean shouldShowOutput(ErrorEventListener listener);
