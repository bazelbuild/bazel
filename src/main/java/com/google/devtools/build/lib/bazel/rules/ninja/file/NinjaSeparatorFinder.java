...
public class NinjaSeparatorFinder implements SeparatorFinder {
  ...
  @Override
  public int findNextSeparator(ByteBufferFragment fragment, int startingFrom)
    ...
    for (int i = startingFrom + 1; i < fragment.length() - 1; i++) {
      byte current = fragment.byteAt(i);
      byte next = fragment.byteAt(i + 1);
      byte afterNextOrSpace = i < (fragment.length() - 2) ? fragment.byteAt(i + 2) : SPACE_BYTE;
      ...
      if (!escaped && SPACE_BYTE != afterNextOrSpace && TAB_BYTE != afterNextOrSpace
