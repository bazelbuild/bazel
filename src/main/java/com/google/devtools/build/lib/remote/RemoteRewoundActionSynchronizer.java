...
public final class RemoteRewoundActionSynchronizer implements RewoundActionSynchronizer {
  ...
  // also has a smaller footprint. Its admission of multiple writers is not relied upon: only the
  // action identified by a key ever acquires the write lock of that key.
  ...
  Every lock key identifies an action (see actionKey). By enterActionPreparationForRewinding, only
  a rewound action acquires the write lock of its own key. It does so before it prepares for
  ...
  artifact input, these are the generating actions of its children: the action that generates the
  tree artifact, or the actions expanded from an ActionTemplate that populate it. In either case,
  the reader depends on that action: ActionExecutionFunction requests all inputs, including
  ...
  Thus an action that holds or waits for the read lock of K depends on the action identified by K,
  which alone can acquire the write lock of K. Calls to enterProcessOutputsAndGetLostArtifacts
  acquire read locks by the same rule.
  ...
  */
   ...
   * generated its children. Consumers of the artifact hold the read locks of these keys while
  ...
  private static Iterable<ActionLookupData> lockKeys(
    ...
    // The children of a tree artifact generated by an ordinary action share its key, which also
    // guards a tree artifact without metadata.
    if (tree == null
        || tree.getChildren().isEmpty()
        || tree.getChildren().first().getGeneratingActionKey().equals(ownKey)) {
      return ImmutableList.of(ownKey);
    }
    return Iterables.transform(tree.getChildren(), TreeFileArtifact::getGeneratingActionKey);
  }
