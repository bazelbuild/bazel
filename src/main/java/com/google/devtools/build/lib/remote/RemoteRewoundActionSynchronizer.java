...
public final class RemoteRewoundActionSynchronizer implements RewoundActionSynchronizer {
  ...
  Readers release it before acquiring fine locks, and the writer releases it after switching to fine
  locks.

  For the fine locks, we show that a cycle of lock waits would imply a cycle in the action graph,
  which Bazel disallows. The proof consists of three steps:
  ...
  * If K is an expanded action's own key, the input is a file produced by that action, so the
  ...
  2. Classifying the edges of a possible cycle
  ...
  * WR: A waits for a write lock in enterActionPreparation. If this is its first write lock, it
  ...
  come from a reader in the same group either as by step 1, that reader would depend on every action
