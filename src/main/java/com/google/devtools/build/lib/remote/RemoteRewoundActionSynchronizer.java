...
public final class RemoteRewoundActionSynchronizer implements RewoundActionSynchronizer {
  ...
  1. Relate lock keys to dependencies between templates and actions.
  ...
  Group all actions from the same template expansion together, with each ordinary action forming
  a group of its own. Represent each group by its template expansion or its ordinary action in
  Skyframe. Dependencies between these representatives cannot form a cycle.

  We rely on the ActionTemplate contract: dependencies of expanded actions on artifacts outside
  their expansion are also dependencies of the template. Individual files of input trees are
  covered by the declared tree inputs. This also covers discovered tree-file inputs of C++
  actions, whose possible headers are declared before discovery. Actions within an expansion
  may consume each other's outputs without making those outputs inputs of the template.

  For Starlark templates, this relies on additional_params accepting only scalar values: external
  artifacts must enter through the declared input or tool arguments. StarlarkTemplateContext also
  rejects entire output directories in the expanded action's inputs, including its tools. Thus
  dependencies within an expansion use individual expanded outputs, not the template's trees.

  The readLockKeys and lockKey methods distinguish inputs from the same expansion from all other
  inputs. A reader in the same expansion acquires the generating action's own read lock, so it
  depends on that action. Any other reader acquires the outer key: the template's key for an
  expanded output, or the generating action's key for an ordinary output.

  For a read of an outer key, the reader's group depends on the group identified by that key.
  An ordinary reader depends on the generating action or on the expansion that produces the
  requested tree. For an expanded reader, its template has that dependency by the contract above.
  An action cannot read its own template's key, since that would require an entire output tree
  of its own template as an input, which is excluded above.

  A template may declare several output trees, and consuming one tree need not depend on every
  action in the expansion. All those trees still require the same expansion computation. Thus
  the dependency between groups holds even when a reader and writer access different trees.
