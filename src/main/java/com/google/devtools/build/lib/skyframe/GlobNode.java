...
/**
 * A node corresponding to a glob.
 */
@Immutable
@ThreadSafe
final class GlobNode implements Node {

  static final GlobNode EMPTY = new GlobNode(
      NestedSetBuilder.<PathFragment>emptySet(Order.STABLE_ORDER));

  private final NestedSet<PathFragment> matches;

  GlobNode(NestedSet<PathFragment> matches) {
  ...
  public boolean equals(Object other) {
    ...
    return getMatches().shallowEquals(((GlobNode) other).getMatches());
  }
  ...
  /**
   * Constructs a {@link NodeKey} for a glob lookup. {@code packageName} is assumed to be an
   ...
   */
  @ThreadSafe
  static NodeKey key(PathFragment packageName, String pattern, boolean excludeDirs)
  ...
  /**
   * Constructs a {@link NodeKey} for a glob lookup.
   *
   * <p>Do not use outside {@code GlobNodeBuilder}.
   */
  @ThreadSafe
  static NodeKey internalKey(PathFragment packageName, PathFragment subdir, String pattern,
      boolean excludeDirs) {
    return new NodeKey(NodeTypes.GLOB,
   ...
   */
  @ThreadSafe
  static NodeKey internalKey(GlobDescriptor glob, String subdirName) {
