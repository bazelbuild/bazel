...
public class RemoteRepositoryRemoteExecutor implements RepositoryRemoteExecutor {
  ...
  public RemoteRepositoryRemoteExecutor(RemoteExecutionCache remoteCache,
      GrpcRemoteExecutor remoteExecutor,
      DigestUtil digestUtil, Context requestCtx, String remoteInstanceName, boolean acceptCached) {
  ...
  public ExecutionResult execute(ImmutableList<String> arguments,
    ...
    try {
      ...
      Command command = RemoteSpawnRunner.buildCommand(/* outputs= */ ImmutableList.of(),
          arguments, environment, platform, workingDirectory);
      Digest commandHash = digestUtil.compute(command);
      MerkleTree merkleTree = MerkleTree.build(
          ImmutableSortedMap.of(), /* metadataProvider= */ null, /* execRoot= */ null, digestUtil);
