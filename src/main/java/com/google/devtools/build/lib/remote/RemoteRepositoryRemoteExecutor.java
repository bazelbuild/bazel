...
public class RemoteRepositoryRemoteExecutor implements RepositoryRemoteExecutor {
  ...
  private ExecutionResult downloadOutErr(ActionResult result)
    ...
    try (SilentCloseable c = Profiler.instance().profile(ProfilerTask.REMOTE_DOWNLOAD, "download stdout/stderr")) {
  ...
  public ExecutionResult execute(
    ...
    try {
      ...
      try (SilentCloseable c = Profiler.instance().profile(ProfilerTask.REMOTE_CACHE_CHECK, "check cache hit")) {
        actionResult = remoteCache.downloadActionResult(actionKey, /* inlineOutErr= */ true);
      }
      if (actionResult == null || actionResult.getExitCode() != 0) {
        try (SilentCloseable c = Profiler.instance().profile(ProfilerTask.UPLOAD_TIME, "upload missing inputs")) {
        ...
        try (SilentCloseable c = Profiler.instance().profile(ProfilerTask.REMOTE_EXECUTION, "execute remotely")) {
