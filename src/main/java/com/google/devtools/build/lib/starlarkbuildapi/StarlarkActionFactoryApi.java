...
""";
  ...
  @StarlarkMethod(
      ...
      parameters = {
        ...
        @Param(
            name = "sibling",
            doc =
                "A file that lives in the same directory as the newly created file. The file must"
                    + " be in the current package. This is mutually exclusive with"
                    + " `directory_or_sibling`.",
            allowedTypes = {
            ...
            defaultValue = "None"),
        @Param(
            name = "directory_or_sibling",
            doc =
                "If a file is provided, the declared file will be created in the same directory as"
                    + " the provided file. If a directory is provided, the file will be created in"
                    + " that directory (only allowed in the implementation of"
                    + " ctx.actions.transform_directory). In both cases, the file must be in the"
                    + " current package. This is mutually exclusive with `sibling`.",
            allowedTypes = {
              @ParamType(type = FileApi.class),
            },
            named = true,
  ...
  FileApi declareFile(String filename, Object siblingUnchecked, Object directoryOrSiblingUnchecked)
  ...
  @StarlarkMethod(
      ...
      doc =
          "<b>Experimental. This API is experimental and may change at any time. Please do not"
              + " depend on it.</b> Transforms a directory into another one using a Starlark"
              + " function. The implementation function is evaluated during the execution phase"
              + " and allows you to create actions based on the files that are in the input"
              + " directory, which are only known during the execution phase after the directory"
              + " has been generated.",
      parameters = {
        @Param(
            ...
            named = true,
            positional = false,
            doc = "The input directory to transform."),
        @Param(
            ...
            named = true,
            positional = false,
            doc = "The output directory to transform the input directory into."),
        @Param(
            ...
            positional = false,
            doc =
                "List of additional output directories generated by actions created by the"
        ...
        @Param(
            ...
            positional = false,
            doc =
                "A Starlark function that is evaluated after the files in the input directory are"
                    + " known. It's called with: 1. an actions module (a.k.a. `ctx.actions`),"
                    + " 2. the list of files in the input `directory`, 3. the output directory and"
                    + " 4. an arbitrary number of kwargs that is passed to the implementation"
                    + " function."),
        @Param(
            name = "execution_requirements",
            allowedTypes = {
        ...
        @Param(
            ...
            doc =
                "<p>Toolchain type of the executable or tools used by the created actions.</p><p>If"
                    + " executable and tools are not coming from a toolchain, set this parameter to"
                    + " `None`.</p><p>If executable and tools are coming from a toolchain,"
                    + " toolchain type must be set so that the created actions execute on the"
                    + " correct execution platform.</p><p>Note that the rule which creates these"
                    + " actions needs to define this toolchain inside its 'rule()'"
                    + " function.</p><p>When `toolchain` and `exec_group` parameters are both set,"
                    + " `exec_group` will be used. An error is raised in case the `exec_group`"
                    + " doesn't specify the same toolchain.</p>"),
      },
      extraKeywords =
          @Param(
              name = "kwargs",
              doc =
                  "Dictionary of additional entries that are passed to the `implementation`"
                      + " function."),
      useStarlarkThread = true)
  void transformDirectory(
