...
public class UploadManifest {
   ...
   * result.
   */
  public void addAction(ActionKey actionKey, Action action, Command command) {
  ...
  private void illegalOutput(Path what) throws ExecException {
    ...
    FailureDetail failureDetail =
        FailureDetail.newBuilder()
            .setMessage(message)
            .setRemoteExecution(
                FailureDetails.RemoteExecution.newBuilder()
                    .setCode(FailureDetails.RemoteExecution.Code.ILLEGAL_OUTPUT))
            .build();
  ...
  public Completable uploadOutputs(RemoteActionExecutionContext context, RemoteCache remoteCache) {
  ...
  public void awaitUploadOutputs(RemoteActionExecutionContext context, RemoteCache remoteCache)
      throws InterruptedException, IOException, ExecException {
    try {
      uploadOutputs(context, remoteCache).blockingAwait();
    } catch (RuntimeException e) {
      throwIfInstanceOf(e.getCause(), InterruptedException.class);
      throwIfInstanceOf(e.getCause(), IOException.class);
      throwIfInstanceOf(e.getCause(), ExecException.class);
      throw e;
    }
  }
}
