  ...
  public void beforeCommand(CommandEnvironment env) {
    if (!enabled && isRequested()) {
      env.getReporter()
          .handle(
              Event.error(
                  "Failed to enable memory tracking, ensure that you set --host_jvm_args=-javaagent:<path to java-allocation-instrumenter-3.3.4.jar>"));
    }
  }

  private static boolean isRequested() {
    String memoryTrackerPropery = System.getProperty("RULE_MEMORY_TRACKER");
    return memoryTrackerPropery != null && memoryTrackerPropery.equals("1");
  }
}
