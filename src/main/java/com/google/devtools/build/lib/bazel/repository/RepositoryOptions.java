...
public class RepositoryOptions extends OptionsBase {
  ...
  @Option(
      ...
      help = "Check if the direct `bazel_dep` dependencies declared in the root module are the same "
          + "versions you get in the resolved dependency graph. Valid values are `warning` to print "
          + "a warning when mismatch detected, `off` to disable the check, or `error` to escalate "
          + "to a resolution failure.")
  public CheckDirectDepsMode checkDirectDependencies;

  /** An enum for specifying different modes for check direct dependency accuracy. */
