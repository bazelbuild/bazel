...
public abstract class LockfileModuleExtensionMetadata {
  ...
  private static Optional<RootModuleFileFixup> generateFixup(
    ...
    // Extract just the module-local names (keys) for set operations
    var actualDevImports = actualDevImportsMap.keySet();
    var actualImports = actualImportsMap.keySet();
  ...
  private Optional<ImportsWithMappings> getRootModuleDirectDepsWithMappings(Set<String> allRepos)
    ...
    return switch (getUseAllRepos()) {
      ...
      case REGULAR -> {
        ...
        yield Optional.of(
            new ImportsWithMappings(ImmutableSet.copyOf(allRepos), identityMap));
      }
