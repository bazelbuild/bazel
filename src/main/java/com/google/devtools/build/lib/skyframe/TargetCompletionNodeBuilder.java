...
/**
 * TargetCompletionNodeBuilder constructs a TargetCompletionActionNode. This works around two
 * problems with an alternative design which would create the completion action in the
 * ConfiguredTargetNodeBuilder directly:
...
final class TargetCompletionNodeBuilder implements NodeBuilder {

  private final SkyframeExecutor.BuildViewProvider buildViewProvider;

  TargetCompletionNodeBuilder(SkyframeExecutor.BuildViewProvider buildViewProvider) {
  ...
  public Node build(NodeKey nodeKey, Environment env) throws NodeBuilderException {
    LabelAndConfiguration lac = (LabelAndConfiguration) nodeKey.getNodeName();
    ConfiguredTargetNode ctNode = (ConfiguredTargetNode) env.getDep(
        ConfiguredTargetNode.key(lac.getLabel(), lac.getConfiguration()));
    if (ctNode == null) {
      return null;
    }

    TopLevelArtifactContext topLevelContext = BuildVariableNode.TOP_LEVEL_CONTEXT.get(env);
    if (topLevelContext == null) {
      return null;
    }

    ConfiguredTarget configuredTarget = ctNode.getConfiguredTarget();
    ...
    return new TargetCompletionActionNode(new TargetCompletionMiddlemanAction(configuredTarget,
