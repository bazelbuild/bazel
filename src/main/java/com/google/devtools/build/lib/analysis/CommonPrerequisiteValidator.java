...
public abstract class CommonPrerequisiteValidator implements PrerequisiteValidator {
  ...
  private static class VisibilityCheckState {
    ...
    RepositoryMapping getRepositoryMapping() {
      return getMainRepositoryMapping(consumer);
    }
  }
  ...
  private void checkForMisplacedPackageGroups(
    ...
    if (prerequisite.getConfiguredTarget().unwrapIfMerged()
      ...
      if (!attrName.equals("visibility")
        ...
        RepositoryMapping repoMapping = getMainRepositoryMapping(rule);
        context.attributeError(
            ...
            "in "
                ...
                + AliasProvider.describeTargetWithAliases(
                    prerequisite, TargetMode.WITH_KIND, repoMapping)
  ...
  private String generateVisibilityConflictMessage(VisibilityCheckState state) {
    RepositoryMapping repoMapping = state.getRepositoryMapping();
    String errorMessage;
    if (!state.verboseVisibilityErrors) {
      ...
      String consumerOrOwnerLocation =
          state.isImplicitDep()
              ? state.getOwnerDefinitionBzl().getDisplayForm(repoMapping)
      ...
      errorMessage =
          String.format(
              ...
              AliasProvider.describeTargetWithAliases(
                  state.prerequisite, TargetMode.WITHOUT_KIND, repoMapping),
      ...
      String dependencyDesc = state.prerequisite.getTargetLabel().getDisplayForm(repoMapping);
      errorMessage =
          String.format(
  ...
  private void maybeAddImplicitDepBullet(VisibilityCheckState state, List<String> bullets) {
    ...
    bullets.add(
        String.format(
            ...
            state.getOwnerName(),
            state.getOwnerDefinitionBzl().getDisplayForm(repoMapping),
            state.getOwnerDefinitionLocation().getDisplayForm(repoMapping)));
  }
      ...
      if (declaringMacro == null) {
        ...
        bullets.add(
            String.format(
                ...
                declaringMacro.getMacroClass().getDefiningBzlLabel().getDisplayForm(repoMapping),
                consumer.getDeclaringPackage().getCanonicalForm()));
      }
    } else {
        ...
        consumingLocation =
            String.format(
                ...
                outermostDelegated
                    .getPackageMetadata()
                    .packageIdentifier()
                    .getDisplayForm(repoMapping));
      } else {
        consumingLocation =
            String.format(
                "the body of the calling macro %s, defined in %s of package %s",
                delegationParent.getMacroClass().getName(),
                delegationParent.getMacroClass().getDefiningBzlLabel().getDisplayForm(repoMapping),
                delegationParent.getDefinitionPackage().getDisplayForm(repoMapping));
      }
      bullets.add(
          String.format(
              """
              ...
              outermostDelegated.getLabel().getDisplayForm(repoMapping),
              consumingLocation));
    }
  }
      ...
      if (isVisibleToLocation(
        ...
        bullets.add(
            String.format(
                ...
                m.getLabel().getDisplayForm(repoMapping),
                m.getMacroClass().getName(),
                m.getDefinitionPackage().getDisplayForm(repoMapping),
    ...
    if (isVisibleToLocation(
      ...
      bullets.add(
          String.format(
              ...
              """,
              moreThanOneLevelUp ? " transitive" : "",
              state.consumer.getPackageMetadata().packageIdentifier().getDisplayForm(repoMapping),
              moreThanOneLevelUp ? "this caller, or an intermediate caller," : "the caller"));
    }
  }
    ...
    if (generatingRuleLabel == null) {
      message =
          "non-test target '"
              ...
              + "' depends on testonly "
              + AliasProvider.describeTargetWithAliases(
                  prerequisite, TargetMode.WITHOUT_KIND, repoMapping)
      ...
      message =
          "non-test target '"
              ...
              + AliasProvider.describeTargetWithAliases(
                  prerequisite, TargetMode.WITHOUT_KIND, repoMapping)
              + " of a testonly rule "
              + generatingRuleLabel.getDisplayForm(repoMapping)
              + " and doesn't have testonly attribute set";
    } else {
      return;
    }
