...
public class WorkerMetricsCollector {
  ...
  private ImmutableMap<Long, Integer> summarizeDescendantsMemory(
    ...
    for (PsInfo psInfo : pidToPsInfo.values()) {
      parentPidToPsInfo.put(psInfo.parentPid, psInfo);
    }
  ...
  /** Recurseviely collects total memory usage of all descendatns of process. */
  private int collectMemoryUsageOfDescendants(
    ...
    for (PsInfo childrenPsInfo : parentPidToPsInfo.get(psInfo.pid)) {
  ...
  private static class PsInfo {
    ...
    public PsInfo(long pid, long parentPid, int memoryInKb) {
      this.pid = pid;
