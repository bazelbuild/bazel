...
public final class FlagSetFunction implements SkyFunction {
      ...
      for (ProjectValue.BuildableUnit buildableUnit : buildableUnits.values()) {
        if (doesBuildableUnitMatchTarget(buildableUnit, target)) {
          if (buildableUnit.isDefault()) {
            if (targetsAndMatchingDefaultBuildableUnits.containsKey(target)) {
              throw new FlagSetFunctionException(
                  new UnsupportedConfigException(
                      String.format(
            ...
            targetsAndMatchingDefaultBuildableUnits.put(target, buildableUnit);
          }
        }
      }
    }
  ...
  // this methods takes a list of default buildable units and compare the flags values of
  // all buildable units.
  // If the flags from buildable units are different, it returns multiple
  // buildable units.
  // If the flags from all buildable units are the same, it returns the first matching buildable
  // unit. The consumer of this method should check that there are no more than 1 buildable unit
  // returned.
  private static ImmutableList<ProjectValue.BuildableUnit>
    ...
    if (defaultBuildableUnitsForAllTargets.isEmpty()) {
      return defaultBuildableUnitsForAllTargets;
    }

    List<ProjectValue.BuildableUnit> resolvedBuildableUnits = new ArrayList<>();

    HashSet<String> firstBuildableUnitFlags =
        new HashSet<>(defaultBuildableUnitsForAllTargets.get(0).flags());
    ...
    return ImmutableList.copyOf(resolvedBuildableUnits);
  }
