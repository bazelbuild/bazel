...
public class Retrier {
  ...
  public Retrier(
    ...
    this(backoffSupplier, shouldRetry, retryScheduler, circuitBreaker, TimeUnit.MILLISECONDS::sleep);
  }
  ...
  /**
   * Executes an {@link AsyncCallable}, retrying execution in case of failure and returning a
   * {@link ListenableFuture} pointing to the result/error.
   */
  public <T> ListenableFuture<T> executeAsync(AsyncCallable<T> call) {
    return executeAsync(call, SettableFuture.create());
  }
   ...
   */
  public <T> ListenableFuture<T> executeAsync(AsyncCallable<T> call, SettableFuture<T> promise) {
  ...
  private <T> void onExecuteAsyncFailure(
    ...
    if (waitMillis >= 0 && t instanceof Exception && isRetriable((Exception) t)) {
      ...
      RetryException error =
          new RetryException(
              "Out of retries or status not retriable.", backoff.getRetryAttempts(), e);
