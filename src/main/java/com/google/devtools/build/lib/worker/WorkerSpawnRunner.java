...
final class WorkerSpawnRunner implements SpawnRunner {
  ...
  /**
   * The next work request ID to use. This field is static so we don't reuse work request IDs across Bazel invocations.
   * Although that shouldn't happen under normal circumstances because we wait until a request finishes before exiting, it
   * can happen if dynamic execution is enabled and one branch beats the other in the race.
   */
