...
public final class FastCDCChunker {
  ...
  public FastCDCChunker(
    ...
    checkArgument(avgSize > 0 && (avgSize & (avgSize - 1)) == 0, "avgSize must be a power of 2, got %s", avgSize);
   ...
   * returns ChunkRef objects that store the file offset and length of each chunk,
   * allowing the chunk data to be read later by seeking to the offset.
   *
   * <p>
