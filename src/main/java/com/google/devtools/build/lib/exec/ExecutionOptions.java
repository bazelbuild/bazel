...
public class ExecutionOptions extends OptionsBase {
  ...
  @Option(
      ...
      help =
          "Explicitly set the total number of local CPU cores available to Bazel to spend on build"
              ...
              + "`, optionally followed by `[-|*]{float}` (eg. `"
              + ResourceConverter.HOST_CPUS_KEYWORD
              + "*.5` to use half the available CPU cores). By default (`"
              + ResourceConverter.HOST_CPUS_KEYWORD
              + "`), Bazel will query system configuration to estimate the number of CPU cores "
              + "available.",
  ...
  @Option(
      ...
      help =
          "Explicitly set the total amount of local host RAM (in MB) available to Bazel to spend on"
              ...
              + "`, optionally followed by `[-|*]<float>` (eg. `"
              + ResourceConverter.HOST_RAM_KEYWORD
              + "*.5` to use half the available RAM). By default, (`"
              + ResourceConverter.HOST_RAM_KEYWORD
              + "*.67`), Bazel will query system configuration to estimate the amount of RAM "
              + "available and will use 67% of it.",
  ...
  @Option(
      ...
      help =
          ...
          by using a tag of the `resources:{resource_name}:{amount}` format.
  ...
  @Option(
      ...
      help =
          ...
          `resources:{resource_name}:{amount}` format.
