...
public final class CcLinkingHelper {
  ...
  /*
   * Adds a suffix (_{hash}) for path of linked DLL. This will be enabled only when rename_dll is enabled
   * and the linked artifact's LinkTargetType is NODEPS_DYNAMIC_LIBRARY (i.e DLL generated by cc_library).
   * We have to add the suffix to make sure the CppLinkAction link against the renamed DLL. If not, CppLinkAction
  ...
  private Artifact getLinkedArtifact(LinkTargetType linkTargetType) throws RuleErrorException {
      ...
      if(featureConfiguration.isEnabled(CppRuleClasses.TARGETS_WINDOWS)
          && linkTargetType == LinkTargetType.NODEPS_DYNAMIC_LIBRARY) {
