...
public class RemoteCache implements AutoCloseable {
  ...
  public List<ListenableFuture<Void>> downloadOutErr(
    ...
    if (!result.getStdoutRaw().isEmpty()) {
      ...
      downloads.add(
          cacheProtocol.downloadBlob(context, result.getStdoutDigest(), outErr.getOutputStream()));
    }
    if (!result.getStderrRaw().isEmpty()) {
      ...
      downloads.add(
          cacheProtocol.downloadBlob(context, result.getStderrDigest(), outErr.getErrorStream()));
    }
    return downloads;
  }
