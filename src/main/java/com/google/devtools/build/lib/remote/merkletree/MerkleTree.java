...
public class MerkleTree {
  ...
  @Nullable
  private final Directory rootProto;
  ...
  private static MerkleTree build(DirectoryTree tree, DigestUtil digestUtil) {
    ...
    if (tree.isEmpty()) {
      return new MerkleTree(
        null, digestUtil.compute(new byte[0]),
        ImmutableSortedSet.of(), ImmutableSortedMap.of(), 0, 0);
    }
  ...
  public static MerkleTree merge(Iterable<MerkleTree> merkleTrees, DigestUtil digestUtil) {
    MerkleTree firstMerkleTree = Iterables.getFirst(merkleTrees, null);
    if (firstMerkleTree == null) {
      return build(new DirectoryTree(ImmutableMap.of(), 0), digestUtil);
    }

    Digest firstRootDigest = firstMerkleTree.getRootDigest();
    if (Iterables.all(merkleTrees, (mt) -> mt.getRootDigest() == firstRootDigest)) {
    ...
    for (MerkleTree merkleTree : merkleTrees) {
      merkleTree.getDirectories().forEach((baseName, subMerkleTree) -> {
          allDirsToMerge.put(baseName, subMerkleTree);
      });
    }
    // Merge the Merkle trees for each path.
    SortedMap<String, MerkleTree> directories = new TreeMap<>();
    allDirsToMerge.asMap().forEach((baseName, dirsToMerge) -> {
        directories.put(baseName, merge(dirsToMerge, digestUtil));
    });
  ...
  private static MerkleTree buildMerkleTree(
    ...
    long inputFiles = Iterables.size(files);
