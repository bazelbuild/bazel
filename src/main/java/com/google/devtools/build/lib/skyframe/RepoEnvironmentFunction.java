 ...
 * (see {@code SkyframeExecutor#handleRepositoryEnvironmentChanges}). This is crucial for incremental
 * efficiency: changing a single variable must not invalidate consumers of the others. Reading the
 * whole environment as one Skyframe node instead would result in widespread invalidation of all
 * dependents whenever any variable changes. Crucially, change pruning is not fully effective here
 * as it doesn't resurrect nodes that aren't part of the current evaluation, but every evaluation
 * marks all transitive dependents as dirty.
 */
