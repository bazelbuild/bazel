...
class ProtoOutputFormatterCallback extends CqueryThreadsafeCallback {
  ...
  public void processOutput(Iterable<KeyedConfiguredTarget> partialResult)
    ...
    for (KeyedConfiguredTarget configuredTarget : partialResult) {
      ...
      builder.setTarget(formatter.toTargetProtoBuffer(accessor.getTarget(configuredTarget)));

      if (options.protoIncludeConfigurations) {
        String checksum = configuredTarget.configurationChecksum();
  ...
  private class ConfiguredProtoOutputFormatter extends ProtoOutputFormatter {
    ...
    protected void addAttributes(
      ...
      if (currentTarget.configuredTarget() instanceof AliasConfiguredTarget) {
        configConditions =
            ((AliasConfiguredTarget) currentTarget.configuredTarget()).getConfigConditions();
      } else if (currentTarget.configuredTarget() instanceof RuleConfiguredTarget) {
        configConditions =
            ((RuleConfiguredTarget) currentTarget.configuredTarget()).getConfigConditions();
      } else {
