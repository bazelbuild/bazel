...
/**
 * Responsible for parsing Ninja file, all its included and subninja files,
 * and returning {@link NinjaScope} with rules and expanded variables,
 * and list of {@link NinjaTarget}.
 *
 * Uses provided {@link ListeningExecutorService} for scheduling tasks in parallel.
 */
public class NinjaPipeline {
  ...
  /**
   * Each NinjaTarget should be parsed in the context of it's parent {@link NinjaScope}.
   * (All the variables in targets are immediately expanded.)
   * We are iterating main and all transitively included scopes, and parsing
   * corresponding targets.
   */
  private List<NinjaTarget> iterateScopesScheduleTargetsParsing(NinjaScope scope,
   ...
   * {@link NinjaPromise<NinjaFileParseResult>} - an object, from which we can
   * obtain the result of the parsing - {@link NinjaFileParseResult} - after we resolved variables
   * in the parent file to the point when the child file was included.
   *
   * In some cases, include and subninja statements can contain variable references, then
   * we must postpone scheduling the parsing of the file until variable is expanded and the
   * path to file becomes known.
   *
   * That is why we use {@link NinjaPromise<NinjaFileParseResult>} to save the future
  ...
  /**
   * Actually schedules the parsing of the Ninja file and returns
   * {@link ListenableFuture<NinjaFileParseResult>} for obtaining the result.
   */
  private ListenableFuture<NinjaFileParseResult> scheduleParsing(Path path) throws IOException {
    ...
    return service.submit(() -> {
      try(ReadableByteChannel channel = path.createReadableByteChannel()) {
        ...
        ParallelFileProcessing.processFile(channel, parameters,
            () -> {
