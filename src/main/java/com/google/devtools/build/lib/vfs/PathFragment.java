 ...
 *
 * <p>Path fragments are case-sensitive and compare as sequences of segments, which has the property
 * that the children of a directory sort immediately after it. Note that this differs from the
 * natural ordering of the underlying strings (e.g. {@code foo/bar-baz/qux} sorts after {@code
 * foo/bar/qux}, not before).
 */
@Immutable
public abstract sealed class PathFragment
  ...
  public int compareTo(PathFragment o) {
    ...
    // byte-by-byte.
    var b1 = StringUnsafe.getInternalStringBytes(this.getPathString());
    var b2 = StringUnsafe.getInternalStringBytes(o.getPathString());
