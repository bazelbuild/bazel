  ...
  @StarlarkMethod(
      ...
      parameters = {
        @Param(
            ...
            doc =
                "The repositories that the extension considers to be direct dependencies of the"
                    + " root module. Can be specified as either a list of repository names or a"
                    + " dict mapping module-local names to extension-exported names.<p>When"
                    + " specified as a <b>list</b> (e.g., <code>[\"foo\", \"bar\"]</code>), the"
                    + " repository names are used as-is in both the extension and the module.<p>"
                    + " When specified as a <b>dict</b> (e.g., <code>{\"my_foo\": \"ext_foo\"}"
                    + "</code>), the keys are the names used in the root module's"
                    + " <code>use_repo</code> calls, and the values are the names of the"
                    + " repositories created by the extension. This allows the root module to"
                    + " import repositories under different names than those created by the"
                    + " extension.<p>If the root module imports additional repositories or does"
                    + " not import all of these repositories via <a"
                    ...
                    + " as well. The repositories specified by these two parameters must be"
                    + " disjoint (neither module-local names nor extension-exported names can"
                    + " overlap).<p>Exactly one of <code>root_module_direct_deps</code> and"
        ...
        @Param(
            ...
            doc =
                "The repositories that the extension considers to be direct dev dependencies of"
                    + " the root module. Can be specified as either a list of repository names or a"
                    + " dict mapping module-local names to extension-exported names.<p>When"
                    + " specified as a <b>list</b> (e.g., <code>[\"foo\", \"bar\"]</code>), the"
                    + " repository names are used as-is in both the extension and the module.<p>"
                    + " When specified as a <b>dict</b> (e.g., <code>{\"my_foo\": \"ext_foo\"}"
