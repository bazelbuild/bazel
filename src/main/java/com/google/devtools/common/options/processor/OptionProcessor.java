...
public final class OptionProcessor extends AbstractProcessor {
  ...
  private static void checkDeprecated(VariableElement optionField) throws OptionProcessorException {
    Option annotation = optionField.getAnnotation(Option.class);
    ...
    if (hasDeprecatedMetadataTag && !hasDeprecatedAnnotation) {
      throw new OptionProcessorException(
          optionField, "Options with metadata tag DEPRECATED must be annotated with @Deprecated.");
    }
    if (hasDeprecatedAnnotation && !hasDeprecatedMetadataTag) {
      throw new OptionProcessorException(
          optionField, "Options annotated with @Deprecated must have metadata tag DEPRECATED.");
    }
  }
