...
static void ParseCommandLine(unique_ptr<vector<char *>> args) {
        ...
        if (opt.sandbox_root.empty()) {
          ...
          if (sandbox_root.back() == '/') {
            ...
            if(opt.sandbox_root.back() == '/'){
              Usage(args->front(), "Sandbox root path has trailing too many trailing slashes");
            }
          } else {
            opt.sandbox_root.assign(sandbox_root);
          }
        } else {
          Usage(args->front(), "Multiple sandbox roots (-s) specified, expected one.");
        }
        break;
      case 'H':
  ...
  if (!opt.working_dir.empty() && !opt.sandbox_root.empty() &&
    ...
    Usage(args->front(), "working-dir %s (-W) should be a "
      "subdirectory of sandbox-dir %s (-h)", opt.working_dir.c_str(), opt.sandbox_root.c_str());
  }
