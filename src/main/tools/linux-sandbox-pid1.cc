...
static void MountSandboxAndGoThere() {
  if (mount(opt.sandbox_root.c_str(), opt.sandbox_root.c_str(), nullptr, MS_BIND | MS_NOSUID, nullptr) < 0) {
    DIE("mount");
  }
...
static void MountAllMounts() {
  ...
  for (int i = 0; i < (signed)opt.bind_mount_sources.size(); i++) {
    if (opt.debug) {
      if (strcmp(opt.bind_mount_sources[i].c_str(), opt.bind_mount_targets[i].c_str()) == 0) {
        ...
        const std::string user_friendly_mount_target("<sandbox>" + opt.bind_mount_targets[i]);
        PRINT_DEBUG("mount: %s -> %s\n", opt.bind_mount_sources[i].c_str(),
        user_friendly_mount_target.c_str());
      }
    }
    const std::string full_sandbox_path(opt.sandbox_root + opt.bind_mount_targets[i]);
    ...
    int result = mount(opt.bind_mount_sources[i].c_str(), full_sandbox_path.c_str(), NULL,
