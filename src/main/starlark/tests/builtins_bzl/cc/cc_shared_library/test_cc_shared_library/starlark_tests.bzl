...
def _forwarding_cc_lib_aspect_impl(target, ctx):
    ...
    return [
        ...
        cc_common.CcSharedLibraryHintInfo_6_X_constructor_do_not_use(
            owners = [owner],
        ),
    ]
...
forwarding_cc_lib_aspect = aspect(
    ...
    provides = [AspectCcInfo, cc_common.CcSharedLibraryHintInfo_6_X_getter_do_not_use],
)

def _wrapped_cc_lib_impl(ctx):
    return [WrappedCcInfo(cc_info = ctx.attr.deps[0][CcInfo]), CcInfo()]
...
wrapped_cc_lib = rule(
    ...
    provides = [WrappedCcInfo, CcInfo],
)

def _forwarding_cc_lib_impl(ctx):
    hints = cc_common.CcSharedLibraryHintInfo_6_X_constructor_do_not_use(attributes = ["deps"])
    if ctx.attr.deps:
...
forwarding_cc_lib = rule(
    ...
    provides = [CcInfo, cc_common.CcSharedLibraryHintInfo_6_X_getter_do_not_use],
)
