...
if [[ "$UNAME" =~ msys_nt* ]]; then
  ...
  cp "$(rlocation "openjdk_java_manifest/file/java.manifest")" .
  ...
  ./bin/jlink --module-path ./jmods/ --add-modules "$modules" \
    ...
    --output reduced
  "C:/Program Files (x86)/Windows Kits/10/bin/10.0.19041.0/x64/mt.exe" -manifest ../java.manifest -outputresource:reduced/bin/java.exe
