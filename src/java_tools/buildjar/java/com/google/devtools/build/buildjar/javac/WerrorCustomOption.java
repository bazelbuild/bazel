...
import static com.google.devtools.build.buildjar.javac.JavacOptions.WARNINGS_AS_ERRORS_DEFAULT;
...
public class WerrorCustomOption {
  ...
  static WerrorCustomOption create(String arg) {
    return new WerrorCustomOption.Builder().process(arg).build();
  }
  ...
  static class Builder {
    ...
    Builder() {
      ...
      for (String errorWarning : WARNINGS_AS_ERRORS_DEFAULT) {
    ...
    Builder process(String flag) {
      ...
      for (String arg : Splitter.on(',').split(flag.substring(WERROR.length()))) {
        ...
        if (arg.equals("+all") || arg.equals("all")) {
          ...
          if (!WARNINGS_AS_ERRORS_DEFAULT.contains(warning)) {
