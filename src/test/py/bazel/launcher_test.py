...
if os.name == 'nt':
  ...
  def _get_exe_manifest(exe_path):
    ...
    if not (module := _LoadLibraryExW(exe_path, None, 0x2)):  # LOAD_LIBRARY_AS_DATAFILE
      raise ctypes.WinError()
    try:
      if not (info := _FindResourceW(module, 1, 24)):  # id=1, RT_MANIFEST=24
        raise ctypes.WinError()
      if not (data := _LoadResource(module, info)):  # "there is no need to free the resource"
        raise ctypes.WinError()
...
class LauncherTest(test_base.TestBase):
  ...
  def testWindowsNativeLauncherUserAccessControlManifest(self):
    ...
    el = manifest.find('.//{urn:schemas-microsoft-com:asm.v3}requestedExecutionLevel')
    ...
    # then, verify the manifest's effectiveness (would pass as admin even without the fix)
  ...
  def testBuildLaunchersWithClangClOnWindows(self):
    ...
    self.ScratchFile(
        ...
        [
            ...
            # Register the no-op RC toolchain since resource compilation is not under test here
