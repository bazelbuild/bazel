...
class PyRunfilesLibraryTest(test_base.TestBase):
  ...
  def testPyRunfilesLibraryCurrentRepository(self):
    ...
    external_repo = '' if self.IsWindows() else '+local_repository+other_repo'
    self.assertIn(
        'in external/other_repo/other_pkg/binary.py:' f" '{external_repo}'",
        stdout,
    )
    self.assertIn('in pkg/library.py: \'\'', stdout)
    ...
    self.assertIn(
        'in external/other_repo/other_pkg/test.py:' f" '{external_repo}'",
        stdout,
    )
    self.assertIn('in pkg/library.py: \'\'', stdout)
