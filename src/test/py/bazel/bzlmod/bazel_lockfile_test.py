  ...
  def testFactsInReproducibleExtension(self):
    ...
    self.ScratchFile(
        ...
        [
            ...
            '    # Also verify that the methods on the facts object behave consistently.',
            '    if (ctx.facts.get(resource_name) != None) != (resource_name in ctx.facts):',
            '        fail("ctx.facts.get() and ctx.facts disagree on whether the key exists")',
            '    if ctx.facts.get(resource_name) != None and ctx.facts.get(resource_name) != ctx.facts[resource_name]:',
            '        fail("ctx.facts.get() and ctx.facts disagree on the value of the key")',
