...
class BazelLockfileTest(test_base.TestBase):
  ...
  def testIsolatedAndNonIsolatedModuleExtensions(self):
    self.ScratchFile(
      ...
      [
        (
          'lockfile_ext_1 = use_extension("extension.bzl", "lockfile_ext",'
          ' isolate = True)'
        ),
    ...
    with open(self.Path('MODULE.bazel.lock'), 'r') as f:
      ...
      self.assertIn('//:extension.bzl%lockfile_ext%<root>~lockfile_ext_1',
                    lockfile['moduleExtensions'])
      self.assertIn('//:extension.bzl%lockfile_ext', lockfile['moduleExtensions'])
  ...
  def testUpdateIsolatedModuleExtension(self):
    ...
    with open(self.Path('MODULE.bazel.lock'), 'r') as f:
      lockfile = json.loads(f.read().strip())
      self.assertIn('//:extension.bzl%lockfile_ext%<root>~lockfile_ext',
                    lockfile['moduleExtensions'])
      self.assertNotIn('//:extension.bzl%lockfile_ext', lockfile['moduleExtensions'])
    ...
    with open(self.Path('MODULE.bazel.lock'), 'r') as f:
      lockfile = json.loads(f.read().strip())
      self.assertNotIn('//:extension.bzl%lockfile_ext%<root>~lockfile_ext',
                       lockfile['moduleExtensions'])
      self.assertIn('//:extension.bzl%lockfile_ext', lockfile['moduleExtensions'])
