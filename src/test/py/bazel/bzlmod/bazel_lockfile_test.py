...
class BazelLockfileTest(test_base.TestBase):
  ...
  def testLockfileWithNoUserSpecificPath(self):
    ...
    self.RunBazel(['build', '--registry='+self.my_registry.getLocalURL(), '//:lala'])

    with open('MODULE.bazel.lock', 'r') as f:
      module_file = f.read()
      # Ensure no user-specific path is in the lockfile, but only check the last two segments to avoid false negatives
      # caused by minor differences in the absolute path segment (e.g. casing or symbolic links).
