  ...
  def testExtensionEvaluationOnlyRerunOnRelevantUsagesChanges(self):
    ...
    self.ScratchFile(
      ...
      [
        ...
        '    num_tags = len([tag for mod in ctx.modules for tag in mod.tags.tag])',
    ...
    self.assertIn('Ext 1 saw 1 tags',
                  lockfile['moduleExtensions'][ext_1_key]['general']['generatedRepoSpecs']['dep'][
                    'attributes']['value'])
    self.assertIn(ext_2_key, lockfile['moduleExtensions'])
    self.assertIn('Ext 2 saw 1 tags',
                  lockfile['moduleExtensions'][ext_2_key]['general']['generatedRepoSpecs']['dep'][
                    'attributes']['value'])
    self.assertIn(ext_3_key, lockfile['moduleExtensions'])
    self.assertIn('Ext 3 saw 0 tags',
                  lockfile['moduleExtensions'][ext_3_key]['general']['generatedRepoSpecs']['dep'][
                    'attributes']['value'])

    # Shut down bazel to empty the cache, modify the MODULE.bazel file in a way that does not affect
    # the resolution of ext_1, but requires rerunning module resolution and removes ext_3, then
    ...
    self.ScratchFile(
      ...
      [
        ...
        '# The usage of ext_1 is unchanged except for locations and imports.',
        'ext_1 = use_extension("extension.bzl", "ext_1")',
        'ext_1.tag()',
    ...
    self.assertIn('Ext 1 saw 1 tags',
                  lockfile['moduleExtensions'][ext_1_key]['general']['generatedRepoSpecs']['dep'][
                    'attributes']['value'])
    # The usages of ext_2 changed, but the extension is not re-evaluated, so its previous, now stale
    ...
    _, _, stderr = self.RunBazel(['build', '@ext_1_dep_new_name//:all', '@ext_2_dep//:all'])
    stderr = '\n'.join(stderr)
    ...
    self.assertIn('Ext 1 saw 1 tags',
                  lockfile['moduleExtensions'][ext_1_key]['general']['generatedRepoSpecs']['dep'][
                    'attributes']['value'])
    self.assertIn(ext_2_key, lockfile['moduleExtensions'])
    self.assertIn('Ext 2 saw 2 tags',
                  lockfile['moduleExtensions'][ext_2_key]['general']['generatedRepoSpecs']['dep'][
                    'attributes']['value'])
    self.assertNotIn(ext_3_key, lockfile['moduleExtensions'])
