...
class BazelOverridesTest(test_base.TestBase):
  ...
  def testOverrideRepositoryOnNonExistentRepo(self):
    ...
    self.AssertNotExitCode(exit_code, 0, stderr)
    self.assertIn(
      "ERROR: Skipping '@@my_repo//:target': no such package '@@my_repo//': the repository @@my_repo does not exist, but has been specified as overridden with --override_repository. Use --inject_repository instead to add a new repository.",
      stderr,
    )
