...
class ModCommandTest(test_base.TestBase):
  ...
  def testModTidy(self):
    self.ScratchFile(
      ...
      [
        ...
        ')',
        'ext2 = use_extension("//:extension.bzl", "ext2", dev_dependency = True)',
    ...
    self.ScratchFile(
      ...
      [
        ...
        '        root_module_direct_dev_deps=["dev_dep", "missing_dev_dep"],',
        '    )',
        '',
    ...
    _, _, stderr = self.RunBazel(
      ['mod', 'deps', '--lockfile_mode=update', '--experimental_isolated_extension_usages'])
    stderr = '\n'.join(stderr)
    ...
    _, stdout, stderr = self.RunBazel(
      ['mod', 'tidy', '--lockfile_mode=update', '--experimental_isolated_extension_usages'])
    ...
    # Fixes are reported.
    self.assertIn('INFO: Updated use_repo calls for @//:extension.bzl%ext1', stderr)
    self.assertIn(
      'INFO: Updated use_repo calls for isolated usage \'ext1_isolated\' of @//:extension.bzl%ext1',
      stderr)
    self.assertIn('INFO: Updated use_repo calls for @//:extension.bzl%ext2', stderr)

    # Rerun bazel mod deps to check that the fixup warnings are gone and the lockfile is up-to-date.
    _, _, stderr = self.RunBazel(
      ['mod', 'deps', '--lockfile_mode=error', '--experimental_isolated_extension_usages'])
    stderr = '\n'.join(stderr)
    self.assertNotIn('ext1 is being evaluated', stderr)
    self.assertNotIn('ext2 is being evaluated', stderr)
    self.assertNotIn(
  ...
  def testModTidyAlwaysFormatsModuleFile(self):
    self.ScratchFile(
      'MODULE.bazel',
      [
        'ext=use_extension("//:extension.bzl", '
        '                  "ext")',
        'use_repo(ext,  "dep")',
      ],
    )
    ...
    # Verify that bazel mod tidy formats the MODULE.bazel file even if there are
    # no use_repos to fix.
  ...
  def testModTidyFixesInvalidImport(self):
    ...
    _, _, stderr = self.RunBazel(['mod', 'tidy'])
    stderr = '\n'.join(stderr)
    self.assertIn('ext defined in @//:extension.bzl reported incorrect imports', stderr)
    self.assertIn('invalid_dep', stderr)
    self.assertIn('INFO: Updated use_repo calls for @//:extension.bzl%ext', stderr)
