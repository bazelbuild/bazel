...
class BazelVendorTest(test_base.TestBase):
  ...
  def testVendorToolsForBazelSubcommands(self):
    ...
    # The filegroup makes the buildozer binary available without building it.
    self.RunBazel([
        'vendor',
        '--vendor_dir=vendor',
        '//...',
    ...
    self.assertTrue(
        any(
            os.path.isfile(self.Path('vendor/' + repo + '/buildozer.exe'))
            for repo in os.listdir(self.Path('vendor'))
        )
    )

    # Target-based vendoring alone does not cover dependency-only extensions.
