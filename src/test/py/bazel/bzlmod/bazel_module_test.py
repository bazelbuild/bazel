...
class BazelModuleTest(test_base.TestBase):
  ...
  def testLocalConfigPlatform_notAllowed(self):
    self.ScratchFile('MODULE.bazel')
    _, _, stderr = self.RunBazel(['build', '@local_config_platform//:host'],
                                 allow_failure=True)
    self.assertIn('The local_config_platform built-in module is disabled by',
                  '\n'.join(stderr))

  def testLocalConfigPlatform_allowed(self):
    self.ScratchFile('MODULE.bazel')
    self.RunBazel(['build', '--noincompatible_disable_native_repo_rules',
