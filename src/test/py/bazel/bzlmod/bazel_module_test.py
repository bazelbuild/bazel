...
class BazelModuleTest(test_base.TestBase):
  ...
  def testRegistryModuleInclude(self):
    ...
    self.AssertNotExitCode(exit_code, 0, stderr)
    self.assertIn(
      'include() directive found at ' + self.main_registry.getURL()
      + '/modules/foo/1.0/MODULE.bazel: but it can only be used in the root '
      + 'module or in modules with non-registry overrides',
      '\n'.join(stderr),
    )
