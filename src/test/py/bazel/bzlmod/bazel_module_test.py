...
class BazelModuleTest(test_base.TestBase):
  ...
  def testPerRegistryModuleMirrors(self):
    ...
    _, stdout, _ = self.RunBazel([
        ...
        '--registry=https://bcr.bazel.build',
        '--module_mirrors=%s=https://main-mirror.example.com' % self.main_registry.getURL(),
        '--module_mirrors=%s=https://second-mirror.example.com' % second_registry.getURL(),
    ])
    stdout = '\n'.join(stdout)
  ...
  def testPerRegistryModuleMirrorsWithDefault(self):
    ...
    _, stdout_aaa, _ = self.RunBazel([
        ...
        '--module_mirrors=%s=https://main-specific-mirror.example.com' % self.main_registry.getURL(),
    ])
    stdout_aaa = '\n'.join(stdout_aaa)
  ...
  def testUnknownRegistryInModuleMirrors(self):
    ...
    self.AssertNotExitCode(exit_code, 0, stderr)
    stderr = '\n'.join(stderr)
    self.assertIn('--module_mirrors references registries not listed in --registries', stderr)
