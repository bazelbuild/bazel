...
class ExternalRepoCompletionTest(test_base.TestBase):
  ...
  def setUp(self):
    ...
    self.ScratchFile(
      '.bazelrc',
      [
        # The command completion script invokes bazel with arguments we don't
        # control, so we need to import the default test .bazelrc here.
    ...
    scratchFile(self.projects_dir.joinpath('ext', 'BUILD'), ['cc_library(name="lib_ext")'])
    scratchFile(self.projects_dir.joinpath('ext', 'tools', 'BUILD'), ['cc_binary(name="tool")'])
    scratchFile(self.projects_dir.joinpath('ext', 'tools', 'zip', 'BUILD'),
    ...
    scratchFile(self.projects_dir.joinpath('ext2', 'BUILD'), ['cc_library(name="lib_ext2")'])
  ...
  def complete(self, bazel_args):
    ...
    # The full command line to complete as typed by the user (may end with a space).
...
""".format(
    ...
    env = os.environ.copy()
    env.update({
      # Have the completion script use the Bazel binary provided by the test runner.
  ...
  def testCompletions(self):
    ...
    self.assertCountEqual(
      ['@', '@//', '@bazel_tools', '@local_config_platform', '@foo', '@foobar', '@my_project',
       '@myrepo', '@myrepo2', '@ext', '@ext2'], self.complete('build @'))
    self.assertCountEqual(['@foo', '@foobar'], self.complete('build @fo'))
    self.assertCountEqual(['@foo', '@foo//', '@foobar'], self.complete('build @foo'))
    self.assertCountEqual(['@foobar', '@foobar//'], self.complete('build @foobar'))
    ...
    self.assertCountEqual(['@ext//tools/', '@ext//tools:'], self.complete('build @ext//tool'))
    self.assertCountEqual(['@ext//tools/zip/', '@ext//tools/zip:'],
                          self.complete('build @ext//tools/zi'))
    self.assertCountEqual(['@my_project//pkg/', '@my_project//pkg:'],
    ...
    self.assertCountEqual(['@@ext~1.0//tools/', '@@ext~1.0//tools:'],
                          self.complete('build @@ext~1.0//tool'))
    self.assertCountEqual(['@@ext~1.0//tools/zip/', '@@ext~1.0//tools/zip:'],
    ...
    self.assertCountEqual(['zipper'], self.complete('build @ext//tools/zip:zipp'))
    self.assertCountEqual(['my_lib'], self.complete('build @my_project//pkg:my_'))
    ...
    self.assertCountEqual(['zipper'], self.complete('build @@ext~1.0//tools/zip:zipp'))
