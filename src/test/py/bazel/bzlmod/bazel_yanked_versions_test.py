...
class BazelYankedVersionsTest(test_base.TestBase):
  ...
  def testYankedVersionsRefreshedOnModeSwitch(self):
    ...
    # Yank aaa@1.0.
    self.main_registry.addMetadata('aaa', yanked_versions={'1.0': 'already dodgy'})
  ...
  def testYankedVersionsRefreshedAfterAllowed(self):
    ...
    self.main_registry.addMetadata('aaa', yanked_versions={'1.0': 'even more dodgy'})

    # After temporarily allowing a yanked version, the yanked info is still refreshed.
    exit_code, _, stderr = self.RunBazel(
      ['build', '--nobuild', '--lockfile_mode=refresh', '//:main'],
      allow_failure=True)
    self.AssertExitCode(exit_code, 48, stderr)
