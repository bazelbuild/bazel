...
function test_java_test() {
  ...
  assert_build "${java_native_tests}:custom"
  jarfile="bazel-bin/examples/java-native/src/main/java/com/example/myproject/libcustom-greeting.jar"
  if [[ ! -f $jarfile ]]; then fail "FAIL 1" ; fi
  ...
  assert_binary_run_from_subdir "bazel-bin/${java_native_tests}/custom foo" "CICA"
