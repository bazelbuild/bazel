...
EOF
  ...
  grep -q '"mnemonic": "TestXmlGeneration"' output.json \
    || fail "missing default TestXmlGeneration mnemonic"
  ...
  grep -q '"mnemonic": "TestCoveragePostProcessing"' output.json \
    || fail "missing default TestCoveragePostProcessing mnemonic"

  bazel test //:test --noincompatible_separate_test_spawn_mnemonics \
    --execution_log_json_file=output.json >> $TEST_log 2>&1 || fail "test failed"
  grep -q '"mnemonic": "TestRunner"' output.json || fail "missing TestRunner mnemonic"
  ! grep -q '"mnemonic": "TestXmlGeneration"' output.json \
    || fail "TestXmlGeneration mnemonic not disabled"

  bazel coverage //:test --experimental_split_coverage_postprocessing \
    --experimental_fetch_all_coverage_outputs --noincompatible_separate_test_spawn_mnemonics \
    --execution_log_json_file=output.json >> $TEST_log 2>&1 || fail "coverage failed"
  ! grep -q '"mnemonic": "TestCoveragePostProcessing"' output.json \
    || fail "TestCoveragePostProcessing mnemonic not disabled"
}
