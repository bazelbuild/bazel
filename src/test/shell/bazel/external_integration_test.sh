...
EOF
  ...
  touch BUILD
  bazel query @hang//:all |&tee $TEST_log && fail "Bazel unexpectedly succeeded"
...
EOF
  ...
  # Start Bazel
  bazel query @defer//:all |& tee $TEST_log &
  local bazel_pid=$!

  # Wait until the .download() calls return
  cat "${server_dir}/gate_socket"

  # Tell the test server the strings it should serve
...
EOF
  ...
  # Start Bazel
  bazel query @defer//:all-targets |& tee $TEST_log &
  local bazel_pid=$!
