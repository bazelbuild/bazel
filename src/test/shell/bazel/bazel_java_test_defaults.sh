...
EOF
  ...
  expect_log "While resolving toolchains for target //pkg:foo: No matching toolchains found for types @bazel_tools//tools/jdk:runtime_toolchain_type"

  bazel build --platforms=//pkg:exotic_platform --java_runtime_version=local_jdk \
    //pkg:foo_deploy.jar &>"$TEST_log" && fail "Build should fail"
  expect_log "While resolving toolchains for target //pkg:foo_deployjars_internal_rule: No matching toolchains found for types @bazel_tools//tools/jdk:runtime_toolchain_type"
}
