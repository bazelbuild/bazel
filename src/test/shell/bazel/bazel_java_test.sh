...
# TODO: Once ready to be enabled on CI, make these separate tests with shared setup
...
function test_extra_args_unused_deps_execution() {
  ...
  mkdir -p bazel_internal/test_rules pkg
  touch bazel_internal/test_rules/BUILD

  cat > bazel_internal/test_rules/rule.bzl <<'EOF'
...
EOF
  ...
  cat > pkg/BUILD <<'EOF'
load("@rules_java//java:java_library.bzl", "java_library")
load("//bazel_internal/test_rules:rule.bzl", "custom_compile_rule")
