...
EOF
  ...
  expect_log "pwd: .*/foo.runfiles/bar$\|pwd: .*/foo.exe.runfiles/bar$"
  expect_log "src: .*/foo.runfiles$\|src: .*/foo.exe.runfiles$"
  expect_log "ws: bar$"
}
...
EOF
  ...
  hello_path=$PWD/scripts/hello
  if $is_windows; then
    hello_path="$(cygpath -m $hello_path).bat"
    cat <<EOF > scripts/hello.bat
@echo off
echo hello script!!! %*
EOF
  fi
  ...
  bazel test //testing:t1 --run_under=$hello_path \
...
function test_xml_is_present_when_timingout() {
  if "$is_windows"; then
    # Skip this test on Windows because the native test wrapper cannot distinguish timeout failure.
    # TODO(bazel-team): find a way to tell if a test is terminated due to timeout on Windows.
    return
  fi
...
function test_xml_is_present_when_timingout_split_xml() {
  ...
  if $is_windows; then
    # On Windows, we /usr/bin/less is not available
    expect_log '<!\[CDATA\['
  else
...
EOF
  ...
  expect_log "errors=\"0\""
  expect_log_once "testcase"
  expect_log "name=\"dir/test_shard_1/2\"\|name=\"dir/test.exe_shard_1/2\""
  cp bazel-testlogs/dir/test/shard_2_of_2/test.xml $TEST_log
  expect_log "errors=\"1\""
  expect_log_once "testcase"
  expect_log "name=\"dir/test_shard_2/2\"\|name=\"dir/test.exe_shard_2/2\""
}
...
EOF
  ...
  expect_log "name=\"dir/success\"\|name=\"dir/success.exe\""
  cat bazel-testlogs/dir/fail/test.xml >$TEST_log
  expect_log "errors=\"1\""
  expect_log_once "testcase"
  expect_log_once "duration=\"[0-9]\+\""
  expect_log "name=\"dir/fail\"\|name=\"dir/fail.exe\""
