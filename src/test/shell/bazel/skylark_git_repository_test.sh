...
function test_git_repository() {
  do_git_repository_test "52f9a3f"
}
...
function test_git_repository_strip_prefix() {
  ...
  do_git_repository_test "dbf9236" "pluto"
}
...
function test_git_repository_not_refetched_on_server_restart() {
  ...
  cat > WORKSPACE <<EOF
load('@bazel_tools//tools/build_defs/repo:git.bzl', 'git_repository')
git_repository(name='g', remote='$repo_dir', commit='22095302', verbose=True)
EOF
  ...
  cat > WORKSPACE <<EOF
load('@bazel_tools//tools/build_defs/repo:git.bzl', 'git_repository')
git_repository(name='g', remote='$repo_dir', commit='db134ae9', verbose=True)
EOF
...
function test_git_repository_not_refetched_on_server_restart_strip_prefix() {
  ...
  cat > WORKSPACE <<EOF
load('@bazel_tools//tools/build_defs/repo:git.bzl', 'git_repository')
git_repository(name='g', remote='$repo_dir', commit='17ea13b', verbose=True)
EOF
  ...
  cat > WORKSPACE <<EOF
load('@bazel_tools//tools/build_defs/repo:git.bzl', 'git_repository')
git_repository(name='g', remote='$repo_dir', commit='17ea13b', verbose=True, strip_prefix="gdir")
EOF
  bazel --batch build @g//:g >& $TEST_log || fail "Build failed"
  expect_log "Cloning"
  assert_contains "GIT 2" bazel-genfiles/external/g/go
...
EOF
  ...
  cat > WORKSPACE <<EOF
load('@bazel_tools//tools/build_defs/repo:git.bzl', 'git_repository')
git_repository(name='g', remote='$repo_dir', commit='17ea13b', strip_prefix="gdir")
EOF
