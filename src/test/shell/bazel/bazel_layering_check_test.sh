...
function test_bazel_layering_check() {
  ...
  CC="${clang_tool}" bazel build \
    //hello:hello -c opt --features=layering_check &> "${TEST_log}" \
    || fail "Build with layering_check failed"
  ...
  CC="${clang_tool}" bazel build \
    ...
    //hello:hello -c opt --features=layering_check &> "${TEST_log}" \
    && fail "Build of private header violation with layering_check "\
      "should have failed"
  ...
  CC="${clang_tool}" bazel build \
    ...
    && fail "Build of private header violation with layering_check "\
      "should have failed"
  expect_log "module //hello:hello does not depend on a module exporting 'base.h'"
}
