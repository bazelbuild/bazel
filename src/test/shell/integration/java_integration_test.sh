...
function test_compiles_hello_world() {
  ...
  bazel clean
  bazel build "${bazel_opts[@]}" //$pkg/java/hello:hello || fail "build failed"
...
function test_compiles_hello_world_from_deploy_jar() {
  ...
  bazel build "${bazel_opts[@]}" //$pkg/java/hello:hello_deploy.jar || fail "build failed"

  bazel run "${bazel_opts[@]}" //$pkg/java/hello:hello -- --singlejar | grep -q 'Hello, World!' \
    || fail "comparison failed"
  ${PRODUCT_NAME}-bin/$pkg/java/hello/hello -- --singlejar | \
    grep -q 'Hello, World!' || fail "comparison failed"

  bazel run "${bazel_opts[@]}" //$pkg/java/hello:hello -- --wrapper_script_flag=--singlejar \
    | grep -q 'Hello, World!' || fail "comparison failed"
  ${PRODUCT_NAME}-bin/$pkg/java/hello/hello -- \
    --wrapper_script_flag=--singlejar | grep -q 'Hello, World!' \
    || fail "comparison failed"

  bazel run "${bazel_opts[@]}" //$pkg/java/hello:hello -- REGULAR_ARG \
    --wrapper_script_flag=--singlejar | grep -q 'Hello, World!' \
    || fail "comparison failed"
...
function test_deterministic_nostamp_build() {
  ...
  bazel build "${bazel_opts[@]}" --nostamp //$pkg/java/hello:hello_deploy.jar \
      || fail "Build failed"
...
function test_compiles_hello_library() {
  ...
  bazel clean
  bazel build "${bazel_opts[@]}" //$pkg/java/main:main || fail "build failed"
  ${PRODUCT_NAME}-bin/$pkg/java/main/main \
      | grep -q "Hello, Library!;Hello, World!" || fail "comparison failed"
  bazel run "${bazel_opts[@]}" //$pkg/java/main:main -- --singlejar && fail "deploy jar should not exist"
...
function test_compiles_hello_library_using_ijars() {
  ...
  bazel clean
  bazel build "${bazel_opts[@]}" --use_ijars //$pkg/java/main:main || fail "build failed"
  ${PRODUCT_NAME}-bin/$pkg/java/main/main \
      | grep -q "Hello, Library!;Hello, World!" || fail "comparison failed"
}
...
function test_compiles_hello_library_from_deploy_jar() {
  ...
  bazel build "${bazel_opts[@]}" //$pkg/java/main:main_deploy.jar || fail "build failed"
...
function test_does_not_create_executable_when_not_asked_for() {
  ...
  bazel build "${bazel_opts[@]}" //$pkg/java/hellosailor:hellosailor_deploy.jar \
      || fail "build failed"
...
EOF
  ...
  bazel build "${bazel_opts[@]}" //$pkg/java/deploy:Hello_deploy.jar || fail "build failed"
...
EOF
  ...
  bazel run "${bazel_opts[@]}" //$pkg/java/hello:hello -- '' foo '' '' 'bar quux' '' \
...
EOF
  bazel build "${bazel_opts[@]}" //$pkg/java/hello:hello //$pkg/java/hello:hello_deploy.jar \
      >&$TEST_log || fail "Expected success"
  bazel run "${bazel_opts[@]}" //$pkg/java/hello:hello -- --singlejar >&$TEST_log
  expect_log "Hello World!"
}
...
EOF
  ...
  bazel build "${bazel_opts[@]}" //$pkg/java/test/client:client --use_ijars || fail "build failed"
  ...
  bazel build "${bazel_opts[@]}" //$pkg/java/test/client:libclient-src.jar --use_ijars \
    || fail "build failed"
...
EOF
  ...
  bazel build "${bazel_opts[@]}" //$pkg/java/com/google/jvmflags:foo || fail "build failed"
...
EOF
  ...
  bazel clean
  bazel build "${bazel_opts[@]}" //$pkg/java/classpath || fail "build failed"
  bazel run "${bazel_opts[@]}" //$pkg/java/classpath -- \
  ...
  bazel build "${bazel_opts[@]}" //$pkg/java/classpath:classpath_deploy.jar || fail "build failed"
...
EOF
  ...
  bazel run "${bazel_opts[@]}" //$pkg/java/foo:foo | grep -q "Success!" || fail "Expected success"
}
...
function test_header_compilation() {
  ...
  bazel build "${bazel_opts[@]}" -s --java_header_compilation=true \
...
EOF
  bazel build "${bazel_opts[@]}" --java_header_compilation=true \
...
EOF
  bazel build "${bazel_opts[@]}" //$pkg/java/hello:hello //$pkg/java/hello:hello_deploy.jar >& "$TEST_log" \
