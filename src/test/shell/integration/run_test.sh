...
function test_run_env() {
  add_rules_shell "MODULE.bazel"
  cat > .bazelrc <<EOF
run --run_env=OVERRIDDEN_RUN_ENV=OVERRIDDEN_VALUE_FROM_BAZELRC
run --run_env=FROM_BAZELRC=VALUE_FROM_BAZELRC
run --run_env=FROM_BAZELRC_THEN_UNUSET=VALUE_FROM_BAZELRC
EOF
  local -r pkg="pkg${LINENO}"
  mkdir -p "${pkg}"
  cat > "$pkg/BUILD" <<'EOF'
