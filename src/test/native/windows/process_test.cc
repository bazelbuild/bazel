...
// as short as they can be, so GetShortPathNameW cannot shorten the path below
// kMaxPath. This forces AsExecutablePathForCreateProcess into its fallback
// path.
TEST(ProcessTest, BatchFileWithLongPathExecutes) {
  ...
  ASSERT_TRUE(blaze_util::AsShortWindowsPath(blaze::GetPathEnv("TEST_TMPDIR"),
                                             &tmpdir_str, &short_error))
      << short_error;
  ...
  // Test root so we can recursively remove everything later on without
  ...
  ASSERT_TRUE(::DuplicateHandle(
      GetCurrentProcess(), GetStdHandle(STD_ERROR_HANDLE), GetCurrentProcess(),
      &stderr_h, 0, TRUE, DUPLICATE_SAME_ACCESS));
  bazel::windows::AutoHandle stderr_dup(stderr_h);
  ...
  EXPECT_TRUE(
      blaze_util::RemoveRecursively(blaze_util::WstringToCstring(test_root)));
}
