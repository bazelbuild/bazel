...
public class ProxyHelperTest {
  ...
  public void testCreateIfNeededHttpLowerCase() throws Exception {
    ...
    assertThat(proxyInfo.proxy().toString()).containsMatch("my\\.example\\.com(/<unresolved>)?:80$");
  }
  ...
  public void testCreateIfNeededHttpsLowerCase() throws Exception {
    ...
    assertThat(proxyInfo.proxy().toString()).containsMatch("my\\.example\\.com(/<unresolved>)?:443$");
  }
    ...
    ProxyHelper helper =
        new ProxyHelper(
            ImmutableMap.of(
                ...
                "HTTPS_PROXY",
                "https://my.example.com"));
    ProxyInfo proxyInfo = helper.createProxyIfNeeded(new URL("https://www.my.subdomain.something.com"));
    assertThat(proxyInfo.proxy()).isEqualTo(Proxy.NO_PROXY);
  }
  ...
  public void testProxyInfoWithUrlEncodedCredentials() throws Exception {
    ...
    ProxyInfo proxyInfo = ProxyHelper.createProxy("http://user:p%40ss%3Aword@proxy.example.com:8080");
    assertThat(proxyInfo.hasCredentials()).isTrue();
    String encoded = proxyInfo.getProxyAuthorizationHeader().substring("Basic ".length());
    String decoded = new String(Base64.getDecoder().decode(encoded));
  ...
  public void testProxyInfoWithUrlEncodedUsername() throws Exception {
    ...
    ProxyInfo proxyInfo = ProxyHelper.createProxy("http://user%40domain:password@proxy.example.com:8080");
    assertThat(proxyInfo.hasCredentials()).isTrue();
    String encoded = proxyInfo.getProxyAuthorizationHeader().substring("Basic ".length());
    String decoded = new String(Base64.getDecoder().decode(encoded));
  ...
  public void testProxyInfoFromSystemProperties() throws Exception {
    ...
    ProxyInfo proxyInfo = ProxyHelper.createProxyInfo(
        "http://proxy.example.com:8080", "sysuser", "syspass");
    assertThat(proxyInfo.hasCredentials()).isTrue();
    String encoded = proxyInfo.getProxyAuthorizationHeader().substring("Basic ".length());
    String decoded = new String(Base64.getDecoder().decode(encoded));
  ...
  public void testProxyInfoSystemPropertiesOnlyUserIgnored() throws Exception {
    // If only username is provided via system properties (no password), credentials should not be set
    ProxyInfo proxyInfo = ProxyHelper.createProxyInfo(
        "http://proxy.example.com:8080", "sysuser", null);
    assertThat(proxyInfo.hasCredentials()).isFalse();
  }

  @Test
  public void testProxyInfoSystemPropertiesOnlyPasswordIgnored() throws Exception {
    // If only password is provided via system properties (no username), credentials should not be set
    ProxyInfo proxyInfo = ProxyHelper.createProxyInfo(
        "http://proxy.example.com:8080", null, "syspass");
    assertThat(proxyInfo.hasCredentials()).isFalse();
  }
