...
public class InputTreeTest {
  ...
  public void emptyTreeShouldWork() throws Exception {
    InputTree tree = InputTree.build(new TreeMap<>(),
        new StaticMetadataProvider(Collections.emptyMap()), execRoot, digestUtil);
    assertThat(tree.directoryNodesAtDepth(0)).isEmpty();
    assertThat(tree.fileNodesAtDepth(0)).isEmpty();
  }
  ...
  public void buildingATreeOfFilesShouldWork() throws Exception {
    ...
    assertThat(tree.fileNodesAtDepth(0)).isEmpty();
    assertThat(tree.fileNodesAtDepth(1)).containsExactly(expectedFooNode, expectedBarNode);
    assertThat(tree.fileNodesAtDepth(2)).containsExactly(expectedBuzzNode);
  }
  ...
  public void directoryInputShouldBeExpanded() throws Exception {
    ...
    FileNode expectedBuzzNode = new FileNode("buzz.cc", buzz, digestUtil.computeAsUtf8("buzz"));
    assertThat(tree.fileNodesAtDepth(0)).isEmpty();
    assertThat(tree.fileNodesAtDepth(1)).containsExactly(expectedFooNode);
    assertThat(tree.fileNodesAtDepth(2)).containsExactly(expectedBarNode);
    assertThat(tree.fileNodesAtDepth(3)).containsExactly(expectedBuzzNode);
  }
  ...
  private VirtualActionInput addVirtualFile(String path, String content,
      SortedMap<PathFragment, ActionInput> sortedInputs) {
  ...
  private static List<String> directoriesAtDepth(int depth, InputTree tree) {
    return asPathSegments(tree.directoryNodesAtDepth(depth));
  }

  private static List<String> filesAtDepth(int depth, InputTree tree) {
    return asPathSegments(tree.fileNodesAtDepth(depth));
  }
