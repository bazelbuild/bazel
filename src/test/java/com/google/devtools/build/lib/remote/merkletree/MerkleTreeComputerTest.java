...
public class MerkleTreeComputerTest {
  ...
  public void buildForSpawn_retainAll_inlinesSubtreeBodyAndStampsTreeArtifactMarker()
    ...
    // The tree artifact's root directory is stamped bazel_is_treeartifact, and its full body (the
    // `file` child) is inlined into the returned tree rather than folded to a bare digest.
    ...
    assertThat(treeArtifactDir.getNodeProperties().getProperties(0).getName())
        .isEqualTo("bazel_is_treeartifact");
