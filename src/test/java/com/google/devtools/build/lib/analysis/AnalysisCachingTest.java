...
public class AnalysisCachingTest extends AnalysisCachingTestBase {
  ...
  @Test
  public void actionOutputIsSourceFile() throws Exception {
    scratch.file(
        "defs.bzl",
        """
        def _my_rule(ctx):
    ...
    scratch.file(
        ...
        """);
    scratch.file("source.txt");
    reporter.removeHandler(failFastHandler);

    assertThrows(ViewCreationFailedException.class, () -> update("//:my_rule"));

    assertContainsEvent(
        "in my_rule rule //:my_rule: File 'source.txt' is listed as an output of action"
            + " 'Writing file source.txt' but is a source file");
  }
