...
public class RewindingTestsHelper {
   ...
   *
   * <p>The more of them run at the same time, the more likely it is that a bug involving rewinding
   * and input consumption is detected.
   */
  private static final int CONCURRENT_ACTION_COUNT = 8;

  /** Reopens inputs to increase the chance of exposing deletion or corruption during rewinding. */
  private static void repeatedlyReadTreeInputs(
      ActionExecutionContext context, ActionInput... inputs) throws IOException {
    for (int i = 0; i < 1000; i++) {
      for (ActionInput input : inputs) {
        assertThat(latin1StringFromActionInput(context, input)).isEqualTo("seed\n");
      }
    }
  }
   ...
   * com.google.devtools.build.lib.actions.ActionTemplate} can be rewound while consumers of its
   * output tree are in flight.
   *
   * <p>Re-execution may overlap those consumers when the filesystem supports it, as Blaze's does.
   * The test accepts either execution order, provided consumers can still read their inputs
   * successfully.
   *
  ...
  public final void runActionTemplateExpansionRewound_withTreeConsumersInFlight() throws Exception {
    runActionTemplateExpansionRewound_withTreeConsumersInFlight(/* multipleTrees= */ false);
  }

  /**
   * Variant of {@link #runActionTemplateExpansionRewound_withTreeConsumersInFlight} in which the
   * expansion populates two tree artifacts: the first action of the chain writes into both trees,
   * whereas the second one consumes a file of one tree and populates the other.
   */
  public final void runActionTemplateExpansionRewound_withTreeConsumersInFlight_multipleTrees()
      throws Exception {
    runActionTemplateExpansionRewound_withTreeConsumersInFlight(/* multipleTrees= */ true);
  }

  private void runActionTemplateExpansionRewound_withTreeConsumersInFlight(
    ...
    // Bazel's RemoteRewoundActionSynchronizer switches from coarse to fine-grained locking on the
    // first rewind. Trigger that switch with an unrelated action before the consumers start, so
    // the coarse lock doesn't mask the behavior under test. All consumers depend on its output.
    addSpawnShim(
        "Executing genrule //foo:warmup_consumer",
        (spawn, context) -> createLostInputsExecException(spawn, context, "warmup.out"));

    // Start all consumers before reporting lost inputs, then repeatedly read their inputs while
    // rewinding proceeds. No consumer waits for re-execution to start or finish: either execution
    // order is valid as long as the inputs remain readable and unchanged.
    CountDownLatch allConsumersStarted = new CountDownLatch(CONCURRENT_ACTION_COUNT);
    ...
    for (int i = 0; i < CONCURRENT_ACTION_COUNT; i++) {
      addSpawnShim(
          ...
          (spawn, context) -> {
            ...
            lostInputsReported.await();
            SpecialArtifact mappedTree =
                SpawnInputUtils.getTreeArtifactWithName(spawn, "mapped_dir");
    ...
    // Report an output of each action in the chain as lost so that both actions re-execute
        ...
        (spawn, context) -> {
          ...
          ExecResult lostInputs =
              createLostInputsExecException(
                  context,
                  SpawnInputUtils.getExpandedToArtifact(
                      "f1.mid.inlined", intermediateTree, spawn, context),
                  SpawnInputUtils.getExpandedToArtifact(
                      "f1.out.inlined", mappedTree, spawn, context));
          lostInputsReported.countDown();
          return lostInputs;
        });
    ...
    verifyAllSpawnShimsConsumed();
    // An output of each action in the chain was lost, so both actions re-run.
    ...
    for (int i = 0; i < CONCURRENT_ACTION_COUNT; i++) {
      assertThat(executedSpawns).hasCount("Consuming //foo:consumer_" + i, 1);
    }
  ...
  /**
   * Verifies that an upstream {@link com.google.devtools.build.lib.actions.ActionTemplate} can be
   * rewound while downstream expanded actions consume individual files of its output tree.
   *
   * <p>As in {@link #runActionTemplateExpansionRewound_withTreeConsumersInFlight}, whether
  ...
  public final void runActionTemplateExpansionRewound_withDownstreamExpansionInFlight()
      throws Exception {
    runActionTemplateExpansionRewound_withDownstreamExpansionInFlight(/* multipleTrees= */ false);
  }

  /**
   * Variant of {@link #runActionTemplateExpansionRewound_withDownstreamExpansionInFlight} in which
   * the rewound sibling populates a different tree artifact than the one the downstream expansion
   * consumes a file of.
   */
  public final void
      runActionTemplateExpansionRewound_withDownstreamExpansionInFlight_multipleTrees()
          throws Exception {
    runActionTemplateExpansionRewound_withDownstreamExpansionInFlight(/* multipleTrees= */ true);
  }

  private void runActionTemplateExpansionRewound_withDownstreamExpansionInFlight(
      boolean multipleTrees) throws Exception {
    ...
    testCase.write(
        ...
        def _copy_child(template_ctx, child, output_directory, tools, suffix = ""):
            ...
            out = template_ctx.declare_file(name + ".out.inlined", directory = output_directory)
            args = template_ctx.args()
            args.add_all([child, out])
            template_ctx.run(
                inputs = [child],
        ...
        def _map_first_impl(template_ctx, input_directories, output_directories, tools, **kwargs):
            for child in input_directories["input"].children:
                if child.basename != "f1.out.inlined":
                    continue
    ...
    // Activate Bazel's fine-grained rewinding locks before the upstream expansion starts. The
    // upstream tree depends on the warmup output, which establishes this ordering without relying
    // on the filesystem's concurrency behavior.
    addSpawnShim(
        "Executing genrule //foo:warmup_consumer",
        (spawn, context) -> createLostInputsExecException(spawn, context, "warmup.out"));

    // Start all downstream actions before reporting lost inputs. Repeated reads then exercise
    // consumption during rewinding without prescribing whether upstream re-execution overlaps it.
    ...
    for (int i = 0; i < CONCURRENT_ACTION_COUNT; i++) {
      addSpawnShim(
          ...
          (spawn, context) -> {
            ...
            lostInputsReported.await();
            repeatedlyReadTreeInputs(
                context, SpawnInputUtils.getInputWithName(spawn, "f1.out.inlined"));
            return ExecResult.delegate();
          });
    }
    // Report both upstream outputs as lost after all downstream actions have started, so both
    // upstream actions re-execute regardless of how precisely rewinding identifies their producers.
    // The downstream actions don't wait for upstream re-execution: a filesystem that delays
    // re-execution until its consumers finish would otherwise deadlock on the test's own barrier.
    addSpawnShim(
        "Consuming //foo:losing_consumer",
        (spawn, context) -> {
          ...
          ExecResult lostInputs =
              createLostInputsExecException(
                  context,
                  SpawnInputUtils.getExpandedToArtifact(
                      "f1.out.inlined", upstreamTree, spawn, context),
                  SpawnInputUtils.getExpandedToArtifact(
                      "f2.out.inlined", otherTree, spawn, context));
          lostInputsReported.countDown();
          return lostInputs;
        });
