...
public class RewindingTestsHelper {
  ...
  public final void runActionTemplateExpansionRewound_withDownstreamExpansionInFlight()
    ...
    // The downstream actions wait only for lost inputs to be reported. When rewinding requires
    // synchronization, the synchronizer waits for these consumers to finish before allowing
    // upstream re-execution. Waiting for that re-execution here would therefore deadlock.
    addSpawnShim(
        "Consuming //foo:losing_consumer",
        (spawn, context) -> {
