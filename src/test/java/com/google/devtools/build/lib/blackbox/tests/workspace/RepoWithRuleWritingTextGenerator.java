...
public class RepoWithRuleWritingTextGenerator {
  ...
  Path setupRepository() throws IOException {
    Path workspace = generateWorkspaceFile();
  ...
  /**
   * Generates the minimal WORKSPACE file for the associated BUILD file.
   */
  Path generateWorkspaceFile() throws IOException {
    if (generateBuildFile) {
      return PathUtils.writeFileInDir(root, "WORKSPACE",
