...
public class RemoteActionInputFetcherTest {
  ...
  public void testFetching() throws Exception {
    ...
    // assert
    assertThat(FileSystemUtils.readContent(a1.getPath(), Charsets.UTF_8)).isEqualTo("hello world");
    assertThat(a1.getPath().isExecutable()).isTrue();
    assertThat(FileSystemUtils.readContent(a2.getPath(), Charsets.UTF_8)).isEqualTo("fizz buzz");
  ...
  public void testStagingVirtualActionInput() throws Exception {
    ...
    assertThat(FileSystemUtils.readContent(p, Charsets.UTF_8)).isEqualTo("hello world");
  ...
  public void testFileNotFound() throws Exception {
    ...
    // arrange
    Map<ActionInput, FileArtifactValue> metadata = new HashMap<>();
    Artifact a = createRemoteArtifact("file1", "hello world", metadata,
  ...
  public void testIgnoreNoneRemoteFiles() throws Exception {
    ...
    FileSystemUtils.writeContent(p, Charsets.UTF_8, "hello world");
  ...
  private Artifact createRemoteArtifact(String pathFragment, String contents,
      Map<ActionInput, FileArtifactValue> metadata, Map<Digest, ByteString> cacheEntries) {
    Path p = artifactRoot.getRoot().getRelative(pathFragment);
    Artifact a = new Artifact(p, artifactRoot);
    byte[] b = contents.getBytes(Charsets.UTF_8);
    HashCode h = HASH_FUNCTION.getHashFunction().hashBytes(b);
    FileArtifactValue f = new RemoteFileArtifactValue(h.asBytes(), b.length, /* locationIndex= */ 1);
    metadata.put(a, f);
  ...
  private static class StaticRemoteActionCache extends AbstractRemoteActionCache {
    ...
    public StaticRemoteActionCache(RemoteOptions options, DigestUtil digestUtil,
    ...
    @Override
    void upload(ActionKey actionKey, Action action, Command command, Path execRoot,
        Collection<Path> files, FileOutErr outErr) {
      throw new UnsupportedOperationException();
    }
