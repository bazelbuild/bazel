...
public class ModuleTest {
    ...
    assertThat(
            module.getRepoMappingWithBazelDepsOnly(
                Stream.of(key, fooKey, barKey, ModuleKey.ROOT)
                    .collect(
                        toImmutableMap(
                            k -> k, ModuleKey::getCanonicalRepoNameWithoutVersion))))
        .isEqualTo(
    ...
    assertThat(
            module.getRepoMappingWithBazelDepsOnly(
                Stream.of(ModuleKey.ROOT, fooKey, barKey)
                    .collect(
                        toImmutableMap(
                            k -> k, ModuleKey::getCanonicalRepoNameWithVersion))))
        .isEqualTo(
            createRepositoryMapping(
                ModuleKey.ROOT,
                "",
  ...
  public void getCanonicalRepoName_isNotAWindowsShortPath() {
    assertNotAShortPath(
        createModuleKey("foo", "").getCanonicalRepoNameWithoutVersion().getName());
    assertNotAShortPath(
        createModuleKey("foo", "1").getCanonicalRepoNameWithVersion().getName());
    assertNotAShortPath(
        createModuleKey("foo", "1.2").getCanonicalRepoNameWithVersion().getName());
    assertNotAShortPath(
