...
public final class StreamedProtoOutputFormatterTest extends PackageLoadingTestCase {
  ...
  @Test
  public void parallelOutputUsesDaemonThreads() throws Exception {
    RecordingFormatter formatter = new RecordingFormatter();

    format(formatter, options(PARALLEL_FLAG), AspectResolver.Mode.CONSERVATIVE, representativeTargets);

    assertThat(formatter.daemonStatuses).containsExactly(true);
    assertNoParallelOutputThreads();
  }

  @Test
  public void closeIsIdempotent(@TestParameter boolean failFast, @TestParameter boolean emptyInput)
      throws Exception {
    StreamedProtoOutputFormatter formatter = new StreamedProtoOutputFormatter();
    QueryOptions queryOptions = options(PARALLEL_FLAG);
    formatter.setOptions(
        queryOptions,
        AspectResolver.Mode.CONSERVATIVE.createResolver(getPackageManager(), reporter),
        DigestHashFunction.SHA256.getHashFunction());
    OutputFormatterCallback<Target> callback =
        formatter.createPostFactoStreamCallback(
            OutputStream.nullOutputStream(), queryOptions, LabelPrinter.legacy());
    try {
      callback.start();
      callback.processOutput(emptyInput ? ImmutableList.of() : representativeTargets);
    } finally {
  ...
  private static class RecordingFormatter extends StreamedProtoOutputFormatter {
    ...
    final Set<Boolean> daemonStatuses = ConcurrentHashMap.newKeySet();

    @Override
    public Build.Target toTargetProtoBuffer(Target target, LabelPrinter labelPrinter)
        throws InterruptedException {
      threadNames.add(Thread.currentThread().getName());
      daemonStatuses.add(Thread.currentThread().isDaemon());
