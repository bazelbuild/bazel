...
public class GrpcCacheClientTest {
  ...
  private GrpcCacheClient newClient(RemoteOptions remoteOptions, Supplier<Backoff> backoffSupplier)
    ...
    return new GrpcCacheClient(channel.retain(),
        creds,
        remoteOptions,
        retrier,
        DIGEST_UTIL,
  ...
  private byte[] downloadBlob(GrpcCacheClient cacheClient, Digest digest)
  ...
  public void testVirtualActionInputSupport() throws Exception {
    ...
    RemoteExecutionCache client = new RemoteExecutionCache(newClient(options), options, DIGEST_UTIL);
  ...
  public void downloadBlobPassesThroughDeadlineExceededWithoutProgress()
    ...
    IOException e =
        assertThrows(IOException.class, () -> downloadBlob(client, digest));
