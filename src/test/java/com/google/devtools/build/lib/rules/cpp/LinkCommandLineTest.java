...
public final class LinkCommandLineTest extends BuildViewTestCase {
  ...
  public void testTreeArtifactLink() throws Exception {
    ...
    verifyArguments(linkConfig.arguments(null, PathMapper.NOOP), treeArtifactsPaths,
        treeFileArtifactsPaths);
    verifyArguments(linkConfig.getRawLinkArgv(null), treeArtifactsPaths, treeFileArtifactsPaths);
    verifyArguments(linkConfig.paramCmdLine().arguments(null, PathMapper.NOOP), treeArtifactsPaths,
        treeFileArtifactsPaths);

    // Should only reference tree file artifacts.
    verifyArguments(linkConfig.arguments(expander, PathMapper.NOOP), treeFileArtifactsPaths,
        treeArtifactsPaths);
    verifyArguments(
        linkConfig.getRawLinkArgv(expander), treeFileArtifactsPaths, treeArtifactsPaths);
    verifyArguments(linkConfig.paramCmdLine().arguments(expander, PathMapper.NOOP),
        treeFileArtifactsPaths, treeArtifactsPaths);
  }
}
