...
public class NinjaLexerStepTest {
  ...
  public void testProcessLineFeedNewLine() {
    ...
    doTestError("\ra", NinjaLexerStep::processLineFeedNewLine, "\ra", false,
  ...
  public void testTryReadVariableInBrackets() {
    ...
    doTest("${  abc  }", step -> assertThat(step.tryReadVariableInBrackets()).isTrue());
    doTest("${abc.xyz-1_2}cde", step -> assertThat(step.tryReadVariableInBrackets()).isTrue(),
        "${abc.xyz-1_2}", true);
    doTestError("${abc", step -> assertThat(step.tryReadVariableInBrackets()).isTrue(),
        "${abc", false, "Variable end symbol '}' expected.");
    doTestError("${abc\n", step -> assertThat(step.tryReadVariableInBrackets()).isTrue(),
        "${abc\n", false, "Variable end symbol '}' expected.");
    doTestError("${}", step -> assertThat(step.tryReadVariableInBrackets()).isTrue(),
        "${}", false, "Variable identifier expected.");
    doTestError("${^}", step -> assertThat(step.tryReadVariableInBrackets()).isTrue(),
        "${^", true, "Variable identifier expected.");
  }
  ...
  public void testTryReadIdentifier() {
    ...
    doTestError("^abc", NinjaLexerStep::tryReadIdentifier, "^", true,
