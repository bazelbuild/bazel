...
public class ByteStreamUploaderTest {
  ...
  public void progressiveCompressedUploadSeesAlreadyExistsAtTheEnd() throws Exception {
    RemoteRetrier retrier =
        TestUtils.newRemoteRetrier(
            ...
            e ->
                Status.fromThrowable(e).getCode() == Code.INTERNAL
  ...
  public void contextShouldBePreservedUponRetries() throws Exception {
    ...
    RemoteRetrier retrier =
        TestUtils.newRemoteRetrier(
            () -> new FixedBackoff(5, 0), (e) -> Result.TRANSIENT_FAILURE, retryService);
    ByteStreamUploader uploader =
        new ByteStreamUploader(
            INSTANCE_NAME,
            referenceCountedChannel,
  ...
  public void chunkedHeaderIsAttachedOnlyForChunkedContext() throws Exception {
    ...
    byte[] chunkedBlob = new byte[CHUNK_SIZE];
    Arrays.fill(chunkedBlob, (byte) 1);
    ...
    Digest chunkedDigest = DIGEST_UTIL.compute(chunkedBlob);
    uploadBlob(
        uploader,
        context.chunked(),
        chunkedDigest,
        Chunker.builder().setInput(chunkedBlob).setChunkSize(CHUNK_SIZE).build());

    assertThat(chunkedHeaderValues).containsExactly(null, "FAST_CDC_2020").inOrder();
  }
  ...
  public void errorsShouldBeReported() throws IOException, InterruptedException {
    RemoteRetrier retrier =
        TestUtils.newRemoteRetrier(
            () -> new FixedBackoff(1, 10), (e) -> Result.TRANSIENT_FAILURE, retryService);
    ByteStreamUploader uploader =
        new ByteStreamUploader(
            INSTANCE_NAME,
            referenceCountedChannel,
  ...
  public void nonRetryableStatusShouldNotBeRetried() throws Exception {
    RemoteRetrier retrier =
        TestUtils.newRemoteRetrier(
            () -> new FixedBackoff(1, 0), /* No Status is retriable. */
  ...
  public void failureAfterUploadCompletes() throws Exception {
    ...
    RemoteRetrier retrier =
        TestUtils.newRemoteRetrier(
            () -> mockBackoff,
            e -> e instanceof StatusRuntimeException ? Result.TRANSIENT_FAILURE : Result.SUCCESS,
            retryService);
    ByteStreamUploader uploader =
        new ByteStreamUploader(
            INSTANCE_NAME,
