...
public class VersionsRendererTest {
  ...
  public void render_directAndTransitiveDeps() throws ParseException {
    ...
    // Up-to-date modules are hidden from the table.
    ...
    // Summary includes total count and up-to-date count.
    assertThat(output).contains("4 modules total, 2 with upgrades available, 2 up to date");
  }
  ...
  public void render_allUpToDate() throws ParseException {
    ...
    // No table rows for up-to-date modules.
    assertThat(output).doesNotContain("rules_java");
  }
  ...
  @Test
  public void render_mixedUpToDateAndUnknown() throws ParseException {
    List<ModuleVersionEntry> directDeps =
        List.of(
            new ModuleVersionEntry(
                "current_module", Version.parse("1.0.0"), Version.parse("1.0.0"), true),
            new ModuleVersionEntry(
                "mystery_module", Version.parse("1.0.0"), /* latest= */ null, true));
    ...
    // Unknown module is shown, up-to-date module is hidden.
    assertThat(output).contains("mystery_module");
    assertThat(output).doesNotContain("current_module");
    assertThat(output).contains("2 modules total, all up to date (1 with unknown versions)");
  }
  ...
  public void render_pinnedModule() throws ParseException {
    List<ModuleVersionEntry> directDeps =
        List.of(
            // Pinned and behind: shown with a "pinned" status, counted apart from upgrades.
            new ModuleVersionEntry(
                "rules_jvm_external",
                Version.parse("6.6"),
                Version.parse("7.0"),
                /* isDirect= */ true,
                /* isPinned= */ true),
            // Pinned but already at the latest version: hidden like up-to-date modules.
            new ModuleVersionEntry(
                "pinned_current",
                Version.parse("2.0"),
                Version.parse("2.0"),
    ...
    // Stale pin is shown with the "pinned" status, but not as an available upgrade.
    assertThat(output).contains("rules_jvm_external");
    assertThat(output).contains("pinned");
    assertThat(output).contains("3 modules total, 1 with upgrades available, 1 pinned, 1 up to date");
    // A pin already at the latest version is hidden.
    assertThat(output).doesNotContain("pinned_current");
  }
  ...
  public void render_utf8Separator() throws ParseException {
    ...
    new VersionsRenderer(out, /* useColor= */ false, /* useUtf8= */ true)
        .render(directDeps, List.of());
    // Decode as UTF-8 since the renderer writes UTF-8 bytes through a Latin-1 stream.
  ...
  public void render_sortsByName() throws ParseException {
    List<ModuleVersionEntry> directDeps =
        List.of(
            new ModuleVersionEntry(
                "zzz_module", Version.parse("1.0.0"), Version.parse("2.0.0"), true),
            new ModuleVersionEntry(
                "aaa_module", Version.parse("2.0.0"), Version.parse("3.0.0"), true));
