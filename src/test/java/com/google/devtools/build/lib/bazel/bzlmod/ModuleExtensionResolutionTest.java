...
public class ModuleExtensionResolutionTest extends FoundationTestCase {
  ...
  public void multipleModules_isolatedUsages() throws Exception {
    ...
    scratch.file(
        ...
        "    for tag in mod.tags.tag:",
        "      data_str += tag.data",
        "      tag.expect_isolated == ctx.is_isolated or fail()",
        "    data_str += '\\n'",
        "  data_repo(name='ext_repo',data=data_str)",
  ...
  public void extensionMetadata_isolate_devUsageWithAllDirectNonDevDeps() throws Exception {
    ...
    assertThat(result.hasError()).isTrue();
    assertContainsEvent(
        "root_module_direct_deps must be empty for an isolated extension usage with dev_dependency = True");
  }
  ...
  public void extensionMetadata_isolated() throws Exception {
    ...
    assertContainsEvent(
        "WARNING /ws/MODULE.bazel:3:21: The module extension ext defined in @ext//:defs.bzl reported incorrect imports of repositories via use_repo():\n"
            + "\n"
        ...
        ImmutableSet.of(EventKind.WARNING));
    assertContainsEvent(
        "WARNING /ws/MODULE.bazel:8:21: The module extension ext defined in @ext//:defs.bzl reported incorrect imports of repositories via use_repo():\n"
            + "\n"
            + "Not imported, but reported as direct dependencies by the extension (may cause the build to fail):\n"
            + "    missing_direct_dep\n"
            + "\n"
  ...
  public void extensionMetadata_isolatedDev() throws Exception {
    ...
    assertContainsEvent(
        "WARNING /ws/MODULE.bazel:3:21: The module extension ext defined in @ext//:defs.bzl reported incorrect imports of repositories via use_repo():\n"
            ...
            + "buildozer 'use_repo_remove dev @ext//:defs.bzl ext:0 indirect_dep' //MODULE.bazel:all",
        ImmutableSet.of(EventKind.WARNING));
    assertContainsEvent(
        "WARNING /ws/MODULE.bazel:8:21: The module extension ext defined in @ext//:defs.bzl reported incorrect imports of repositories via use_repo():\n"
            + "\n"
