...
public class AbstractRemoteActionCacheTests {
  ...
  public void testDownloadMinimalFiles() throws Exception {
    ...
    // act
    InMemoryOutput inMemoryOutput = remoteCache.downloadMinimal(r, ImmutableList.of(a1, a2),
        /* inMemoryOutputPath= */ null, new FileOutErr(), execRoot, injector);

    // assert
    assertThat(inMemoryOutput).isNull();
    verify(injector).injectRemoteFile(eq(a1), eq(toBinaryDigest(d1)),
        eq(d1.getSizeBytes()), anyInt());
    verify(injector).injectRemoteFile(eq(a2), eq(toBinaryDigest(d2)),
        eq(d2.getSizeBytes()), anyInt());
  ...
  public void testDownloadMinimalDirectory() throws Exception {
    ...
    SpecialArtifact dir = new SpecialArtifact(artifactRoot, PathFragment.create("outputs/dir"),
        ArtifactOwner.NullArtifactOwner.INSTANCE, SpecialArtifactType.TREE);

    MetadataInjector injector = mock(MetadataInjector.class);

    // act
    InMemoryOutput inMemoryOutput = remoteCache.downloadMinimal(r, ImmutableList.of(dir),
        /* inMemoryOutputPath= */ null, new FileOutErr(), execRoot, injector);
    ...
    Map<PathFragment, RemoteFileArtifactValue> m =
        ImmutableMap.<PathFragment, RemoteFileArtifactValue>builder()
            .put(PathFragment.create("file1"), new RemoteFileArtifactValue(toBinaryDigest(d1),
                d1.getSizeBytes(), 1))
            .put(PathFragment.create("a/file2"), new RemoteFileArtifactValue(toBinaryDigest(d2),
                d2.getSizeBytes(), 1))
        .build();
  ...
  public void testDownloadMinimalDirectoryFails() throws Exception {
    ...
    // act
    try {
      remoteCache.downloadMinimal(r, ImmutableList.of(dir),
          /* inMemoryOutputPath= */ null, new FileOutErr(), execRoot, injector);
  ...
  public void testDownloadMinimalWithStdoutStderr() throws Exception {
    ...
    // act
    InMemoryOutput inMemoryOutput = remoteCache.downloadMinimal(r, ImmutableList.of(),
        /* inMemoryOutputPath= */ null, outErr, execRoot, injector);
  ...
  public void testDownloadMinimalWithInMemoryOutput() throws Exception {
    ...
    // act
    InMemoryOutput inMemoryOutput = remoteCache.downloadMinimal(r, ImmutableList.of(a1, a2),
        inMemoryOutputPathFragment, new FileOutErr(), execRoot, injector);

    // assert
    assertThat(inMemoryOutput).isNotNull();
    ByteString expectedContents = ByteString.copyFrom("content1", Charsets.UTF_8);
