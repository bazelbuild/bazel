  ...
  private void expiredAuthTokensShouldBeRetried_get(ErrorType errorType) throws Exception {
    ...
    try {
      ...
      HttpBlobStore blobStore =
          new HttpBlobStore(new URI("http://localhost:" + serverPort), 1, credentials);
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) {
      if (messageCount == 0) {
        ...
        if (errorType == ErrorType.UNAUTHORIZED) {
          ...
          response =
              new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.UNAUTHORIZED);
          response.headers().set(HttpHeaderNames.WWW_AUTHENTICATE, "Bearer realm=\"localhost\"," +
              "error=\"" + errorType.name().toLowerCase() + "\"," +
