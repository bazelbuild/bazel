...
public class MerkleTreeTest {
  ...
  @Test
  public void emptyMerkleTreeShouldWork() throws IOException {
    MerkleTree tree = MerkleTree.build(Collections.emptySortedMap(),
        new StaticMetadataProvider(Collections.emptyMap()), execRoot, digestUtil);
    Digest emptyDigest = digestUtil.compute(new byte[0]);
    assertThat(tree.getRootDigest()).isEqualTo(emptyDigest);
  }

  @Test
  public void buildingAMerkleTreeShouldWork() throws IOException {
    ...
    MerkleTree tree = MerkleTree.build(sortedInputs,
        new StaticMetadataProvider(metadata), execRoot, digestUtil);

    Directory fizzDir = Directory.newBuilder()
        ...
        .build();
    Directory srcsDir = Directory.newBuilder()
        ...
        .addDirectories(DirectoryNode.newBuilder()
            .setName("fizz")
            .setDigest(digestUtil.compute(fizzDir)))
        .build();
    Directory rootDir = Directory.newBuilder()
        .addDirectories(DirectoryNode.newBuilder()
            .setName("srcs")
            .setDigest(digestUtil.compute(srcsDir)))
        .build();

    Digest expectedRootDigest = digestUtil.compute(rootDir);
    assertThat(tree.getRootDigest()).isEqualTo(expectedRootDigest);

    Digest[] dirDigests = new Digest[] {digestUtil.compute(fizzDir),
        digestUtil.compute(srcsDir), digestUtil.compute(rootDir)};
    ...
    Digest[] inputDigests = new Digest[] {digestUtil.computeAsUtf8("foo"),
        digestUtil.computeAsUtf8("bar"), digestUtil.computeAsUtf8("buzz"),
  ...
  private Artifact addFile(String path, String content,
