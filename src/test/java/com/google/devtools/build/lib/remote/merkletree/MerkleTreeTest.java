...
public class MerkleTreeTest {
  ...
  public void mergeMerkleTrees() throws IOException {
    ...
    Artifact foo = addFile("srcs/foo.cc", "foo", sortedInputs1, metadata);
    Artifact bar = addFile("srcs/bar.cc", "bar", sortedInputs2, metadata);
    Artifact buzz = addFile("srcs/fizz/buzz.cc", "buzz", sortedInputs1, metadata);
    Artifact fizzbuzz = addFile("srcs/fizz/fizzbuzz.cc", "fizzbuzz", sortedInputs2, metadata);
    sortedInputsAll.putAll(sortedInputs1);
    sortedInputsAll.putAll(sortedInputs2);

    MerkleTree treeEmpty = MerkleTree.build(
        new TreeMap<>(), new StaticMetadataProvider(metadata), execRoot, digestUtil);
    MerkleTree tree1 = MerkleTree.build(
        sortedInputs1, new StaticMetadataProvider(metadata), execRoot, digestUtil);
    MerkleTree tree2 = MerkleTree.build(
        sortedInputs2, new StaticMetadataProvider(metadata), execRoot, digestUtil);
    MerkleTree treeAll = MerkleTree.build(
    ...
    assertThat(mergedTree1 == tree1).isTrue();
