...
public final class RewindingSynchronizerTest {
  ...
  @Test
  public void acquireReadLock_replacementsDisabled_doesNotEvaluateKey() throws Exception {
    synchronizer.reset(/* replacementsEnabled= */ false);

    synchronizer.acquireReadLock(RewindingSynchronizerTest::failKeyEvaluation).close();
  }

  @Test
  public void acquireReadLock_noReplacementYet_doesNotEvaluateKey() throws Exception {
  ...
  @Test
  public void acquireReadLock_afterReplacement_doesNotBlockOtherKeysProducer() throws Exception {
    switchToPerKeyLocks();
    AtomicBoolean acquired = new AtomicBoolean();

    try (SilentCloseable unused = synchronizer.acquireReadLock(() -> KEY)) {
      TestThread producer = producerThread(OTHER_KEY, acquired);
      producer.start();
  ...
  public void acquireReadLock_otherKeyReplacementInProgress_doesNotWait() throws Exception {
    ...
    try (TransferableWriteLock unused = synchronizer.acquireWriteLock(OTHER_KEY)) {
      synchronizer.acquireReadLock(() -> KEY).close();
    }
  }
  ...
  @Test
  public void acquireReadLock_producerWaitsForSharedReadLock_doesNotWait() throws Exception {
    synchronizer.reset(/* replacementsEnabled= */ true);
    synchronizer.markReplacementsPossible();
    SilentCloseable sharedLock =
        synchronizer.acquireReadLock(RewindingSynchronizerTest::failKeyEvaluation);
    AtomicBoolean acquired = new AtomicBoolean();
    TestThread producer = producerThread(KEY, acquired);
    producer.start();
    ...
    // A read on behalf of the holder of the shared lock, e.g. the materialization of one of its
    // inputs on another thread, must not queue behind the producer waiting for that holder. It
    // must run on another thread: a reentrant lock would also admit a reader on this thread.
    TestThread nestedReader = new TestThread(() -> synchronizer.acquireReadLock(() -> KEY).close());
    try {
      nestedReader.start();
      nestedReader.joinAndAssertState(TIMEOUT_MILLIS);
      assertThat(acquired.get()).isFalse();
    } finally {
      sharedLock.close();
    }
  ...
  @Test
  public void acquireWriteLock_interruptedWhileWaitingForSharedReadLock_nextProducerWaits()
      throws Exception {
    synchronizer.reset(/* replacementsEnabled= */ true);
    synchronizer.markReplacementsPossible();
    SilentCloseable sharedLock =
        synchronizer.acquireReadLock(RewindingSynchronizerTest::failKeyEvaluation);
    TestThread interruptedProducer =
        new TestThread(
            () ->
                assertThrows(InterruptedException.class, () -> synchronizer.acquireWriteLock(KEY)));
    interruptedProducer.start();
    awaitState(interruptedProducer, Thread.State.WAITING);
    interruptedProducer.interrupt();
    interruptedProducer.joinAndAssertState(TIMEOUT_MILLIS);

    // The shared lock hasn't been retired, so the next producer still waits for its holder.
    AtomicBoolean acquired = new AtomicBoolean();
    TestThread producer = producerThread(OTHER_KEY, acquired);
    producer.start();
    awaitState(producer, Thread.State.WAITING);
  ...
  @Test
  public void acquireReadLock_producerWaitsForPerKeyReadLocks_isAdmitted() throws Exception {
    switchToPerKeyLocks();
    SilentCloseable readLocks = synchronizer.acquireReadLocks(() -> ImmutableList.of(KEY));
    AtomicBoolean acquired = new AtomicBoolean();
    TestThread producer = producerThread(KEY, acquired);
    producer.start();
    awaitState(producer, Thread.State.WAITING);

    // A read on behalf of the holder of the read lock must be admitted while the producer waits.
    synchronizer.acquireReadLock(() -> KEY).close();
    assertThat(acquired.get()).isFalse();
    readLocks.close();
    producer.joinAndAssertState(TIMEOUT_MILLIS);
  ...
  @Test
  public void acquireReadLocks_waitingForProducer_doesNotBlockItsDependency() throws Exception {
    switchToPerKeyLocks();
    TestThread reader =
        new TestThread(
            () -> {
              try (SilentCloseable unused =
                  synchronizer.acquireReadLocks(() -> ImmutableList.of(KEY, OTHER_KEY))) {}
            });
    try (TransferableWriteLock unused = synchronizer.acquireWriteLock(OTHER_KEY)) {
      reader.start();
      awaitState(reader, Thread.State.WAITING);
      AtomicBoolean acquired = new AtomicBoolean();
      TestThread dependency = producerThread(KEY, acquired);
      dependency.start();
      try {
        dependency.joinAndAssertState(TIMEOUT_MILLIS);
        assertThat(acquired.get()).isTrue();
      } finally {
