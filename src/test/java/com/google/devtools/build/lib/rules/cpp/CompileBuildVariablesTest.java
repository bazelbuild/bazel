...
public class CompileBuildVariablesTest extends BuildViewTestCase {
  ...
  public void testPresenceOfConlyFlags() throws Exception {
    useConfiguration(
        "--conlyopt=-foo",
        "--cxxopt=-not-passed",
        "--per_file_copt=//x:bin@-per-file");

    scratch.file("x/BUILD", "cc_binary(name = 'bin', srcs = ['bin.c'], copts = ['-bar'], conlyopts = ['-baz'], cxxopts = ['-not-passed'])");
    ...
    assertThat(copts)
        .containsExactlyElementsIn(
                ImmutableList.<String>of("-foo", "-bar", "-baz", "-per-file"))
        .inOrder();
  }
  ...
  public void testCxxFlagsOrder() throws Exception {
    ...
    scratch.file("x/BUILD", "cc_binary(name = 'bin', srcs = ['bin.cc'], copts = ['-bar'], cxxopts = ['-baz'], conlyopts = ['-not-passed'])");
    scratch.file("x/bin.cc");
