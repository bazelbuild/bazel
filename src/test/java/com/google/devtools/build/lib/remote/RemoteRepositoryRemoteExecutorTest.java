...
public class RemoteRepositoryRemoteExecutorTest {
  ...
  @Mock
  public RemoteExecutionCache remoteCache;

  @Mock
  public GrpcRemoteExecutor remoteExecutor;
  ...
  public void setup() {
    ...
    repoExecutor = new RemoteRepositoryRemoteExecutor(remoteCache, remoteExecutor, DIGEST_UTIL,
        Context.current(), /* remoteInstanceName= */ "foo", /* acceptCached= */ true);
  }
  ...
  public void testZeroExitCodeFromCache() throws IOException, InterruptedException {
    ...
    ExecutionResult executionResult =
        repoExecutor.execute(ImmutableList.of("/bin/bash", "-c", "exit 0"),
            /* executionProperties= */ ImmutableMap.of(),/* environment= */ ImmutableMap.of(),
            /* workingDirectory= */ null, /* timeout= */ Duration.ZERO);
  ...
  public void testNoneZeroExitCodeFromCache() throws IOException, InterruptedException {
    ...
    // Act
    ExecutionResult executionResult =
        repoExecutor.execute(ImmutableList.of("/bin/bash", "-c", "exit 1"),
        /* executionProperties= */ ImmutableMap.of(),/* environment= */ ImmutableMap.of(),
        /* workingDirectory= */ null, /* timeout= */ Duration.ZERO);
