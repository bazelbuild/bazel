...
public class RemoteExecutionServiceTest {
  ...
  public void downloadOutputs_outputFiles_maintainsExecutableBit() throws Exception {
    ...
    builder.addOutputFilesBuilder().setPath("outputs/bar").setDigest(barDigest).setIsExecutable(true);
  ...
  public void downloadOutputs_outputDirectories_works() throws Exception {
    ...
    Tree barTreeMessage =
        Tree.newBuilder()
            ...
            .build();
    Digest barTreeDigest = cache.addContents(remoteActionExecutionContext, barTreeMessage.toByteArray());
    ActionResult.Builder builder = ActionResult.newBuilder();
  ...
  public void downloadOutputs_nestedOutputDirectories_works() throws Exception {
    ...
    Directory wobbleDirMessage =
        Directory.newBuilder()
            ...
            .build();
    Digest wobbleDirDigest = cache.addContents(remoteActionExecutionContext, wobbleDirMessage.toByteArray());
    Tree barTreeMessage =
        Tree.newBuilder()
            .setRoot(
                Directory.newBuilder()
    ...
    // assert
    assertThat(digestUtil.compute(execRoot.getRelative("outputs/a/foo"))).isEqualTo(fooDigest);
    assertThat(digestUtil.compute(execRoot.getRelative("outputs/a/bar/wobble/qux"))).isEqualTo(quxDigest);
  ...
  public void downloadOutputs_outputDirectoriesWithSameHash_works() throws Exception {
    ...
    // assert
    assertThat(digestUtil.compute(execRoot.getRelative("outputs/a/bar/foo/file"))).isEqualTo(fileDigest);
    assertThat(digestUtil.compute(execRoot.getRelative("outputs/a/foo/file"))).isEqualTo(fileDigest);
    assertThat(context.isLockOutputFilesCalled()).isTrue();
  }
  ...
  public void downloadOutputs_onFailure_maintainDirectories() throws Exception {
    ...
    builder.addOutputFiles(
        OutputFile.newBuilder().setPath("outputs/outputdir/outputfile").setDigest(outputFileDigest));
    builder.addOutputFiles(OutputFile.newBuilder().setPath("outputs/otherfile").setDigest(otherFileDigest));
    RemoteActionResult result = RemoteActionResult.createFromCache(builder.build());
    Spawn spawn = newSpawnFromResult(result);
    FakeSpawnExecutionContext context = newSpawnExecutionContext(spawn);
    RemoteExecutionService service = newRemoteExecutionService();
  ...
  public void downloadOutputs_outputFilesWithTopLevel_download() throws Exception {
    ...
    RemoteExecutionService service = newRemoteExecutionService(remoteOptions, spawn.getOutputFiles());
    RemoteAction action = service.buildRemoteAction(spawn, context);
  ...
  public void downloadOutputs_outputDirectoriesWithMinimal_injectingMetadata() throws Exception {
    ...
    ActionResult r =
        ActionResult.newBuilder()
            .setExitCode(0)
            .addOutputDirectories(OutputDirectory.newBuilder().setPath("outputs/dir").setTreeDigest(dt))
            .build();
    ...
    TreeArtifactValue tree =
        TreeArtifactValue.newBuilder(dir)
            .putChild(
                ...
                new RemoteFileArtifactValue(toBinaryDigest(d1), d1.getSizeBytes(), 1, action.getActionId()))
            .putChild(
                TreeFileArtifact.createTreeOutput(dir, "a/file2"),
                new RemoteFileArtifactValue(toBinaryDigest(d2), d2.getSizeBytes(), 1, action.getActionId()))
            .build();
  ...
  public void downloadOutputs_missingInMemoryOutputWithMinimal_returnsNull() throws Exception {
    // Test that downloadOutputs returns null if a declared in-memory output is missing from action result.
    ...
    Artifact a1 = ActionsTestUtil.createArtifact(artifactRoot, "file1");
    Spawn spawn =
        newSpawn(ImmutableMap.of(REMOTE_EXECUTION_INLINE_OUTPUTS, "outputs/file1"), ImmutableSet.of(a1));
    MetadataInjector injector = mock(MetadataInjector.class);
    FakeSpawnExecutionContext context = newSpawnExecutionContext(spawn, injector);
    RemoteOptions remoteOptions = Options.getDefaults(RemoteOptions.class);
    remoteOptions.remoteOutputsMode = RemoteOutputsMode.MINIMAL;
  ...
  private RemoteExecutionService newRemoteExecutionService(RemoteOptions remoteOptions, Collection<? extends ActionInput> topLevelOutputs) {
