...
public class TreeNodeRepositoryTest {
  ...
  public void testDirectoryInput() throws Exception {
    ...
    TreeNode fooHNode = fooNode.getChildEntries().get(1).getChild(); // foo.h > foo.cc in sort order!
    ...
    Digest barDigest = repo.getMerkleDigest(barNode);
    assertThat(digests).containsExactly(rootDigest, aDigest, barDigest, fooDigest, fooHDigest, fooCcDigest);
