...
public final class HeaderDiscoveryTest {
  ...
  private final ArtifactRoot sourceRoot =
      ArtifactRoot.asSourceRoot(Root.fromPath(execRoot));
        ...
        () ->
            checkHeaderInclusion(
                ...
                NestedSetBuilder.create(
                    Order.STABLE_ORDER,
                    treeArtifact(derivedRoot.getRelative("tree_artifact2")))));
  }
  ...
  public void nonWindowsPlatform_usesExactCaseResolution() throws Exception {
    ...
    assertThat(result.toList()).containsExactly(resolvedArtifact);
    verify(artifactResolver, never())
        .resolveSourceArtifactsAsciiCaseInsensitively(any(), any());
  }
  ...
  public void windowsPlatform_multipleDeps_mixedResolution() throws Exception {
    ...
    NestedSet<Artifact> result =
        discoverInputs(
            ...
            artifactResolver,
            ImmutableList.of(
                execRoot.getRelative("pkg/source.h"),
                derivedRoot.getRelative("gen/gen.h")),
  ...
  private SourceArtifact sourceArtifact(String execPath) {
    return new SourceArtifact(
        sourceRoot, PathFragment.create(execPath), ArtifactOwner.NULL_OWNER);
  }
  ...
  private ActionsTestUtil.NullAction windowsAction(NestedSet<Artifact> inputs) {
    return new ActionsTestUtil.NullAction(
        actionOwnerWithPlatform(windowsPlatform()), inputs);
  }
