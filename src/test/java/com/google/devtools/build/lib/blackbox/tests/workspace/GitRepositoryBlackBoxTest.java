 ...
 *
 * We use {@link GitRepositoryHelper} and {@link RepoWithRuleWritingTextGenerator} helper classes.
 *
 * 1. We are creating some git repository with the preset contents, which will be used for
 * fetching contents for the test. We plan to fetch contents specifying either commit hash,
 * tag, or branch.
 * For all test variants, we are creating the same repository, as the same HEAD commit
 * is marked with a tag, and can be addressed with commit hash, master branch, and tag name.
 *
 * 2. The contents of the git repository working tree is generated by
 * {@link RepoWithRuleWritingTextGenerator}. We pass some certain text to that generator;
 * that exact text should appear as a result of the build of the generated target
 * "call_write_text" in the file "out.txt".
 *
 * 3. We generate the new_git_repository repository rule, which refers to the git repository,
 * created in #1, specifying different git repository attributes in each test.
 * We call 'bazel build' for the "call_write_text" target of the external repository,
 * and asserting the contents in the "out.txt" file.
 */
public class GitRepositoryBlackBoxTest extends AbstractBlackBoxTest {
   ...
   * Please see the general approach description in the class javadoc comment.
   */
  @Test
  ...
  /**
   * Tests usage of new_git_repository workspace rule with the "commit" attribute.
   * Please see the general approach description in the class javadoc comment.
   */
  @Test
  ...
  /**
   * Tests usage of new_git_repository workspace rule with the "branch" attribute.
   * Please see the general approach description in the class javadoc comment.
   */
  @Test
