 ...
 * specific branch or default branch. For all test variants, we are creating the same repository,
 * as the same HEAD commit is marked with a tag, and can be addressed with commit hash, main
 * branch, and tag name.
 *
...
public class GitRepositoryBlackBoxTest extends AbstractBlackBoxTest {
   ...
   * attribute, which means the repository's default branch is to be checked out. Please see
   * the general approach description in the class javadoc comment.
   */
  @Test
