...
public class Cpp20ModuleCompileTest extends BuildViewTestCase {
  private void enableCpp20Module() throws Exception {
    getAnalysisMock()
            ...
            .setupCcToolchainConfig(
                    mockToolsConfig,
                    Crosstool.CcToolchainConfig.builder().withFeatures(CppRuleClasses.CPP20_MODULE));
  }
  ...
  public void testCpp20ModuleFeatureNotEnabled1() throws Exception {
    ...
    scratch.file("foo/BUILD", "cc_library(name='foo', module_interfaces=['foo.cppm'])");
    scratch.file("foo/foo.cppm", "foo");
    getConfiguredTarget("//foo:foo");
    assertContainsEvent("to use C++20 Modules, the feature cpp20_module must be enabled");
  }
  ...
  public void testCpp20ModuleFeatureEnabled() throws Exception {
    ...
    scratch.file("foo/foo.cppm", "foo");
    getConfiguredTarget("//foo:foo");
    assertDoesNotContainEvent("to use C++20 Modules, the feature cpp20_module must be enabled");
  }
  ...
  public void testObjectFile() throws Exception {
    ...
    ConfiguredTarget archiveInSrcsTest =
            scratchConfiguredTarget(
                    "cc_in_module_interfaces",
                    "cc_in_module_interfaces_test",
  ...
  public void testModuleCompileOption() throws Exception {
    enableCpp20Module();
    useConfiguration("--cpu=k8");
    scratch.file("a/BUILD", "cc_library(",
            "name='foo', ",
            "features = ['cpp20_module'], ",
  ...
  public void testModuleCompileOnly() throws Exception {
    ...
    scratch.file("a/BUILD", "cc_library(",
            ...
            ")");
    ConfiguredTarget target = getConfiguredTarget("//a:foo");
    var outputs = getOutputGroup(target, OutputGroupInfo.FILES_TO_COMPILE).toList();
