...
public final class BazelMockCcSupport extends MockCcSupport {
  ...
  public void setup(MockToolsConfig config) throws IOException {
    ...
    for (String ruleName : ImmutableList.of("cc_binary", "cc_import", "cc_library",
        "cc_shared_library", "cc_static_library", "cc_test", "objc_import", "objc_library")) {
      config.overwrite("third_party/bazel_rules/rules_cc/cc/" + ruleName + ".bzl",
    ...
    for (String ruleName : ImmutableList.of("fdo_prefetch_hints", "fdo_profile")) {
      config.overwrite("third_party/bazel_rules/rules_cc/cc/toolchains/" + ruleName + ".bzl",
          MessageFormat.format("""
