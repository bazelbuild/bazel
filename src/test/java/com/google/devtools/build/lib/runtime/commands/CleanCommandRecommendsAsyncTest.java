...
public class CleanCommandRecommendsAsyncTest {
  ...
  public CleanCommandRecommendsAsyncTest(boolean asyncOnCommandLine, OS os, boolean expectSuggestion)
      throws Exception {
  ...
  public void testCleanProvidesExpectedSuggestion() throws Exception {
    ...
    boolean async = CleanCommand.canUseAsync(this.asyncOnCommandLine, /* expunge= */ false,
        os, reporter);
    if (os != OS.LINUX) {
      assertThat(async).isFalse();
    }

    boolean matches = storedEventHandler.getEvents()
        .asList()
        .stream()
        .map(Event::getMessage)
        .anyMatch(event -> { return event.contains(EXPECTED_SUGGESTION); });
