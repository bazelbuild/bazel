...
public class RemoteSpawnRunnerTest {
  ...
  public void testRemoteExecutionTimeoutTimings() throws Exception {
    // If remote execution times out the SpawnResult should still have the start and wall times reported correctly.
    ...
    ActionResult cachedResult = ActionResult.newBuilder().setExitCode(0).setExecutionMetadata(executedMetadata).build();
    when(cache.downloadActionResult(
            any(RemoteActionExecutionContext.class),
            any(ActionKey.class),
            /* inlineOutErr= */ eq(false)))
    ...
    assertThat(res.getStartTime()).isEqualTo(Instant.ofEpochSecond(executionStart.getSeconds(), executionStart.getNanos()));
