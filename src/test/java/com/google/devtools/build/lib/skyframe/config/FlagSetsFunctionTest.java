...
""");
  ...
  @Test
  public void enforceCanonicalConfigsExtraNativeFlag_commandLine() throws Exception {
    scratch.file(
...
""");
  ...
  @Test
  @Ignore(
      "It's not possible to test the Starlark flag case in this test. Somewhere the test setup"
          + " doesn't recognize Starlark flags as being user options, and passes the build."
          + " flagset_test.sh tests the Starlark flag case.")
  public void enforceCanonicalConfigsExtraStarlarkFlag_commandLine() throws Exception {
    scratch.file(
        "test/build_settings.bzl",
        """
string_flag = rule(implementation = lambda ctx: [], build_setting = config.string(flag = True))
""");
  ...
  @Test
  public void noEnforceCanonicalConfigsExtraFlag_commandLine() throws Exception {
    scratch.file(
        "test/build_settings.bzl",
        """
string_flag = rule(implementation = lambda ctx: [], build_setting = config.string(flag = True))
""");
  ...
  @Test
  public void enforceCanonicalConfigsExtraFlag_notConfigAffecting() throws Exception {
    scratch.file(
        "test/build_settings.bzl",
        """
string_flag = rule(implementation = lambda ctx: [], build_setting = config.string(flag = True))
""");
    ...
    setBuildLanguageOptions(
        "--experimental_enable_scl_dialect=true", "--experimental_bzl_visibility");
