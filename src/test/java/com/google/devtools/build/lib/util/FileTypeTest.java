...
public class FileTypeTest {
  ...
  public void handlesPathObjects() {
    ...
    Path readme_uppercase = new InMemoryFileSystem(DigestHashFunction.SHA256).getPath("/readme.TXT");

    assertThat(TEXT.matches(readme)).isTrue();
    assertTrueOnWindows(TEXT.matches(readme_uppercase));
  }

  @Test
  public void handlesPathFragmentObjects() {
    PathFragment readme = PathFragment.create("some/where/readme.txt");
    PathFragment readme_uppercase = PathFragment.create("some/where/readme.TXT");
  ...
  public void checkingSingleWithTypePredicate() throws Exception {
    ...
    HasFileType item_uppercase = filename("config.TXT");

    assertThat(FileType.contains(item, TEXT)).isTrue();
    assertThat(FileType.contains(item, CFG)).isFalse();
    assertTrueOnWindows(FileType.contains(item_uppercase, TEXT));
  }
  ...
  public void filteringWithAlwaysTrue() throws Exception {
    ...
    assertThat(FileType.filter(unfiltered, FileTypeSet.ANY_FILE)).containsExactly(
        unfiltered.get(0), unfiltered.get(1), unfiltered.get(2), unfiltered.get(3),
        unfiltered.get(4)).inOrder();
  }
