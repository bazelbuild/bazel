...
public class ScrubberTest {
  ...
  public void matchExactMnemonic() {
    var scrubber = new Scrubber(Config.newBuilder().addRules(Config.Rule.newBuilder()
        .setMatcher(Config.Matcher.newBuilder().setMnemonic("Foo"))).build());
  ...
  public void matchUnionMnemonic() {
    var scrubber = new Scrubber(Config.newBuilder().addRules(Config.Rule.newBuilder()
        .setMatcher(Config.Matcher.newBuilder().setMnemonic("Foo|Bar"))).build());
  ...
  public void matchExactLabel() {
    var scrubber = new Scrubber(Config.newBuilder().addRules(Config.Rule.newBuilder()
        .setMatcher(Config.Matcher.newBuilder().setLabel("//foo:bar"))).build());
  ...
  public void matchUnionLabel() {
    var scrubber = new Scrubber(Config.newBuilder().addRules(Config.Rule.newBuilder()
        .setMatcher(Config.Matcher.newBuilder().setLabel("//foo:bar|//spam:eggs"))).build());
  ...
  public void matchWildcardLabel() {
    var scrubber = new Scrubber(Config.newBuilder().addRules(Config.Rule.newBuilder()
        .setMatcher(Config.Matcher.newBuilder().setLabel("//foo:.*"))).build());
  ...
  public void rejectToolAction() {
    ...
    assertThat(scrubber
        .forSpawn(createSpawn("//foo:bar", "Foo"))).isNotNull();
    assertThat(scrubber
        .forSpawn(createSpawn("//foo:bar", "Foo", /* forTool= */ true))).isNull();
  }

  @Test
  public void acceptToolAction() {
    var scrubber = new Scrubber(Config.newBuilder().addRules(Config.Rule.newBuilder()
            .setMatcher(Config.Matcher.newBuilder().setLabel("//foo:bar").setMnemonic("Foo")
                .setMatchTools(true)))
        .build());
    ...
    assertThat(scrubber
        .forSpawn(createSpawn("//foo:bar", "Foo"))).isNotNull();
    assertThat(scrubber
        .forSpawn(createSpawn("//foo:bar", "Foo", /* forTool= */ true))).isNotNull();
  }

  @Test
  public void noOmittedInputs() {
    var spawnScrubber = new Scrubber(
        Config.newBuilder().addRules(Config.Rule.getDefaultInstance()).build()).forSpawn(
        createSpawn());
  ...
  public void exactOmittedInput() {
    var spawnScrubber = new Scrubber(
        Config.newBuilder().addRules(Config.Rule.newBuilder()
                .setTransform(Config.Transform.newBuilder().addOmittedInputs("foo/bar")))
            .build()).forSpawn(createSpawn());
    ...
    assertThat(spawnScrubber.shouldOmitInput(
        ActionInputHelper.fromPath("bazel-out/foo/bar"))).isFalse();
  }
  ...
  public void multipleOmittedInputs() {
    ...
    assertThat(spawnScrubber.shouldOmitInput(
        ActionInputHelper.fromPath("bazel-out/foo/bar"))).isFalse();
    assertThat(
        spawnScrubber.shouldOmitInput(ActionInputHelper.fromPath("spam/eggs/bacon"))).isFalse();
    assertThat(spawnScrubber.shouldOmitInput(
        ActionInputHelper.fromPath("bazel-out/spam/eggs"))).isFalse();
  }
    ...
    var spawnScrubber = new Scrubber(
        Config.newBuilder().addRules(Config.Rule.newBuilder()
                .setTransform(Config.Transform.newBuilder().addArgReplacements(
                    Config.Replacement.newBuilder().setSource("foo").setTarget("bar"))))
            .build()).forSpawn(createSpawn());
  ...
  public void wildcardArgReplacement() {
    var spawnScrubber = new Scrubber(
        Config.newBuilder().addRules(Config.Rule.newBuilder()
                .setTransform(Config.Transform.newBuilder().addArgReplacements(
                    Config.Replacement.newBuilder().setSource("foo[12]").setTarget("bar"))))
            .build()).forSpawn(createSpawn());
    ...
    var spawnScrubber = new Scrubber(
        Config.newBuilder().addRules(Config.Rule.newBuilder()
                .setTransform(Config.Transform.newBuilder().addArgReplacements(
                        Config.Replacement.newBuilder().setSource("foo").setTarget("bar")).
                    addArgReplacements(
                        Config.Replacement.newBuilder().setSource("spam").setTarget("eggs"))))
            .build()).forSpawn(createSpawn());
  ...
  public void orthogonalRules() {
    var scrubber = new Scrubber(
        Config.newBuilder()
            .addRules(Config.Rule.newBuilder()
                .setMatcher(Config.Matcher.newBuilder().setLabel("//foo:bar"))
                .setTransform(Config.Transform.newBuilder()
                    .addArgReplacements(
                        Config.Replacement.newBuilder().setSource("foo").setTarget("bar"))))
            .addRules(Config.Rule.newBuilder()
  ...
  public void lastRuleWins() {
    var spawnScrubber = new Scrubber(
        Config.newBuilder()
            .addRules(Config.Rule.newBuilder().setTransform(Config.Transform.newBuilder()
                .addArgReplacements(
                    Config.Replacement.newBuilder().setSource("foo").setTarget("bar"))))
            .addRules(Config.Rule.newBuilder().setTransform(Config.Transform.newBuilder()
                .addArgReplacements(
  ...
  private static Spawn createSpawn(String label, String mnemonic, boolean forTool) {
    return new SpawnBuilder("cmd").withOwnerLabel(label).withMnemonic(mnemonic)
        .setBuiltForToolConfiguration(forTool).build();
  }
}
