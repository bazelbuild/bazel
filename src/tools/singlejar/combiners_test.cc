...
TEST_F(CombinersTest, Log4J2PluginDatCombinerSimple) {
  const std::string plugins_cache_path = "META-INF/org/apache/logging/log4j/core/config/plugins/Log4j2Plugins.dat";
  Log4J2PluginDatCombiner combiner(plugins_cache_path, false);

  InputJar input_jar1;
  ASSERT_TRUE(input_jar1.Open(runfiles->Rlocation("io_bazel/src/tools/singlejar/data/log4j2_plugins_set_1.jar").c_str()));
  ...
  InputJar input_jar2;
  ASSERT_TRUE(input_jar2.Open(runfiles->Rlocation("io_bazel/src/tools/singlejar/data/log4j2_plugins_set_2.jar").c_str()));
  ...
  std::ifstream expected_file(runfiles->Rlocation("io_bazel/src/tools/singlejar/data/log4j2_plugins_set_result.dat"), std::ios::binary);
  ...
  uint8_t buffer[256];
  ASSERT_EQ(Z_STREAM_END, inflater.Inflate((buffer), sizeof(buffer)));
  ASSERT_EQ(memcmp(reinterpret_cast<char *>(buffer), expected_content, expected_size), 0) << "Byte arrays are not equal";
  free(reinterpret_cast<void *>(entry));
}

TEST_F(CombinersTest, Log4J2PluginDatCombinerDuplicate) {
  const std::string plugins_cache_path = "META-INF/org/apache/logging/log4j/core/config/plugins/Log4j2Plugins.dat";
  ...
  ASSERT_TRUE(input_jar2.Open(runfiles->Rlocation("io_bazel/src/tools/singlejar/data/log4j2_plugins_set_1.jar").c_str()));
  while ((cdh = input_jar2.NextEntry(&lh))) {
    if (cdh->file_name_is(plugins_cache_path.c_str())) {
